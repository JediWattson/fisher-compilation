...
from fisher_graph import (
    complete_h4_fisher_soft_polarity_log_response as log_response_provider,
)
from fisher_graph import (
    gemma3_l3_l4_complete_h4_soft_polarity_log_response_nested_development as runner,
)
...
def _fold(
    ...
    *,
    ...
    response: tuple[float, float] = (0.125, 0.5),
    changed: bool = True,
    healthy: bool = True,
    anchors: bool = True,
) -> dict[str, object]:
    return {
        ...
        "held_objective_by_arm": {
            ...
            "same_response_unreflected": unreflected,
            "log_response_reflected": candidate,
            "log_response_reflected_exact_mirror": mirror,
        },
...
def test_parser_protocol_geometry_output_protection_and_cli() -> None:
    ...
    assert runner.DEFAULT_OUTPUT.name.endswith("v20k.json")
    ...
    assert (
        "fisher-graph-gemma-l3-l4-complete-h4-soft-polarity-v20k-"
        "log-response-nested"
    ) in pyproject
...
def test_inner_oof_response_selection_is_family_equal_and_tie_breaks_smaller() -> None:
    ...
    for index, family in enumerate(FAMILIES[:7]):
        ...
        objectives[runner._response_key((0.125, 0.5))] = 0.7 + index * 0.01
        objectives[runner._response_key((0.25, 0.5))] = 0.7 + index * 0.01
        evidence[family] = {
    ...
    assert tuple(selection["selected_response"]) == (0.125, 0.5)
    assert selection["selected_family_equal_objective"] == pytest.approx(0.73)
    assert selection["log_response_ladder_receipt"]["candidate_count"] == 11
    assert selection["log_response_selection_receipt"]["selected_candidate_id"] == (
        "log_response_03"
    )
...
def test_decision_requires_primary_mechanism_and_positive_changed_gates() -> None:
    ...
    insufficient_nonlinear = {
        family: _fold(
            family,
            response=(0.125, 0.5) if index < 4 else (0.125, 0.0),
        )
        for index, family in enumerate(FAMILIES)
    }
    failed_curvature = runner._aggregate_decision(insufficient_nonlinear)
    assert failed_curvature["selected_mix_positive_count"] == 4
    assert failed_curvature["curvature_evidence_gate_passed"] is False
    assert failed_curvature["primary_development_gate_passed"] is True
    assert failed_curvature["mechanism_gate_passed"] is False
    assert failed_curvature["development_oof_passed"] is False
...
def test_all_77_inner_candidates_freeze_before_first_capability(
    ...
    def freeze(*args, **kwargs):
        ...
        assert sum(len(rows) for rows in provider_hashes.values()) == 77
        traces = {
            family: {
    ...
    assert order == ["freeze_77", "capability"]
...
def test_scalar_fragment_is_mode_0600_and_v20i_shape_cannot_be_rehashed(
    ...
    runner._v20b._publish_scalar_fragment(
        ...
        label="test V20k reflection fold",
    )
...
def test_scalar_report_publication_is_mode_0600_and_contains_no_raw_tensors(
    ...
    runner._v20b._publish_scalar_fragment(
        ...
        label="test V20k log-response report",
    )
...
def test_existing_report_fast_path_is_model_free(
    ...
    output = tmp_path / "existing-v20k.json"
    output.write_text("{}")
    monkeypatch.setattr(runner, "_validate_output", lambda value: Path(value))
    monkeypatch.setattr(
        runner,
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: pytest.fail("existing V20k report constructed Gemma"),
    )
    monkeypatch.setattr(
        runner,
    ...
    result = (
        runner.run_gemma3_l3_l4_complete_h4_soft_polarity_log_response_nested_development(
            output=output
        )
    )
...
def test_completed_fold_resume_aggregates_without_constructing_model(
    ...
    output = tmp_path / "resumed-v20k.json"
    ...
    monkeypatch.setattr(
        runner,
        "prepare_complete_h4_rank320_live_context",
        lambda **kwargs: pytest.fail("resumed V20k folds constructed Gemma"),
    )
    monkeypatch.setattr(
        runner._v20b,
...
def _synthetic_log_response_provider_receipt(
    *,
    role: str,
    ...
    """Build tensor-free metadata identical to a log-response provider receipt."""
    ...
    provider_type = (
        log_response_provider.AutonomousCompleteH4FisherSoftPolarityLogResponseProvider
    )
    fake._payload = lambda: provider_type._payload(fake)
    provider_payload = fake._payload()
    provider_artifact = log_response_provider._sha256(
        log_response_provider._PROVIDER_DOMAIN, provider_payload
    )
    ...
    receipt = runner._hashed(
        {
            ...
            "box_certificate": (
                runner.fisher_soft_polarity_log_response_box_certificate(
...
def _synthetic_nonlog_response_provider_receipt(
    *,
    role: str,
...
def _inner_replay_fixture() -> tuple[
    ...
    for family in families:
        ...
        for response in runner._RESPONSES:
            ...
            provider, provider_receipt = _synthetic_log_response_provider_receipt(
                role="inner_reflected_response_candidate",
                response=response,
                direction=direction,
            ...
            trace = runner._hashed(
                {
                    ...
                    "arm": (
                        f"inner_{family}_radius_{radius.hex()}_mix_{mix.hex()}"
                    ),
    ...
    manifest = runner._hashed(
        {
            ...
            "log_response_ladder_receipt_sha256": (
                runner._LOG_RESPONSE_LADDER_RECEIPT_SHA256
            ),
    ...
    receipt = runner._hashed(
        {
            ...
            "exact_inner_execution_count": 7 * 11 * 2,
            "inner_endpoint_retrained_per_fold": False,
            "inner_held_family_used_for_endpoint_fit": True,
            "inner_claim_scope": (
...
def _forge_inner_log_response_provider_payload(
    receipt: dict[str, object],
    *,
    family: str,
    response: tuple[float, float],
    ...
    if field == "box_certificate.proof":
        ...
        provider_artifact = log_response_provider._sha256(
            log_response_provider._PROVIDER_DOMAIN,
...
def test_fully_rehashed_inner_provider_accounting_tamper_is_rejected(
    ...
    forged = copy.deepcopy(receipt)
    family = FAMILIES[0]
    key = runner._response_key((0.125, 0.5))
    manifest = forged["inner_provider_manifest"]
    provider_receipt = manifest[
        "provider_receipts_by_inner_family_and_response"
    ][family][key]
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("gain_formula", "forged_log_response_gain_formula"),
        ("box_certificate.proof", "forged_box_certificate_proof"),
    ),
)
def test_fully_rehashed_log_response_payload_tamper_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
    payload_field: str,
    tampered_value: object,
) -> None:
    ...
    forged = _forge_inner_log_response_provider_payload(
        receipt,
        family=FAMILIES[0],
        response=(0.125, 0.5),
        field=payload_field,
...
def _outer_replay_fixture(tmp_path: Path) -> tuple[dict[str, object], ...]:
    ...
    output = tmp_path / "v20k.json"
    source = {
        "artifact_sha256": _sha("v20k-source"),
        "v20j_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20j:{family}") for family in FAMILIES
        },
    }
    ...
    soft_transfers = {
        ...
        "same_response_unreflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
        ...
        "log_response_reflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
        ...
        "log_response_reflected_exact_mirror": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
    ...
    for arm in runner._ARMS:
        if arm in soft_transfers:
            direction = {
                ...
                "same_response_unreflected": source_direction[
                    "natural_direction"
                ],
                "log_response_reflected": selected_direction,
                "log_response_reflected_exact_mirror": tuple(
            ...
            providers[arm], receipts[arm] = (
                _synthetic_log_response_provider_receipt(
                    role=arm,
                    response=response,
                    direction=direction,
            ...
            receipts[arm] = _synthetic_nonlog_response_provider_receipt(
                role=arm,
    ...
    fragment = {
        ...
        "log_response_fit_protocol_sha256": runner._log_response_fit.SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256,
        ...
        "fragment_sha256": _sha("v20k-fragment"),
    }
    return (
        fragment,
        source,
