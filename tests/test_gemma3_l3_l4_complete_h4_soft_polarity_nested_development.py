...
import fisher_graph.gemma3_l3_l4_complete_h4_soft_polarity_nested_development as runner
...
def test_parser_defaults_to_separate_v20f_output() -> None:
...
def test_prerequisites_authenticate_before_model_and_failed_oof_never_refits(
    ...
    report = runner.run_gemma3_l3_l4_complete_h4_soft_polarity_nested_development(
        output=output
    )
...
def test_existing_report_fast_path_never_constructs_model(
    ...
    assert runner.run_gemma3_l3_l4_complete_h4_soft_polarity_nested_development(
        output=output
    ) == {"authenticated": True}
