...
from fisher_graph import (
    complete_h4_fisher_soft_polarity_signed_stack as signed_stack_provider,
)
from fisher_graph import (
    gemma3_l3_l4_complete_h4_soft_polarity_signed_stack_nested_development as runner,
)
...
def _fold(
    ...
    *,
    ...
    response: tuple[float, float] = (0.125, -0.125),
    changed: bool = True,
    healthy: bool = True,
    anchors: bool = True,
) -> dict[str, object]:
    return {
        ...
        "held_objective_by_arm": {
            ...
            "same_stack_unreflected": unreflected,
            "signed_stack_reflected": candidate,
            "signed_stack_reflected_exact_mirror": mirror,
        },
...
def test_parser_protocol_geometry_output_protection_and_cli() -> None:
    ...
    assert runner.DEFAULT_OUTPUT.name.endswith("v20l.json")
    ...
    assert (
        "fisher-graph-gemma-l3-l4-complete-h4-soft-polarity-v20l-"
        "signed-stack-nested"
    ) in pyproject
...
def test_response_pair_rejects_noncanonical_or_off_ladder_values(
    value: tuple[object, object],
) -> None:
    with pytest.raises(ValueError):
        runner._response_pair(value)


def test_inner_oof_response_selection_is_family_equal_and_signed_tie_break_is_fixed() -> None:
    ...
    for index, family in enumerate(FAMILIES[:7]):
        ...
        objectives[runner._response_key((0.125, -0.125))] = 0.7 + index * 0.01
        objectives[runner._response_key((0.125, 0.125))] = 0.7 + index * 0.01
        objectives[runner._response_key((0.25, -0.125))] = 0.7 + index * 0.01
        evidence[family] = {
    ...
    assert tuple(selection["selected_response"]) == (0.125, -0.125)
    assert selection["selected_family_equal_objective"] == pytest.approx(0.73)
    assert selection["signed_stack_ladder_receipt"]["candidate_count"] == 15
    assert selection["signed_stack_selection_receipt"]["selected_candidate_id"] == (
        "signed_stack_02"
    )
...
def test_decision_requires_primary_mechanism_signed_stack_and_changed_gates() -> None:
    ...
    insufficient_signed_stack = {
        family: _fold(
            family,
            response=(0.125, -0.125) if index < 2 else (
                (0.125, 0.125) if index < 4 else (0.125, 0.0)
            ),
        )
        for index, family in enumerate(FAMILIES)
    }
    failed_stack = runner._aggregate_decision(insufficient_signed_stack)
    assert failed_stack["selected_signed_mix_nonzero_count"] == 4
    assert failed_stack["signed_stack_evidence_gate_passed"] is False
...
def test_exact_canonical_work_accounting() -> None:
    ...
    assert work["inner_conditional_leave_one_family_out_model_forward_count"] == 1680
    assert work["outer_held_model_forward_count"] == 112
    assert work["canonical_model_forward_count"] == 1936
    assert work["canonical_suffix_backward_count"] == 128
    assert work["canonical_local_autograd_contraction_count"] == 112
    assert work["canonical_teacher_access_count"] == 1904
    assert work["masked_fisher_solve_count"] == 56
    assert work["signed_stack_candidate_count"] == 840
    assert work["inner_provider_candidate_count"] == 840
    assert work["outer_arm_provider_count"] == 56
    assert work["all_eight_final_refit_model_forward_count"] == 0


def test_all_105_inner_candidates_freeze_before_first_capability(
    ...
    def freeze(*args, **kwargs):
        ...
        assert sum(len(rows) for rows in provider_hashes.values()) == 105
        traces = {
            family: {
    ...
    assert order == ["freeze_105", "capability"]
...
def test_scalar_fragment_is_mode_0600_and_v20i_shape_cannot_be_rehashed(
    ...
    runner._v20b._publish_scalar_fragment(
        ...
        label="test V20l reflection fold",
    )
...
def test_scalar_report_publication_is_mode_0600_and_contains_no_raw_tensors(
    ...
    runner._v20b._publish_scalar_fragment(
        ...
        label="test V20l signed-stack report",
    )
...
def test_existing_report_fast_path_is_model_free(
    ...
    output = tmp_path / "existing-v20l.json"
    output.write_text("{}")
    monkeypatch.setattr(runner, "_validate_output", lambda value: Path(value))
    monkeypatch.setattr(
        runner,
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: pytest.fail("existing V20l report constructed Gemma"),
    )
    monkeypatch.setattr(
        runner,
    ...
    result = (
        runner.run_gemma3_l3_l4_complete_h4_soft_polarity_signed_stack_nested_development(
            output=output
        )
    )
...
def test_completed_fold_resume_aggregates_without_constructing_model(
    ...
    output = tmp_path / "resumed-v20l.json"
    ...
    monkeypatch.setattr(
        runner,
        "prepare_complete_h4_rank320_live_context",
        lambda **kwargs: pytest.fail("resumed V20l folds constructed Gemma"),
    )
    monkeypatch.setattr(
        runner._v20b,
...
def _synthetic_signed_stack_provider_receipt(
    *,
    role: str,
    ...
    """Build tensor-free metadata identical to a signed-stack provider receipt."""
    ...
    provider_type = (
        signed_stack_provider.AutonomousCompleteH4FisherSoftPolaritySignedStackProvider
    )
    fake._payload = lambda: provider_type._payload(fake)
    provider_payload = fake._payload()
    provider_artifact = (
        signed_stack_provider.fisher_soft_polarity_signed_stack_provider_artifact_sha256(
            provider_payload
        )
    )
    ...
    receipt = runner._hashed(
        {
            ...
            "box_certificate": (
                runner.fisher_soft_polarity_signed_stack_box_certificate(
...
def _synthetic_nonsigned_stack_provider_receipt(
    *,
    role: str,
...
def _inner_replay_fixture() -> tuple[
    ...
    for family in families:
        ...
        for response in runner._RESPONSES:
            ...
            provider, provider_receipt = _synthetic_signed_stack_provider_receipt(
                role="inner_reflected_response_candidate",
                response=response,
                direction=direction,
            ...
            trace = runner._hashed(
                {
                    ...
                    "arm": (
                        f"inner_{family}_radius_{radius.hex()}_signed_mix_"
                        f"{mix.hex()}"
                    ),
    ...
    manifest = runner._hashed(
        {
            ...
            "signed_stack_ladder_receipt_sha256": (
                runner._SIGNED_STACK_LADDER_RECEIPT_SHA256
            ),
    ...
    receipt = runner._hashed(
        {
            ...
            "exact_inner_execution_count": 7 * 15 * 2,
            "inner_endpoint_retrained_per_fold": False,
            "inner_held_family_used_for_endpoint_fit": True,
            "inner_claim_scope": (
...
def _forge_inner_signed_stack_provider_payload(
    receipt: dict[str, object],
    *,
    family: str,
    response: tuple[float, float],
    ...
    if field == "box_certificate.proof":
        ...
        provider_artifact = signed_stack_provider._sha256(
            signed_stack_provider._PROVIDER_DOMAIN,
...
def test_fully_rehashed_inner_provider_accounting_tamper_is_rejected(
    ...
    forged = copy.deepcopy(receipt)
    family = FAMILIES[0]
    key = runner._response_key((0.125, -0.125))
    manifest = forged["inner_provider_manifest"]
    provider_receipt = manifest[
        "provider_receipts_by_inner_family_and_response"
    ][family][key]
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("signed_mix_sha256", _sha("forged-mix")),
        ...
        ("gain_formula", "forged_signed_stack_gain_formula"),
        ("box_certificate.proof", "forged_box_certificate_proof"),
    ),
)
def test_fully_rehashed_signed_stack_payload_tamper_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
    payload_field: str,
    tampered_value: object,
) -> None:
    ...
    forged = _forge_inner_signed_stack_provider_payload(
        receipt,
        family=FAMILIES[0],
        response=(0.125, -0.125),
        field=payload_field,
...
def test_rehashed_inner_manifest_cannot_break_provider_or_trace_binding(
    ...
    manifest[field][FAMILIES[0]][runner._response_key((0.0, 0.0))] = _sha(
        f"forged:{binding}"
    )
...
def _outer_replay_fixture(tmp_path: Path) -> tuple[dict[str, object], ...]:
    ...
    output = tmp_path / "v20l.json"
    source = {
        "artifact_sha256": _sha("v20l-source"),
        "v20j_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20j:{family}") for family in FAMILIES
        },
        "v20k_fold_fragment_sha256s_by_family": {
    ...
    soft_transfers = {
        ...
        "same_stack_unreflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
        ...
        "signed_stack_reflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
        ...
        "signed_stack_reflected_exact_mirror": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
    ...
    for arm in runner._ARMS:
        if arm in soft_transfers:
            direction = {
                ...
                "same_stack_unreflected": source_direction[
                    "natural_direction"
                ],
                "signed_stack_reflected": selected_direction,
                "signed_stack_reflected_exact_mirror": tuple(
                    -item for item in selected_direction
                ),
            }[arm]
            response = (
                matched_linear_response
                if arm == "matched_linear_reflected"
                else selected_response
            )
            providers[arm], receipts[arm] = (
                _synthetic_signed_stack_provider_receipt(
                    role=arm,
                    response=response,
                    direction=direction,
            ...
            receipts[arm] = _synthetic_nonsigned_stack_provider_receipt(
                role=arm,
    ...
    fold = runner._hashed(
        {
            ...
            "selected_signed_mix_nonzero": True,
    ...
    fragment = {
        ...
        "signed_stack_fit_protocol_sha256": runner._signed_stack_fit.SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256,
        ...
        "fragment_sha256": _sha("v20l-fragment"),
    }
    return (
        fragment,
        source,
