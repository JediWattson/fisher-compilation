...
from fisher_graph import (
    complete_h4_fisher_soft_polarity_simplex_response as simplex_response_provider,
)
from fisher_graph import (
    gemma3_l3_l4_complete_h4_soft_polarity_simplex_response_nested_development as runner,
)
    ...
    *,
    ...
    return {
        ...
        "held_objective_by_arm": {
            ...
            "simplex_response_reflected": candidate,
...
def _source() -> dict[str, object]:
    return {
        ...
        "v20j_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20j:{family}") for family in FAMILIES
        },
        "v20k_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20k:{family}") for family in FAMILIES
        },
        "v20l_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20l:{family}") for family in FAMILIES
        },
    }
...
def test_parser_protocol_geometry_output_protection_and_cli() -> None:
    ...
    assert runner.DEFAULT_OUTPUT.name.endswith("v20m.json")
    ...
    with pytest.raises(ValueError, match="preserve immutable prerequisite"):
        runner._validate_output(runner._v20j.DEFAULT_OUTPUT)
    with pytest.raises(ValueError, match="preserve immutable prerequisite"):
        runner._validate_output(runner._v20k.DEFAULT_OUTPUT)
    with pytest.raises(ValueError, match="preserve immutable prerequisite"):
        runner._validate_output(runner._v20l.DEFAULT_OUTPUT)
    ...
    assert (
        "fisher-graph-gemma-l3-l4-complete-h4-soft-polarity-v20m-"
        "simplex-response-nested"
    ) in pyproject
    assert "full_vocabulary_KL_teacher_to_candidate" in str(
        runner._FIXED_PROTOCOL["response_selection"]
    )
...
def test_prerequisites_fail_closed_when_any_immediate_v20l_pin_is_unset(
    monkeypatch: pytest.MonkeyPatch, field: str
) -> None:
    assert runner._V20L_LOGICAL_SHA256 == (
        "803fc80586266936ee67601e85caee7b4357e5c3c7923805c0615b6ce96ad849"
    )
    assert runner._V20L_FILE_SHA256 == (
        "71f5d41b4bef4e2c55d674e105b2093a0e168d42ffdebf5f7fbe2c4fb57a4d38"
    )
    assert runner._V20L_SOURCE_SHA256 == (
        "d43273ea0a4555be88bb8afef20bff410dbbc53c148c063b148c600a78513094"
    )
    assert runner._V20L_FOLD_SHA256S == {
    ...
    with pytest.raises(RuntimeError, match="fail-closed.*V20l"):
...
def test_response_tuple_rejects_noncanonical_or_off_ladder_values(
    value: tuple[object, object, object],
) -> None:
    with pytest.raises(ValueError):
        runner._response_tuple(value)


def test_inner_oof_response_selection_is_family_equal_and_simplex_tie_break_is_fixed() -> None:
    evidence: dict[str, dict[str, object]] = {}
    for index, family in enumerate(FAMILIES[:7]):
        objectives = {
            runner._response_key(response): (
                2.0 + index * 0.01 + sum(response)
            )
            for response in runner._RESPONSES
        }
        objectives[runner._response_key((0.125, 0.125, -0.0625))] = (
            0.7 + index * 0.01
        )
        objectives[runner._response_key((0.125, 0.125, 0.0625))] = (
            0.7 + index * 0.01
        )
        objectives[runner._response_key((0.25, 0.125, -0.0625))] = (
            0.7 + index * 0.01
        )
        evidence[family] = {
            "artifact_sha256": _sha(f"inner:{family}"),
            "outer_held_family_id": FAMILIES[-1],
            "objective_by_response": objectives,
        }

    selection = runner._aggregate_response_selection(evidence)

    assert selection["inner_family_order"] == FAMILIES[:7]
    assert tuple(selection["selected_response"]) == (0.125, 0.125, -0.0625)
    assert selection["selected_family_equal_objective"] == pytest.approx(0.73)
    assert selection["simplex_response_ladder_receipt"]["candidate_count"] == 19
    assert selection["simplex_response_selection_receipt"]["selected_candidate_id"] == (
        "simplex_response_03"
    )
...
def test_decision_requires_primary_mechanism_simplex_response_and_changed_gates() -> None:
    ...
    assert decision["selected_interior_simplex_response_count"] == 8
    assert decision["selected_boundary_simplex_response_count"] == 0
    assert decision["selected_zero_bias_simplex_response_count"] == 0
    assert decision[
        "all_selected_interior_responses_distinguishable_from_matched_v20l_boundary"
    ] is True
    ...
    matched_boundary_failure = {
        family: _fold(family, matched_boundary=0.8 if index < 4 else 0.4)
        for index, family in enumerate(FAMILIES)
    }
    failed_boundary = runner._aggregate_decision(matched_boundary_failure)
    assert failed_boundary["primary_development_gate_passed"] is True
    assert failed_boundary["candidate_win_count_by_reference_arm"][
        "matched_v20l_boundary_reflected"
    ] == 4
    assert failed_boundary["mechanism_gate_passed"] is False

    insufficient_simplex_response = {
        family: _fold(
            family,
            response=(
                (0.125, 0.125, -0.0625)
    ...
    failed_stack = runner._aggregate_decision(insufficient_simplex_response)
    assert failed_stack["selected_nontrivial_simplex_response_count"] == 4
    assert failed_stack["simplex_response_evidence_gate_passed"] is False
    assert failed_stack["primary_development_gate_passed"] is True
    assert failed_stack["mechanism_gate_passed"] is False
    assert failed_stack["development_oof_passed"] is False

    indistinguishable_interior = dict(passing)
    indistinguishable_interior[FAMILIES[0]] = _fold(
        FAMILIES[0], changed_from_boundary=False
    )
    failed_interior = runner._aggregate_decision(indistinguishable_interior)
    assert failed_interior["selected_interior_simplex_response_count"] == 8
    assert failed_interior[
        "all_selected_interior_responses_distinguishable_from_matched_v20l_boundary"
    ] is False
    assert failed_interior["mechanism_gate_passed"] is False

    boundary_responses = {
        family: _fold(
            family,
            response=(0.125, 0.125, -0.125),
            changed_from_boundary=False,
        )
        for family in FAMILIES
    }
    boundary_decision = runner._aggregate_decision(boundary_responses)
    assert boundary_decision["selected_boundary_simplex_response_count"] == 8
    assert boundary_decision["selected_interior_simplex_response_count"] == 0
    assert boundary_decision[
        "all_selected_interior_responses_distinguishable_from_matched_v20l_boundary"
    ] is True

    zero_response = dict(passing)
    zero_response[FAMILIES[0]] = _fold(
        FAMILIES[0], response=(0.0, 0.0, 0.0)
    )
    failed_change = runner._aggregate_decision(zero_response)
    assert (
        failed_change[
...
def test_report_authorizes_only_next_fresh_shadow_and_never_refits_here(
    ...
    assert passed["exact_objective_kind"] == (
        "token_mean_exact_float64_full_vocabulary_kl_teacher_to_candidate"
    )
    assert passed["v20j_authority"]["report_sha256"] == _sha("v20j-report")
    assert passed["v20j_authority"]["file_sha256"] == _sha("v20j-file")
    assert passed["v20j_authority"]["source_receipt_sha256"] == _sha(
        "v20j-source"
    )
    assert passed["v20j_authority"]["rollback_to_base"] is True
    assert passed["v20k_authority"]["report_sha256"] == _sha("v20k-report")
    assert passed["v20k_authority"]["file_sha256"] == _sha("v20k-file")
    assert passed["v20k_authority"]["source_receipt_sha256"] == _sha(
        "v20k-source"
    )
    assert passed["v20k_authority"]["rollback_to_base"] is True
    assert passed["v20l_authority"]["report_sha256"] == _sha("v20l-report")
    assert passed["v20l_authority"]["file_sha256"] == _sha("v20l-file")
    assert passed["v20l_authority"]["source_receipt_sha256"] == _sha(
        "v20l-source"
    )
    assert passed["v20l_authority"]["rollback_to_base"] is True
...
def test_exact_canonical_work_accounting() -> None:
    ...
    assert work["inner_conditional_leave_one_family_out_model_forward_count"] == 2128
    assert work["outer_held_model_forward_count"] == 128
    assert work["canonical_model_forward_count"] == 2400
    assert work["canonical_suffix_backward_count"] == 128
    assert work["canonical_local_autograd_contraction_count"] == 112
    assert work["canonical_teacher_access_count"] == 2368
    assert work["masked_fisher_solve_count"] == 56
    assert work["simplex_response_candidate_count"] == 1064
    assert work["inner_provider_candidate_count"] == 1064
    assert work["outer_arm_provider_count"] == 64
    assert work["all_eight_final_refit_model_forward_count"] == 0


def test_all_133_inner_candidates_freeze_before_first_capability(
    ...
    def freeze(*args, **kwargs):
        ...
        order.append("freeze_133")
        provider_hashes = {
            family: {
                runner._response_key(response): _sha(
                    f"provider:{family}:{response}"
                )
                for response in runner._RESPONSES
            }
            for family in inner_families
        }
        assert sum(len(rows) for rows in provider_hashes.values()) == 133
        traces = {
            family: {
                response: {"artifact_sha256": _sha(f"trace:{family}:{response}")}
                for response in runner._RESPONSES
            }
            for family in inner_families
        }
        manifest = {
            ...
            "provider_artifact_sha256s_by_inner_family_and_response": (
                provider_hashes
            ),
    ...
    assert order == ["freeze_133", "capability"]


def test_all_eight_outer_arms_freeze_before_capability(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    outer = FAMILIES[0]
    records = tuple(
    ...
    with pytest.raises(FirstCapability):
        runner._score_outer_arms(
            ...
            outer_family_id=outer,
            authenticated_v20g_fold={},
            authenticated_v20l_fold={},
        )

    assert order == ["freeze_8", "capability"]


def test_execute_outer_fold_threads_v20l_authority_only_to_outer_score(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    outer = FAMILIES[0]
    ...
    authenticated_v20l_fold = {
        "fragment_sha256": _sha("v20l-fold"),
    }
    ...
    def fit_inner(*args, **kwargs):
        calls["inner_kwargs"] = kwargs
        return (
            {"artifact_sha256": _sha("inner")},
            {"selected_response": (0.25, 0.125, -0.0625)},
        )

    monkeypatch.setattr(runner, "_fit_inner_response", fit_inner)
    monkeypatch.setattr(
        runner._reflection,
        "build_soft_polarity_reflection_fit_receipt",
        lambda **kwargs: {"artifact_sha256": _sha("reflection")},
    )
    monkeypatch.setattr(
        runner,
    ...
    def score_outer(*args, **kwargs):
        calls["outer_kwargs"] = kwargs
        return (
            {"artifact_sha256": _sha("manifest")},
            {"artifact_sha256": _sha("held")},
            {"artifact_sha256": _sha("fold")},
        )

    monkeypatch.setattr(runner, "_score_outer_arms", score_outer)

    live = runner._execute_outer_fold(
        object(),
        (),
        object(),
        family_ids=FAMILIES,
        ...
        authenticated_v20i_fold={},
        authenticated_v20l_fold=authenticated_v20l_fold,
    )

    assert "authenticated_v20l_fold" not in calls["inner_kwargs"]
    assert (
        calls["outer_kwargs"]["authenticated_v20l_fold"]
        is authenticated_v20l_fold
    )
    assert live.fold_receipt["artifact_sha256"] == _sha("fold")


def test_scalar_fragment_is_mode_0600_and_v20i_shape_cannot_be_rehashed(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    path = tmp_path / "v20m.fold.json"
    runner._v20b._publish_scalar_fragment(
        ...
        label="test V20m reflection fold",
    )
    ...
    with pytest.raises(ValueError, match="key set differs"):
        runner._load_fold_fragment(
            ...
            authenticated_v20l_fold={"fragment_sha256": _sha("v20l")},
        )


def test_scalar_report_publication_is_mode_0600_and_contains_no_raw_tensors(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    output = tmp_path / "v20m.json"
    monkeypatch.setattr(runner, "_validate_output", lambda value: Path(value))
    ...
    runner._v14._scalar_report(report)
    runner._v20b._publish_scalar_fragment(
        report,
        path=output,
        domain=runner._REPORT_DOMAIN,
        hash_key="report_sha256",
        label="test V20m simplex-response report",
    )

    assert output.stat().st_mode & 0o777 == 0o600
    assert report["integrity"][
        "raw_prompts_tokens_logits_h4_gradients_or_provider_tensors_serialized"
    ] is False
...
def test_existing_report_fast_path_is_model_free(
    ...
    output = tmp_path / "existing-v20m.json"
    output.write_text("{}")
    monkeypatch.setattr(runner, "_validate_output", lambda value: Path(value))
    monkeypatch.setattr(
        runner,
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: pytest.fail("existing V20m report constructed Gemma"),
    )
    monkeypatch.setattr(
        runner,
    ...
    result = (
        runner.run_gemma3_l3_l4_complete_h4_soft_polarity_simplex_response_nested_development(
            output=output
        )
    )
...
def test_completed_fold_resume_aggregates_without_constructing_model(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    output = tmp_path / "resumed-v20m.json"
    ...
    authority = {family: {} for family in FAMILIES}
    monkeypatch.setattr(runner, "_validate_output", lambda value: Path(value))
    monkeypatch.setattr(
        runner,
        "_load_prerequisites",
    ...
    monkeypatch.setattr(
        runner,
        "_fold_path",
        lambda _output, family: fold_paths[family],
    )
    monkeypatch.setattr(
        runner,
        "_load_fold_fragment",
    ...
    monkeypatch.setattr(
        runner,
        "prepare_complete_h4_rank320_live_context",
        lambda **kwargs: pytest.fail("resumed V20m folds constructed Gemma"),
    )
    monkeypatch.setattr(
        runner._v20b,
        "_publish_scalar_fragment",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        runner,
        "_load_existing_report",
        lambda *args, **kwargs: {"authenticated": True, "resumed": True},
    )
    ...
    assert result == {"authenticated": True, "resumed": True}
...
def _synthetic_simplex_response_provider_receipt(
    *,
    role: str,
    ...
    """Build tensor-free metadata identical to a simplex-response provider receipt."""
    ...
    fake = SimpleNamespace(
        ...
        incremental_prepared_float_scalar_count=42_263,
    ...
    provider_type = (
        simplex_response_provider.AutonomousCompleteH4FisherSoftPolaritySimplexResponseProvider
    )
    fake._payload = lambda: provider_type._payload(fake)
    provider_payload = fake._payload()
    provider_artifact = (
        simplex_response_provider.fisher_soft_polarity_simplex_response_provider_artifact_sha256(
            provider_payload
        )
    )
    ...
    receipt = runner._hashed(
        {
            ...
            "box_certificate": (
                runner.fisher_soft_polarity_simplex_response_box_certificate(
...
def _synthetic_nonsimplex_response_provider_receipt(
    *,
    role: str,
...
def _inner_replay_fixture() -> tuple[
    ...
    for family in families:
        ...
        for response in runner._RESPONSES:
            key = runner._response_key(response)
            radius, u, v = response
            ...
            provider, provider_receipt = _synthetic_simplex_response_provider_receipt(
                role="inner_reflected_response_candidate",
                response=response,
                direction=direction,
            ...
            trace = runner._hashed(
                {
                    ...
                    "arm": (
                        f"inner_{family}_radius_{radius.hex()}_u_{u.hex()}_"
                        f"v_{v.hex()}"
                    ),
    ...
    manifest = runner._hashed(
        {
            ...
            "response_order": runner._RESPONSES,
            "simplex_response_ladder_receipt_sha256": (
                runner._SIMPLEX_RESPONSE_LADDER_RECEIPT_SHA256
            ),
            ...
            "provider_transfer_evidence_sha256s_by_inner_family_and_response": transfer_hashes,
    ...
    for family in families:
        ...
        trace_bundle = runner._v14._sha256(
            {
                runner._response_key(response): trace_hashes[family][
                    runner._response_key(response)
                ]
                for response in runner._RESPONSES
            },
        ...
        inner_evidence[family] = runner._hashed(
            {
                ...
                "capability_receipt": {},
                "exact_execution_count": len(runner._RESPONSES) * 2,
                "zero_response_exact_v20g_eta_zero_output_anchor": True,
    ...
    selection = runner._aggregate_response_selection(inner_evidence)
    receipt = runner._hashed(
        {
            "outer_held_family_id": outer,
            "source_direction_receipt_sha256": source["artifact_sha256"],
            ...
            "exact_inner_execution_count": 7 * 19 * 2,
            "inner_endpoint_retrained_per_fold": False,
            "inner_held_family_used_for_endpoint_fit": True,
            "inner_claim_scope": (
...
def _rehash_inner_provider_manifest_chain(
    ...
    for family, evidence in forged["inner_evidence_by_family"].items():
        ...
        for response in runner._RESPONSES:
            key = runner._response_key(response)
            arm = evidence["response_evidence"][key]
...
def _forge_inner_simplex_response_provider_payload(
    ...
    *,
    ...
    provider_receipt = manifest[
        "provider_receipts_by_inner_family_and_response"
    ][family][key]
    if field == "box_certificate.proof":
        ...
        provider_artifact = simplex_response_provider._sha256(
            simplex_response_provider._PROVIDER_DOMAIN,
    ...
    manifest["provider_receipts_by_inner_family_and_response"][family][key] = (
        _rehash(provider_receipt, runner._PROVIDER_DOMAIN)
    )
    manifest["provider_artifact_sha256s_by_inner_family_and_response"][family][
        key
    ] = provider_artifact

    arm = forged["inner_evidence_by_family"][family]["response_evidence"][key]
    ...
    manifest["response_trace_sha256s_by_inner_family_and_response"][family][
        key
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("prepared_float_scalar_count", 560_008),
...
def test_fully_rehashed_inner_provider_accounting_tamper_is_rejected(
    ...
    forged = copy.deepcopy(receipt)
    family = FAMILIES[0]
    key = runner._response_key((0.125, 0.125, -0.0625))
    manifest = forged["inner_provider_manifest"]
    provider_receipt = manifest[
        "provider_receipts_by_inner_family_and_response"
    ][family][key]
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("radius_sha256", _sha("forged-radius")),
        ("shrink_mass_sha256", _sha("forged-shrink-mass")),
        ("polarity_bias_sha256", _sha("forged-polarity-bias")),
        ...
        ("gain_formula", "forged_simplex_response_gain_formula"),
        ("box_certificate.proof", "forged_box_certificate_proof"),
    ),
)
def test_fully_rehashed_simplex_response_payload_tamper_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
    payload_field: str,
    tampered_value: object,
) -> None:
    ...
    forged = _forge_inner_simplex_response_provider_payload(
        receipt,
        family=FAMILIES[0],
...
def test_rehashed_true_zero_response_boolean_cannot_bypass_exact_v20g_anchor(
    ...
    forged = copy.deepcopy(receipt)
    family = FAMILIES[0]
    zero_response = (0.0, 0.0, 0.0)
    key = runner._response_key(zero_response)
    arm = forged["inner_evidence_by_family"][family]["response_evidence"][key]
    example = f"{family}:0"
    ...
    manifest = forged["inner_provider_manifest"]
    trace_hashes = manifest["response_trace_sha256s_by_inner_family_and_response"][
        family
    ]
    trace_bundle = runner._v14._sha256(
        {runner._response_key(r): trace_hashes[runner._response_key(r)] for r in runner._RESPONSES},
        domain=runner._INNER_EXECUTION_DOMAIN,
    )
    provider = arm["provider_artifact_sha256"]
    seed = runner._v14._sha256(
    ...
    inner["response_evidence"][key] = _rehash(
        arm, runner._INNER_EXECUTION_DOMAIN
    )
    inner["objective_by_response"][key] = arm["objective"]
    forged["inner_evidence_by_family"][family] = _rehash(
    ...
    with pytest.raises(ValueError, match="zero-response V20g output anchor"):
        runner._validate_inner_receipt(
            forged,
            source_direction=source,
            outer_family_id=FAMILIES[-1],
...
def test_rehashed_inner_manifest_cannot_break_provider_or_trace_binding(
    ...
    field = (
        "provider_artifact_sha256s_by_inner_family_and_response"
        if binding == "provider"
        else "response_trace_sha256s_by_inner_family_and_response"
    )
    manifest[field][FAMILIES[0]][runner._response_key((0.0, 0.0, 0.0))] = _sha(
        f"forged:{binding}"
    )
...
def _outer_replay_fixture(tmp_path: Path) -> tuple[dict[str, object], ...]:
    ...
    output = tmp_path / "v20m.json"
    source = {
        "artifact_sha256": _sha("v20m-source"),
        "v20j_fold_fragment_sha256s_by_family": {
            family: _sha(f"v20j:{family}") for family in FAMILIES
        },
        "v20k_fold_fragment_sha256s_by_family": {
    ...
    response_selection = runner._hashed(
        {"selected_response": (0.125, 0.125, -0.0625)},
        domain=runner._RESPONSE_SELECTION_DOMAIN,
    )
    ...
    v20l_boundary_response = (0.125, 0.125, -0.125)
    boundary_objectives, boundary_h4, boundary_logits = outputs[
        "matched_v20l_boundary_reflected"
    ]
    authenticated_v20l = {
        "fragment_sha256": source["v20l_fold_fragment_sha256s_by_family"][outer],
        "fold_receipt": {
            "outer_held_family_id": outer,
            "selected_response": (0.125, -0.125),
        },
        "held_evidence": {
            "arm_evidence": {
                "signed_stack_reflected": {
                    "objective": sum(boundary_objectives.values()) / 2,
                    "objectives_by_example": copy.deepcopy(boundary_objectives),
                    "post_cast_h4_sha256s": copy.deepcopy(boundary_h4),
                    "supervised_full_vocab_logits_sha256s": copy.deepcopy(
                        boundary_logits
                    ),
                }
            }
    ...
    selected_response = tuple(response_selection["selected_response"])
    ...
    soft_transfers = {
        ...
        "matched_v20l_boundary_reflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
        ...
        "same_simplex_response_unreflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
        ...
        "simplex_response_reflected": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
        ...
        "simplex_response_reflected_exact_mirror": runner._provider_seed(
            endpoint_receipt_sha256=str(endpoint_receipt["artifact_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
    ...
    for arm in runner._ARMS:
        if arm in soft_transfers:
            direction = {
                ...
                "same_simplex_response_unreflected": source_direction[
                    "natural_direction"
                ],
                "simplex_response_reflected": selected_direction,
                "simplex_response_reflected_exact_mirror": tuple(
                    -item for item in selected_direction
                ),
            }[arm]
            response = {
                "matched_linear_reflected": matched_linear_response,
                "matched_v20l_boundary_reflected": v20l_boundary_response,
            ...
            providers[arm], receipts[arm] = (
                _synthetic_simplex_response_provider_receipt(
                    role=arm,
                    response=response,
                    direction=direction,
            ...
            receipts[arm] = _synthetic_nonsimplex_response_provider_receipt(
                role=arm,
    ...
    manifest = runner._hashed(
        {
            ...
            "selected_response": selected_response,
            "matched_linear_response": matched_linear_response,
            "matched_v20l_boundary_response": v20l_boundary_response,
            "matched_v20l_boundary_source_fold_sha256": authenticated_v20l[
                "fragment_sha256"
            ],
            "arm_order": runner._ARMS,
    ...
    fold = runner._hashed(
        {
            ...
            "selected_response": selected_response,
            "selected_response_key": runner._response_key(
                selected_response
            ),
            "selected_variant_artifact_sha256": reflection_fit[
                "selected_variant_artifact_sha256"
            ...
            "candidate_exact_execution_changed_from_matched_v20l_boundary": True,
            "selected_radius_positive": True,
            "selected_u_positive": True,
            "selected_interior_simplex_response": True,
            "selected_boundary_simplex_response": False,
            "selected_zero_bias_simplex_response": False,
    ...
    fragment = {
        ...
        "simplex_response_fit_protocol_sha256": runner._simplex_response_fit.SOFT_POLARITY_SIMPLEX_RESPONSE_FIT_PROTOCOL_SHA256,
        ...
        "endpoint_evidence": endpoint_evidence,
        "inner_receipt": {
            "response_selection_receipt": response_selection,
        ...
        "fragment_sha256": _sha("v20m-fragment"),
    }
    return (
        fragment,
        source,
...
def test_rehashed_v20i_reflection_lineage_substitution_is_rejected(
    ...
    fragment, _, _, _, _, authenticated_v20i, _, _ = _outer_replay_fixture(
        tmp_path
    )
...
def test_replay_accepts_canonical_json_sorted_mapping_keys(
    ...
    runner._validate_fold_fragment(
        ...
        authenticated_v20i_fold=authenticated_v20i,
        authenticated_v20l_fold=authenticated_v20l,
    )
