...
import fisher_graph.complete_h4_fisher_soft_polarity_log_response_fit as core
from fisher_graph.complete_h4_fisher_soft_polarity_log_response_fit import (
...
EXPECTED_PROTOCOL_SHA256 = (
    "9c66942a8c5275d5360987c5372339078b0c10d94cb8c335e60fe811902b0aec"
)
...
def _selection(
    ...
    ladder = build_soft_polarity_log_response_ladder_receipt()
    scores = _objectives() if objectives is None else objectives
    selection = build_soft_polarity_log_response_inner_oof_selection_receipt(
...
def test_fixed_ladder_protocol_and_candidate_receipts_are_authenticated() -> None:
    assert SOFT_POLARITY_LOG_RESPONSE_LADDER == (
        (0.0, 0.0),
        (1.0 / 8.0, 0.0),
        (1.0 / 8.0, 1.0 / 4.0),
        (1.0 / 8.0, 1.0 / 2.0),
    ...
    assert SOFT_POLARITY_LOG_RESPONSE_PAIRS == SOFT_POLARITY_LOG_RESPONSE_LADDER
    assert SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256 == (
        EXPECTED_PROTOCOL_SHA256
    )
    assert SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS == tuple(
        f"log_response_{index:02d}" for index in range(11)
    )

    ladder = build_soft_polarity_log_response_ladder_receipt()
    validate_soft_polarity_log_response_ladder_receipt(ladder)
    validate_soft_polarity_log_response_ladder_receipt(
    ...
    assert tuple(
        tuple(pair) for pair in ladder["candidate_pairs_r_lambda"]
    ) == SOFT_POLARITY_LOG_RESPONSE_LADDER
    assert ladder["candidate_count"] == 11
    ...
    for index, candidate in enumerate(ladder["candidate_receipts"]):
        ...
        assert candidate["candidate_id"] == (
            SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS[index]
        )
        assert candidate["ladder_index"] == index
        assert candidate["r"] == r
        assert candidate["lambda"] == lambda_
...
def test_q_is_the_exact_continuous_odd_log_response_formula() -> None:
    for r, lambda_ in SOFT_POLARITY_LOG_RESPONSE_LADDER:
        for z in (-2.0, -0.25, 0.0, 0.25, 2.0):
            expected = math.tanh(
                r
                * (
                    (1.0 - lambda_) * z
                    + lambda_ * math.asinh(4.0 * z) / 4.0
                )
            )
            actual = soft_polarity_log_response_q(z, r=r, lambda_=lambda_)
            assert actual == pytest.approx(expected)
            assert soft_polarity_log_response_q(
                -z, r=r, lambda_=lambda_
            ) == pytest.approx(-actual)
    assert soft_polarity_log_response_q(0.0, r=0.25, lambda_=1.0) == 0.0


@pytest.mark.parametrize(
    "kwargs,expected",
    (
        ({"z": math.nan, "r": 0.0, "lambda_": 0.0}, "must be finite"),
        ({"z": 1.0, "r": math.inf, "lambda_": 0.0}, "must be finite"),
        ({"z": True, "r": 0.0, "lambda_": 0.0}, "must be numeric"),
        ({"z": 1.0, "r": -0.1, "lambda_": 0.0}, "nonnegative"),
        ({"z": 1.0, "r": 0.1, "lambda_": -0.1}, "inside"),
        ({"z": 1.0, "r": 0.1, "lambda_": 1.1}, "inside"),
        ({"z": 1.0, "r": 0.1, "lambda_": True}, "must be numeric"),
        (
            {"z": float.fromhex("0x1.fffffffffffffp+1023"), "r": 0.1, "lambda_": 1.0},
...
def test_q_rejects_invalid_or_nontotal_inputs(
    ...
    with pytest.raises((TypeError, ValueError), match=expected):
        soft_polarity_log_response_q(**kwargs)
...
def test_inner_selection_is_family_equal_exact_kl_and_replayable() -> None:
    ...
    assert build_soft_polarity_log_response_fit_receipt is (
        build_soft_polarity_log_response_inner_oof_selection_receipt
    )
    target = SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS[9]
    assert selection["selected_candidate_id"] == target
    assert selection["selected_ladder_index"] == 9
    assert selection["selected_r"] == 1.0 / 4.0
    assert selection["selected_lambda"] == 3.0 / 4.0
...
def test_ties_prefer_smaller_lambda_then_smaller_r_then_fixed_order() -> None:
    ladder = build_soft_polarity_log_response_ladder_receipt()

    lambda_tie = {
        family: {
            candidate_id: (
                0.0 if index in (2, 6) else 1.0
            )
            for index, candidate_id in enumerate(
                SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS
            )
    ...
    selected = build_soft_polarity_log_response_fit_receipt(
        ladder_receipt=ladder,
        all_development_family_ids=FAMILIES,
        outer_held_family_id=OUTER,
        exact_objectives_by_family_and_candidate=lambda_tie,
    )
    assert selected["selected_candidate_id"] == "log_response_06"
    assert selected["selected_lambda"] == 0.0
    assert selected["selected_r"] == 0.25

    r_tie = copy.deepcopy(lambda_tie)
    for family in INNER:
        for candidate_id in SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS:
            r_tie[family][candidate_id] = 1.0
        r_tie[family]["log_response_01"] = 0.0
        r_tie[family]["log_response_06"] = 0.0
    selected = build_soft_polarity_log_response_fit_receipt(
        ladder_receipt=ladder,
        all_development_family_ids=FAMILIES,
        outer_held_family_id=OUTER,
    ...
    assert selected["selected_candidate_id"] == "log_response_01"
    assert selected["selected_lambda"] == 0.0
    ...
    assert selected["selected_candidate_id"] == "log_response_00"
...
def test_selection_rejects_incomplete_or_extra_exact_geometry(
    ...
    if missing_kind == "family":
        ...
        objectives[INNER[0]].pop(SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS[0])
...
def test_selection_rejects_nonfinite_or_nonnumeric_objectives(
    ...
    ladder = build_soft_polarity_log_response_ladder_receipt()
    objectives = _objectives()
    objectives[INNER[0]][SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS[0]] = bad_value
...
def test_ladder_rejects_rehashed_candidate_drift_and_unknown_fields() -> None:
    ...
    with pytest.raises(ValueError, match="candidate receipt key set drifted"):
        validate_soft_polarity_log_response_ladder_receipt(unknown)
...
def test_fully_rehashed_objective_forgery_cannot_bypass_authoritative_replay() -> None:
    ...
    forged_objectives[INNER[0]]["log_response_00"] = -100.0
    forged = build_soft_polarity_log_response_fit_receipt(
        ladder_receipt=ladder,
        all_development_family_ids=FAMILIES,
        outer_held_family_id=OUTER,
        exact_objectives_by_family_and_candidate=forged_objectives,
    )
    assert forged["selected_candidate_id"] == "log_response_00"
    with pytest.raises(ValueError, match="selection receipt content drifted"):
        validate_soft_polarity_log_response_fit_receipt(
            forged,
            ladder_receipt=ladder,
