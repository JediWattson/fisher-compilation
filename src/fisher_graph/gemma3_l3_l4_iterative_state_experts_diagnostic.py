"""Execute iteration three: causal sign experts over Gemma modal state."""
...
def _load_rejected_prior_iteration(
    ...
    *,
    ...
    """Load and prove that rejected iteration two is the direct parent."""

    source = Path(path)
    if _file_sha256(source) != expected_report_file_sha256:
        raise ValueError("prior state-router report file hash mismatch")
    with source.open("r", encoding="utf-8") as handle:
        value = json.load(handle)
    if not isinstance(value, dict):
        raise TypeError("prior state-router report must be a JSON object")
    validate_gemma_iterative_state_router_report(value)
    if value.get("schema") != _STATE_ROUTER_SCHEMA:
        raise ValueError("prior report is not the iteration-two state router")
    semantics = _mapping(
        value.get("semantics"),
        label="prior state-router semantics",
    )
    if semantics.get("iteration") != 2:
        raise ValueError("prior report is not the iteration-two state router")
    if value.get("report_sha256") != expected_report_sha256:
        raise ValueError("prior state-router report logical hash mismatch")
    if value.get("collection_sha256") != expected_collection_sha256:
        raise ValueError("prior state-router collection hash mismatch")
    decision = _mapping(
        value.get("decision"),
        label="prior state-router decision",
    )
    if (
        ...
        raise ValueError(
            "state-experts iteration requires the rejected state-router "
            "parent with no retained provider or deployment"
        )
    lineage = _mapping(
        value.get("lineage"),
        label="prior state-router lineage",
    )
    if set(lineage) != (
        _LIVE_PARENT_LINEAGE_KEYS | _PRIOR_ITERATION_LINEAGE_KEYS
    ):
        raise ValueError("prior state-router lineage fields differ")
    return value


def _experts_make_fit_record(
    *,
    parent_h4: object,
    lag_b_correction: object,
    **kwargs: object,
) -> object:
    # The sign experts consume the authenticated modal parent directly.
    # The generic shell still realizes lag B so all iterations retain the
    # exact same 16 + 16 two-phase execution envelope.
    del lag_b_correction
    return build_gemma_iterative_state_experts_fit_record(
        parent_h4=parent_h4,  # type: ignore[arg-type]
        **kwargs,
    )


def _experts_fit_fold(
    *,
...
def _experts_fit_full(
    *,
    parent_artifact_sha256: str,
    **kwargs: object,
) -> object:
    return fit_gemma_iterative_state_experts_full_provider(
        **kwargs,
        parent_artifact_sha256=parent_artifact_sha256,
    )


def run_gemma_iterative_state_experts_diagnostic(
    *,
    ...
    """Run the frozen state-experts recipe on expanded A-fit only."""
    ...
    diagnostic_recipe = _GemmaIterativeDiagnosticRecipe(
        campaign_recipe=GEMMA_ITERATIVE_STATE_EXPERTS_CAMPAIGN_RECIPE,
        make_fit_record=_experts_make_fit_record,
        fit_fold=_experts_fit_fold,
        build_report=build_gemma_iterative_state_experts_report,
        fit_full=_experts_fit_full,
        validate_report=validate_gemma_iterative_state_experts_report,
        report_label="iterative state-experts",
        ...
        source_code_files=(
            "gemma3_l3_l4_iterative_state_experts_diagnostic.py",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run frozen causal modal sign experts on reusable A-fit."
        )
    )
    parser.add_argument(
        "--corpus-artifact",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_gemma_iterative_state_experts_diagnostic(
