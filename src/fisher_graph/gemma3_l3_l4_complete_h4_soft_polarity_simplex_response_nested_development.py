"""V20m nested simplex-response development campaign for Gemma 3.

V20m authenticates the completed V20l report, all eight V20l fold fragments,
and their lineage before model construction.  It keeps V20i's six-family
masked Fisher directions and training-only CVaR-2 reflection fit, but
decouples signed-stack attenuation ``u`` from signed target weight ``v``:

``q(z) = (1-u*z^2)*tanh(r*z) + v*z^2``, with ``0 <= |v| <= u <= 1/4``.

The frozen nineteen-response ladder contains a zero anchor, two matched-linear
controls, V20l boundary responses, and interior simplex responses.  All
seven-by-nineteen
...
nested model cross-validation.  Eight outer mechanism arms are then frozen
...
__all__ = [
    ...
    "build_parser",
    "main",
    "run_gemma3_l3_l4_complete_h4_soft_polarity_simplex_response_nested_development",
]


_LOCAL_ROOT = Path(".local-runs/google--gemma-3-270m")
DEFAULT_OUTPUT = _LOCAL_ROOT / (
    "modal-generator-l3-l4-complete-h4-soft-polarity-simplex-response-nested-"
    "r16-k256-a-fit16-dev-v20m.json"
)
...
_SCHEMA = (
    "fisher_graph.gemma3_l3_l4.complete_h4_soft_polarity_simplex_response_nested.v20m"
)
_FOLD_SCHEMA = (
    "fisher_graph.complete_h4_soft_polarity_simplex_response_nested_outer_fold.v20m"
)
_FORMAT_VERSION = 29
_REPORT_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-nested-report:v20m\0"
_SOURCE_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-nested-source:v20m\0"
_FOLD_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-nested-fold:v20m\0"
_INNER_FIT_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-inner-fit:v20m\0"
_INNER_MANIFEST_DOMAIN = (
    b"fisher-graph:soft-polarity-simplex_response-inner-manifest:v20m\0"
)
_INNER_EXECUTION_DOMAIN = (
    b"fisher-graph:soft-polarity-simplex_response-inner-execution:v20m\0"
)
_RESPONSE_SELECTION_DOMAIN = (
    b"fisher-graph:soft-polarity-simplex_response-response-selection:v20m\0"
)
_OUTER_MANIFEST_DOMAIN = (
    b"fisher-graph:soft-polarity-simplex_response-outer-manifest:v20m\0"
)
_OUTER_EXECUTION_DOMAIN = (
    b"fisher-graph:soft-polarity-simplex_response-outer-execution:v20m\0"
)
_PROVIDER_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-provider:v20m\0"
_TRACE_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-trace:v20m\0"
_DECISION_DOMAIN = b"fisher-graph:soft-polarity-simplex_response-decision:v20m\0"
...
if _RESPONSES != tuple(_simplex_response_fit.SOFT_POLARITY_SIMPLEX_RESPONSE_LADDER):
    raise RuntimeError("V20m runner response ladder differs from core protocol")
...
_ARMS = (
    ...
    "simplex_response_reflected",
    "simplex_response_reflected_exact_mirror",
)
_PRIMARY_ARM = "simplex_response_reflected"
...
_FIXED_PROTOCOL: dict[str, object] = {
    "protocol": "v20m_nested_reflection_and_bounded_simplex_response_fit",
    "scientific_status": (
        "posthoc_after_v20l_reused_A16_development_hypothesis_only"
    ),
    "source": (
        "pinned_V20l_report_and_folds_plus_V20k_V20j_V20i_lineage_and_"
    ...
    "response_selection": (
        ...
        "full_vocabulary_KL_teacher_to_candidate_then_smaller_u_then_"
        "smaller_abs_v_then_smaller_radius_then_fixed_index_then_artifact_sha256"
    ),
    ...
    "inner_freeze_barrier": (
        "all_seven_times_nineteen_providers_and_traces_before_any_inner_capability"
    ),
    "outer_arms": _ARMS,
    "outer_freeze_barrier": "all_eight_providers_and_traces_before_outer_capability",
    "primary_gate": (
    ...
    "mechanism_gate": (
        ...
        "and_below_precommitted_matched_V20l_boundary_with_five_wins"
    ),
...
_TRANSFER_PROTOCOL_SHA256 = _v14._sha256(
    {
        ...
        "operation": "V20m_domain_separated_simplex_response_provider_materialization",
...
def _simplex_parameters(value: object) -> tuple[float, float, float]:
    raw = _sequence(value, label="V20m simplex parameters")
    if len(raw) != 3:
        raise ValueError("V20m response must contain exactly radius, u, and v")
    if any(type(item) not in (int, float) for item in raw):
        raise ValueError("V20m response values must be JSON numbers")
    ...
    if (
        ...
        raise ValueError("V20m simplex parameters are outside the certified domain")
...
def _response_tuple(value: object) -> tuple[float, float, float]:
    ...
    if selected not in _RESPONSES:
        raise ValueError("V20m response is outside the fixed ladder")
    return selected
...
def _response_order(value: object) -> tuple[tuple[float, float, float], ...]:
    return tuple(
        ...
        for item in _sequence(value, label="V20m response order")
    )
...
def _validate_v20i_reflection_lineage(
    *,
    ...
    inherited_outer = _mapping(
        ...
        label="V20m inherited V20i outer reflection fit",
    )
    if _v14._canonical_json_bytes(outer_reflection_fit) != (
        _v14._canonical_json_bytes(inherited_outer)
    ):
        raise ValueError("V20m outer reflection lineage differs from pinned V20i")

    current_inner = _mapping(
        inner_receipt.get("inner_evidence_by_family"),
        label="V20m current inner reflection evidence",
    )
    inherited_inner_receipt = _mapping(
        authenticated_v20i_fold.get("inner_receipt"),
        label="V20m inherited V20i inner receipt",
    )
    inherited_inner = _mapping(
        inherited_inner_receipt.get("inner_evidence_by_family"),
        label="V20m inherited V20i inner reflection evidence",
    )
    if set(current_inner) != set(inherited_inner):
        raise ValueError("V20m inner reflection family lineage differs from V20i")
    for family in sorted(current_inner):
        current = _mapping(
            current_inner[family], label="V20m current inner reflection family"
        )
        inherited = _mapping(
            inherited_inner[family], label="V20m inherited inner reflection family"
        )
        for field in ("masked_direction_receipt", "reflection_fit_receipt"):
            if _v14._canonical_json_bytes(current.get(field)) != (
                ...
                raise ValueError(
                    f"V20m {field} lineage differs from pinned V20i"
                )
...
def _validate_output(path: Path | str) -> Path:
    ...
    if destination in protected:
        raise ValueError("V20m output must preserve immutable prerequisite artifacts")
    if destination.parent != local_root:
        raise ValueError("V20m output must remain directly under .local-runs")
    return destination
...
def _load_prerequisites() -> tuple[
    ...
    """Authenticate V20l and its complete lineage before construction."""

    if (
        _V20L_LOGICAL_SHA256 is None
        or _V20L_FILE_SHA256 is None
        or _V20L_SOURCE_SHA256 is None
        or _V20L_FOLD_SHA256S is None
    ):
        raise RuntimeError(
            "V20m is fail-closed until the completed V20l report and all eight "
    ...
    (
        ...
        v20l_source,
    ) = _v20l._load_prerequisites()
    panel_receipt = dict(
        _mapping(
            prerequisite.get("nested_panel_receipt"),
            label="V20m inherited panel receipt",
        )
    )
    bridge_binding = _sha(
        prerequisite.get("authenticated_bridge_binding_sha256"),
        label="V20m inherited bridge binding",
    )
    if _v14._file_sha256(_V20L_OUTPUT) != _V20L_FILE_SHA256:
        raise RuntimeError("pinned V20l report file hash drifted")
    v20l_report = _v20l._load_existing_report(
        _V20L_OUTPUT,
        source=v20l_source,
    ...
    observed_fold_hashes = {
        _identifier(family, label="V20m V20l fold family"): _sha(
            value, label="V20m V20l fold hash"
        )
        for family, value in _mapping(
            v20l_report.get("fold_fragment_sha256s_by_family"),
            label="V20m V20l fold hashes",
        ).items()
    }
    if (
        v20l_report.get("report_sha256") != _V20L_LOGICAL_SHA256
        or v20l_report.get("all_eight_outer_folds_completed") is not True
        or _mapping(
            v20l_report.get("decision"), label="V20m V20l decision"
        ).get("integrity_passed")
        is not True
        or v20l_report.get("final_refit") is not None
        or v20l_report.get("calibration_b_opened") is not False
        or _mapping(
            v20l_report.get("source_receipt"), label="V20m V20l source"
        ).get("artifact_sha256")
        != _V20L_SOURCE_SHA256
        or observed_fold_hashes != _V20L_FOLD_SHA256S
    ):
        raise RuntimeError("pinned V20l development authority differs")

    families = tuple(sorted(_V20L_FOLD_SHA256S))
    authenticated_v20l_folds = {
        family: _v20l._load_fold_fragment(
            output=_V20L_OUTPUT,
            source=v20l_source,
            panel_receipt=panel_receipt,
    ...
    if {
        ...
        for family, fragment in authenticated_v20l_folds.items()
    } != _V20L_FOLD_SHA256S:
        raise RuntimeError("pinned V20l fold authority differs")
    source = _hashed(
        {
            ...
            "v20l_report_sha256": _V20L_LOGICAL_SHA256,
            "v20l_file_sha256": _V20L_FILE_SHA256,
            "v20l_source_receipt_sha256": _V20L_SOURCE_SHA256,
            "v20l_classification": v20l_report.get("classification"),
            "v20l_passed": v20l_report.get("passed"),
            "v20l_rollback_to_base": v20l_report.get("rollback_to_base"),
            "v20l_fold_fragment_sha256s_by_family": dict(
                sorted(_V20L_FOLD_SHA256S.items())
            ),
            "reflection_fit_protocol_sha256": (
                _reflection.SOFT_POLARITY_REFLECTION_FIT_PROTOCOL_SHA256
            ),
...
def _selected_direction(
    ...
    if reflection_fit.get("selected_variant_available") is not True:
        raise RuntimeError("V20m reflection fit has no admissible direction")
    raw = tuple(
        float(item)
        for item in _sequence(
            reflection_fit.get("selected_normalized_direction"),
            label="V20m selected reflection direction",
        )
    )
    if len(raw) != 4 or not all(math.isfinite(item) for item in raw):
        raise RuntimeError("V20m reflection direction is not a finite four-vector")
...
def _unreflected_direction(
    ...
    raw = tuple(
        ...
        for item in _sequence(
            ...
            label="V20m unreflected direction",
        )
    )
    if len(raw) != 4 or not all(math.isfinite(item) for item in raw):
        raise RuntimeError("V20m unreflected direction is not finite")
    return raw  # type: ignore[return-value]
    ...
    return _v14._sha256(
        {
            ...
            "endpoint_receipt_sha256": _sha(
                endpoint_receipt_sha256, label="V20m provider endpoint"
            ),
            "direction_artifact_sha256": _sha(
                direction_artifact_sha256, label="V20m provider direction"
            ),
            "reflection_fit_sha256": _sha(
                reflection_fit_sha256, label="V20m provider reflection fit"
            ),
            "response": (radius, u, v),
            ...
            "outer_held_family_id": _identifier(
                outer_family_id, label="V20m provider outer family"
            ),
...
def _provider_receipt(
    ...
    *,
    ...
    metadata = _mapping(provider.metadata(), label=f"V20m {role} metadata")
    ...
    if isinstance(
        ...
        if response is None or direction is None:
            raise ValueError(
                "V20m simplex_response provider receipt needs response and direction"
            )
        radius, u, v = _simplex_parameters(response)
        expected = tuple(float(item) for item in direction)
        if selected_direction != expected:
            raise RuntimeError("V20m provider differs from its frozen direction")
        if (
            float(provider.radius) != radius
            or float(provider.shrink_mass) != u
            or float(provider.polarity_bias) != v
        ):
            raise RuntimeError("V20m provider coefficients differ from response")
        bound = max(abs(item) for item in corners)
        if abs(bound - 1.0) > 1.0e-12:
            raise RuntimeError("V20m provider direction is not box normalized")
    payload = {
        "role": role,
        "response": response,
        "response_key": (
        ...
        "provider_artifact_sha256": _sha(
            provider.artifact_sha256, label=f"V20m {role} provider artifact"
        ),
...
def _expected_provider_accounting_from_v20i(
    ...
    manifest = _mapping(
        ...
        label="V20m inherited V20i provider manifest",
    )
    receipts = _mapping(
        manifest.get("provider_receipts"),
        label="V20m inherited V20i provider receipts",
    )
    reference_role = {
        "base": "base",
    ...
    authority = _mapping(
        ...
        label=f"V20m inherited V20i {reference_role} provider receipt",
    )
...
def _validate_provider_receipt_evidence(
    ...
    *,
    ...
    if set(receipt) != _PROVIDER_RECEIPT_KEYS:
        raise ValueError("V20m provider receipt key set differs")
    provider_artifact = _sha(
        expected_provider_artifact_sha256,
        label="V20m expected provider artifact",
    )
    if (
        ...
        raise ValueError("V20m provider receipt identity differs")

    metadata = _mapping(
        receipt.get("provider_metadata"), label="V20m provider metadata"
    )
    metadata_sha = _sha(
        receipt.get("provider_metadata_sha256"),
        label="V20m provider metadata hash",
    )
    if (
        _v14._sha256(metadata, domain=_PROVIDER_DOMAIN) != metadata_sha
        or metadata.get("artifact_sha256") != provider_artifact
    ):
        raise ValueError("V20m provider metadata authentication differs")

    receipt_accounting = tuple(
        _strict_receipt_integer(receipt, key, label="V20m provider receipt")
        for key in (
            "rank",
            "conditional_rank",
            "prepared_float_scalar_count",
    ...
    metadata_accounting = tuple(
        _strict_receipt_integer(metadata, key, label="V20m provider metadata")
        for key in (
            "rank",
            "conditional_rank",
            "prepared_float_scalar_count",
    ...
    if (
        ...
        raise ValueError("V20m provider accounting differs from pinned V20i")

    if metadata.get("bridge_binding_sha256") not in (
        None,
        expected_bridge_binding_sha256,
    ):
        raise ValueError("V20m provider bridge metadata differs")
    ...
    if simplex_response_role:
        if expected_response is None or expected_direction is None:
            raise ValueError("V20m simplex_response provider expectation is incomplete")
        ...
        if len(direction) != 4 or not all(math.isfinite(item) for item in direction):
            raise ValueError("V20m expected simplex_response direction differs")
        payload = _mapping(
            receipt.get("provider_payload"), label="V20m simplex_response provider payload"
        )
        ...
        if _v14._canonical_json_bytes(
            ...
            raise ValueError("V20m simplex_response provider box certificate differs")
        ...
        for key, expected in expected_bindings.items():
            if validated.payload.get(key) != expected:
                raise ValueError(f"V20m simplex_response provider {key} differs")
        for key in (
        ...
        if validated.artifact_sha256 != provider_artifact:
            raise ValueError("V20m simplex_response provider artifact replay differs")
    else:
        if receipt.get("provider_payload") is not None:
            raise ValueError("V20m non-simplex_response provider serialized a payload")
        if expected_role in ("fixed_plus", "fixed_minus"):
            expected_transfer = _sha(
                expected_transfer_evidence_sha256,
                label="V20m fixed-control transfer evidence",
            )
            expected_bindings = {
                "base_provider_artifact_sha256": _sha(
                    ...
                    label="V20m fixed-control endpoint base",
                ),
                "proposal_provider_artifact_sha256": _sha(
                    expected_endpoint_receipt.get(
                        "proposal_provider_artifact_sha256"
                    ),
                    label="V20m fixed-control endpoint proposal",
                ),
                "transfer_protocol_sha256": _TRANSFER_PROTOCOL_SHA256,
                "transfer_evidence_sha256": expected_transfer,
            }
            for key, expected in expected_bindings.items():
                if metadata.get(key) != expected:
                    raise ValueError(f"V20m fixed-control {key} differs")
...
def _freeze_inner_providers(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20m inner outer family")
    ...
    if (
        ...
        raise RuntimeError("V20m inner family geometry differs")
    ...
    for inner in training_families:
        ...
        selected_artifact = _sha(
            ...
            label="V20m inner selected reflection variant",
        )
        held_records = tuple(
            record for record in ordered if record.sequence.family_id == inner
        )
        if len(held_records) != _PROMPTS_PER_FAMILY:
            raise RuntimeError("V20m inner-held prompt geometry differs")
    ...
    if len(flat_hashes) != _INNER_FAMILY_COUNT * len(_RESPONSES) or len(
        ...
        raise RuntimeError("V20m inner provider artifacts are not all distinct")
    manifest = _hashed(
        {
...
def _aggregate_response_selection(
    ...
    if len(families) != _INNER_FAMILY_COUNT:
        raise ValueError("V20m response selection requires seven inner OOF families")
    outer_ids = {
        _identifier(
            inner_evidence_by_family[family].get("outer_held_family_id"),
            label="V20m response selection outer family",
        )
        for family in families
    }
    if len(outer_ids) != 1 or next(iter(outer_ids)) in families:
        raise ValueError("V20m response selection outer family geometry differs")
    ...
    for family in families:
        raw = _mapping(
            ...
            label="V20m inner objective ladder",
        )
        if set(raw) != set(_RESPONSE_KEYS):
            raise ValueError("V20m inner objective response geometry differs")
...
def _fit_inner_response(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20m inner-response outer family")
    ...
    for inner in tuple(manifest["inner_family_order"]):
        ...
        for response in _RESPONSES:
            ...
            if set(family_scores) != {inner}:
                raise RuntimeError("V20m inner score family geometry differs")
        ...
        if not zero_anchor:
            raise RuntimeError("V20m inner eta-zero output anchor differs from V20g")
...
def _matched_v20l_boundary_response(
    ...
    fold = _mapping(
        ...
        label="V20m authenticated V20l fold receipt",
    )
    outer = _identifier(outer_family_id, label="V20m V20l boundary family")
    if fold.get("outer_held_family_id") != outer:
        raise ValueError("V20m V20l boundary family differs")
...
def _freeze_outer_providers(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20m outer provider family")
    ...
    reflected_provider, reflected_seed = _materialize_provider(
        ...
        role="outer_simplex_response_reflected",
    )
    ...
    providers: dict[str, object] = {
        ...
        "simplex_response_reflected": reflected_provider,
    ...
    if tuple(providers) != _ARMS or len(
        ...
        raise RuntimeError("V20m outer provider arm artifacts are not distinct")
    ...
    receipts = {
        ...
        "same_simplex_response_unreflected": _provider_receipt(
            providers["same_simplex_response_unreflected"],
            role="same_simplex_response_unreflected",
            response=response,
            direction=unreflected,
        ),
        "simplex_response_reflected": _provider_receipt(
            providers["simplex_response_reflected"],
            role="simplex_response_reflected",
            response=response,
            direction=reflected,
        ),
    ...
    manifest = _hashed(
        {
            ...
            "soft_provider_transfer_evidence_sha256s": {
                ...
                "simplex_response_reflected": reflected_seed,
                "simplex_response_reflected_exact_mirror": mirror_seed,
            },
            "fixed_control_transfer_evidence_sha256": control_seed,
            "all_eight_providers_frozen_before_outer_capability": True,
            "all_eight_traces_frozen_before_outer_capability": True,
...
def _score_outer_arms(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20m held outer family")
    held = _v20b._ordered_records(
        tuple(record for record in records if record.sequence.family_id == outer)
    )
    if len(held) != _PROMPTS_PER_FAMILY:
        raise RuntimeError("V20m outer-held prompt geometry differs")
    ...
    if (
        manifest.get("all_eight_providers_frozen_before_outer_capability") is not True
        or manifest.get("all_eight_traces_frozen_before_outer_capability") is not True
        or manifest.get("outer_capability_count_at_freeze") != 0
        or manifest.get("outer_objectives_or_teacher_rows_used_at_freeze")
        is not False
    ):
        raise PermissionError("V20m outer freeze barrier is not satisfied")
    ...
    for arm in _ARMS:
        ...
        if set(family_scores) != {outer}:
            raise RuntimeError("V20m outer score family geometry differs")
    ...
    _v20b._validate_capability_receipt(
        ...
        label="V20m outer-held capability",
    )
    inherited_arms = _mapping(
        _mapping(
            authenticated_v20g_fold.get("held_evidence"),
            label="V20m inherited V20g held evidence",
        ).get("arm_evidence"),
        label="V20m inherited V20g held arms",
    )
    control_anchors: dict[str, bool] = {}
    for arm in ("base", "fixed_plus", "fixed_minus"):
        inherited = _mapping(
            inherited_arms.get(arm), label=f"V20m inherited V20g {arm} arm"
        )
        ...
        control_anchors[arm] = (
            ...
            and dict(
                _mapping(
                    ...
                    label=f"V20m current {arm} objectives",
                )
            )
            == dict(
                _mapping(
                    inherited.get("objectives_by_example"),
                    label=f"V20m inherited {arm} objectives",
                )
            )
            and dict(
                _mapping(
                    current.get("post_cast_h4_sha256s"),
                    label=f"V20m current {arm} H4 hashes",
                )
            )
            == dict(
                _mapping(
                    inherited.get("post_cast_h4_sha256s"),
                    label=f"V20m inherited {arm} H4 hashes",
                )
            )
            and dict(
                _mapping(
                    current.get("supervised_full_vocab_logits_sha256s"),
                    label=f"V20m current {arm} logits hashes",
                )
            )
            == dict(
                _mapping(
                    inherited.get("supervised_full_vocab_logits_sha256s"),
                    label=f"V20m inherited {arm} logits hashes",
                )
            )
        )
    if not all(control_anchors.values()):
        raise RuntimeError("V20m outer control output anchor differs from V20g")
    inherited_v20l_arm = _mapping(
        _mapping(
            _mapping(
                authenticated_v20l_fold.get("held_evidence"),
                label="V20m inherited V20l held evidence",
            ).get("arm_evidence"),
            label="V20m inherited V20l held arms",
        ).get("signed_stack_reflected"),
        label="V20m inherited V20l selected boundary arm",
    )
    ...
    boundary_logits = _mapping(
        ...
        label="V20m matched V20l boundary output hashes",
    )
    ...
    held_evidence = _hashed(
        {
            ...
            "capability_receipt": capability_receipt,
            "all_eight_providers_and_traces_frozen_before_outer_capability": True,
...
def _execute_outer_fold(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20m outer family")
    ...
    inherited_endpoint = _mapping(
        ...
        label="V20m inherited endpoint receipt",
    )
    inherited_evidence = _mapping(
        authenticated_v20g_fold.get("endpoint_evidence"),
        label="V20m inherited endpoint evidence",
    )
    if (
        ...
        raise RuntimeError("V20m reconstructed endpoint differs from pinned V20g")
    fit = _mapping(
        authenticated_v20g_fold.get("fit_receipt"),
        label="V20m inherited V20g fit receipt",
    )
    source_direction = _mapping(
        fit.get("direction_receipt"), label="V20m inherited V20g direction"
    )
    _v20g._core.validate_soft_polarity_direction_receipt(source_direction)
    if source_direction.get("held_family_id") != outer:
        raise RuntimeError("V20m inherited direction held family differs")
...
def _validate_inner_receipt(
    ...
    *,
    ...
    receipt = _validate_hashed(
        value, domain=_INNER_FIT_DOMAIN, label="V20m inner receipt"
    )
    ...
    if (
        ...
        raise ValueError("V20m inner receipt boundary differs")
    manifest = _validate_hashed(
        _mapping(
            receipt.get("inner_provider_manifest"),
            label="V20m inner provider manifest",
        ),
        domain=_INNER_MANIFEST_DOMAIN,
        label="V20m inner provider manifest",
    )
    families = tuple(
        _identifier(item, label="V20m inner family")
        for item in _sequence(
            manifest.get("inner_family_order"), label="V20m inner family order"
        )
    )
    source_families = tuple(
        _identifier(item, label="V20m source training family")
        for item in _sequence(
            source_direction.get("training_family_ids"),
            label="V20m source training families",
        )
    )
    training_ids_by_family = _mapping(
        source_direction.get("training_example_ids_by_family"),
        label="V20m source training example ids",
    )
    if (
    ...
    if any(
        ...
        raise ValueError("V20m inner manifest family bindings differ")
    gradient_evidence = _mapping(
        _mapping(
            authenticated_v20g_fold.get("fit_training_evidence"),
            label="V20m inherited fit evidence",
    ...
    if set(inherited_zero_objectives) != family_set:
        raise ValueError("V20m inherited eta-zero family geometry differs")
    raw_inner = _mapping(
        receipt.get("inner_evidence_by_family"),
        label="V20m inner evidence map",
    )
    if set(raw_inner) != set(families):
        raise ValueError("V20m inner evidence family geometry differs")
    validated_inner: dict[str, Mapping[str, object]] = {}
    for family in families:
        evidence = _validate_hashed(
            _mapping(raw_inner[family], label="V20m inner family evidence"),
            domain=_INNER_EXECUTION_DOMAIN,
            label="V20m inner family evidence",
        )
        masked = _mapping(
            evidence.get("masked_direction_receipt"),
            label="V20m masked direction receipt",
        )
        ...
        reflection_fit = _mapping(
            ...
            label="V20m inner reflection fit",
        )
        ...
        expected_examples = tuple(
            _identifier(item, label="V20m inner expected example")
            for item in _sequence(
                training_ids_by_family.get(family),
                label="V20m inner expected examples",
            )
        )
        objectives = _mapping(
            evidence.get("objective_by_response"),
            label="V20m inner objectives",
        )
        response_evidence = _mapping(
            evidence.get("response_evidence"),
            label="V20m inner response evidence",
        )
        if (
            ...
            raise ValueError("V20m inner evidence schedule differs")
        family_provider_hashes = _mapping(
            provider_hashes[family], label="V20m inner family provider hashes"
        )
        family_provider_receipts = _mapping(
            provider_receipts[family], label="V20m inner family provider receipts"
        )
        family_transfer_hashes = _mapping(
            transfer_hashes[family],
            label="V20m inner family provider transfer hashes",
        )
        family_trace_hashes = _mapping(
            trace_hashes[family], label="V20m inner family trace hashes"
        )
        if any(
            ...
            raise ValueError("V20m inner manifest response bindings differ")
        trace_bundle_sha = _v14._sha256(
            {
        ...
        for response in _RESPONSES:
            ...
            arm = _validate_hashed(
                _mapping(
                    response_evidence[key], label="V20m inner response arm evidence"
                ),
                domain=_INNER_EXECUTION_DOMAIN,
                label="V20m inner response arm evidence",
            )
            trace = _validate_hashed(
                _mapping(arm.get("response_trace"), label="V20m inner trace"),
                domain=_TRACE_DOMAIN,
                label="V20m inner trace",
            )
            provider_receipt = _validate_hashed(
                _mapping(
                    family_provider_receipts[key],
                    label="V20m inner provider receipt",
                ),
                domain=_PROVIDER_DOMAIN,
                label="V20m inner provider receipt",
            )
            score_bundle = _validate_exact_score_bundle(
                ...
                label=f"V20m inner {family} response {key}",
            )
            arm_objectives, arm_h4, arm_logits, arm_executions = score_bundle
            provider_artifact = _sha(
                family_provider_hashes[key], label="V20m inner provider hash"
            )
            transfer_artifact = _sha(
                family_transfer_hashes[key],
                label="V20m inner provider transfer hash",
            )
            ...
            trace_artifact = _sha(
                family_trace_hashes[key], label="V20m inner trace hash"
            )
            seed = _v14._sha256(
                {
                    "inner_manifest_sha256": manifest["artifact_sha256"],
            ...
            response_gain_hashes = _mapping(
                ...
                label="V20m inner response gain hashes",
            )
            for value in response_gain_hashes.values():
                _sha(value, label="V20m inner response gain hash")
            ...
            if (
                ...
                or _v14._canonical_json_bytes(
                    _mapping(
                        ...
                        label="V20m simplex_response box certificate",
                    )
                )
                ...
                raise ValueError("V20m inner response evidence differs")
        _v20b._validate_capability_receipt(
            ...
            expected_accesses_per_example=len(_RESPONSES),
            label="V20m resumed inner-held capability",
        )
        zero = _mapping(
            response_evidence[_response_key((0.0, 0.0, 0.0))],
            label="V20m inner zero-response evidence",
        )
        expected_zero_objectives = {
            _identifier(example, label="V20m inherited zero objective example"): float(
                objective
            )
            for example, objective in _mapping(
                inherited_zero_objectives[family],
                label="V20m inherited family zero objectives",
            ).items()
        }
        zero_anchor = (
            dict(
        ...
        if (
            ...
            raise ValueError("V20m inner zero-response V20g output anchor differs")
    ...
    persisted_selection = _mapping(
        ...
        label="V20m persisted response selection",
    )
    if _v14._canonical_json_bytes(selection) != _v14._canonical_json_bytes(
        persisted_selection
    ):
        raise ValueError("V20m inner response selection replay differs")
    return receipt
...
def _validate_fold_fragment(
    ...
    *,
    ...
    fragment = _mapping(value, label="V20m fold fragment")
    if set(fragment) != _FOLD_FRAGMENT_KEYS:
        raise ValueError("V20m fold fragment key set differs")
    outer = _identifier(outer_family_id, label="V20m fold outer family")
    expected_header = {
        "schema": _FOLD_SCHEMA,
        "format_version": _FORMAT_VERSION,
        "target_output": _validate_output(output).as_posix(),
    ...
    if any(fragment.get(key) != expected for key, expected in expected_header.items()):
        raise ValueError("V20m fold fragment header differs")
    if (
        ...
        raise ValueError("V20m fold scalar-only boundary differs")
    for key in ("endpoint_receipt", "endpoint_evidence"):
        if _v14._canonical_json_bytes(fragment.get(key)) != _v14._canonical_json_bytes(
            ...
            raise ValueError("V20m fold endpoint lineage differs")

    fit = _mapping(
        authenticated_v20g_fold.get("fit_receipt"),
        label="V20m validation V20g fit",
    )
    source_direction = _mapping(
        fit.get("direction_receipt"), label="V20m validation source direction"
    )
    inner = _validate_inner_receipt(
        _mapping(fragment.get("inner_receipt"), label="V20m inner receipt"),
    ...
    if _v14._canonical_json_bytes(response_selection) != _v14._canonical_json_bytes(
        ...
        raise ValueError("V20m duplicated response selection differs")
    ...
    manifest = _validate_hashed(
        _mapping(
            fragment.get("provider_manifest"), label="V20m outer manifest"
        ),
        domain=_OUTER_MANIFEST_DOMAIN,
        label="V20m outer manifest",
    )
    endpoint_receipt = _mapping(
        authenticated_v20g_fold.get("endpoint_receipt"),
        label="V20m outer inherited endpoint receipt",
    )
    if (
        ...
        != reflection_fit.get("selected_variant_artifact_sha256")
        or manifest.get("all_eight_providers_frozen_before_outer_capability")
        is not True
        or manifest.get("all_eight_traces_frozen_before_outer_capability") is not True
        or manifest.get("outer_capability_count_at_freeze") != 0
        or manifest.get("outer_objectives_or_teacher_rows_used_at_freeze")
        ...
        raise ValueError("V20m outer provider manifest differs")
    provider_hashes = _mapping(
        manifest.get("provider_artifact_sha256s"),
        label="V20m outer provider hashes",
    )
    provider_receipts = _mapping(
        manifest.get("provider_receipts"),
        label="V20m outer provider receipts",
    )
    trace_hashes = _mapping(
        manifest.get("response_trace_sha256s"),
        label="V20m outer trace hashes",
    )
    soft_transfer_hashes = _mapping(
        manifest.get("soft_provider_transfer_evidence_sha256s"),
        label="V20m outer soft provider transfer hashes",
    )
    ...
    if not all(arm in inherited_arms for arm in ("base", "fixed_plus", "fixed_minus")):
        raise ValueError("V20m inherited V20g control arms differ")
    expected_examples = tuple(
        sorted(
            _identifier(example, label="V20m outer expected example")
            for example in _mapping(
                _mapping(
                    inherited_arms["base"],
                    label="V20m inherited V20g base arm",
                ).get("objectives_by_example"),
                label="V20m inherited V20g base objectives",
            )
        )
    )
    held = _validate_hashed(
        _mapping(fragment.get("held_evidence"), label="V20m held evidence"),
        domain=_OUTER_EXECUTION_DOMAIN,
        label="V20m held evidence",
    )
    arms = _mapping(held.get("arm_evidence"), label="V20m held arm evidence")
    if (
        set(arms) != set(_ARMS)
        or held.get("outer_held_family_id") != outer
        or held.get("outer_manifest_sha256") != manifest.get("artifact_sha256")
        or held.get("all_eight_providers_and_traces_frozen_before_outer_capability")
        is not True
        ...
        raise ValueError("V20m outer held schedule differs")
    objectives: dict[str, float] = {}
    exact_outputs: dict[
    ...
    expected_soft_transfers = {
        ...
        "same_simplex_response_unreflected": _provider_seed(
            endpoint_receipt_sha256=str(manifest["endpoint_receipt_sha256"]),
        ...
        "simplex_response_reflected": _provider_seed(
            endpoint_receipt_sha256=str(manifest["endpoint_receipt_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
    ...
    expected_soft_directions = {
        ...
        "simplex_response_reflected": selected_direction,
    ...
    expected_soft_responses = {
        ...
        "simplex_response_reflected": selected_response,
    ...
    if (
        {
            arm: _sha(
                ...
                label=f"V20m {arm} soft transfer hash",
            )
        ...
        raise ValueError("V20m outer provider transfer lineage differs")
    health_by_arm: dict[str, bool] = {}
    for arm in _ARMS:
        evidence = _validate_hashed(
            _mapping(arms[arm], label=f"V20m {arm} arm evidence"),
            domain=_OUTER_EXECUTION_DOMAIN,
            label=f"V20m {arm} arm evidence",
        )
        trace = _validate_hashed(
            _mapping(evidence.get("response_trace"), label=f"V20m {arm} trace"),
            domain=_TRACE_DOMAIN,
            label=f"V20m {arm} trace",
        )
        provider_receipt = _validate_hashed(
            _mapping(
                provider_receipts[arm],
                label=f"V20m {arm} provider receipt",
            ),
            domain=_PROVIDER_DOMAIN,
            label=f"V20m {arm} provider receipt",
        )
        score_bundle = _validate_exact_score_bundle(
            evidence,
            expected_example_ids=expected_examples,
            label=f"V20m outer {arm}",
        )
        arm_objectives, arm_h4, arm_logits, arm_executions = score_bundle
        provider_artifact = _sha(
            provider_hashes[arm], label=f"V20m {arm} provider hash"
        )
        _validate_provider_receipt_evidence(
            provider_receipt,
            expected_role=arm,
        ...
        trace_artifact = _sha(
            trace_hashes[arm], label=f"V20m {arm} trace hash"
        )
        seed = _v14._sha256(
            {
                "outer_manifest_sha256": manifest["artifact_sha256"],
        ...
        response_gain_hashes = _mapping(
            trace.get("response_gain_sha256s"),
            label=f"V20m {arm} response gain hashes",
        )
        for value in response_gain_hashes.values():
            _sha(value, label=f"V20m {arm} response gain hash")
        ...
        if (
            ...
            or (
                ...
                and (
                    ...
                    or _v14._canonical_json_bytes(
                        _mapping(
                            provider_receipt.get("box_certificate"),
                            label=f"V20m {arm} simplex_response certificate",
                        )
                    )
            ...
            raise ValueError("V20m outer arm health differs")
    ...
    _v20b._validate_capability_receipt(
        ...
        expected_held_family_id=None,
        expected_accesses_per_example=len(_ARMS),
        label="V20m resumed outer-held capability",
    )
    control_anchors: dict[str, bool] = {}
    for arm in ("base", "fixed_plus", "fixed_minus"):
        inherited = _mapping(
            inherited_arms[arm], label=f"V20m inherited V20g {arm} arm"
        )
        ...
        control_anchors[arm] = bool(
            ...
            == {
                _identifier(example, label=f"V20m inherited {arm} example"): float(
                    objective
                )
                for example, objective in _mapping(
                    inherited.get("objectives_by_example"),
    ...
    persisted_control_anchors = _mapping(
        ...
        label="V20m persisted control anchors",
    )
    if (
        ...
        raise ValueError("V20m outer V20g control output anchor differs")
    inherited_v20l_arm = _mapping(
        _mapping(
            _mapping(
                authenticated_v20l_fold.get("held_evidence"),
                label="V20m resumed V20l held evidence",
            ).get("arm_evidence"),
            label="V20m resumed V20l held arms",
        ).get("signed_stack_reflected"),
        label="V20m resumed V20l selected boundary arm",
    )
    ...
    v20l_boundary_anchor = bool(
        ...
        == {
            _identifier(example, label="V20m inherited V20l boundary example"): float(
                objective
            )
            for example, objective in _mapping(
                inherited_v20l_arm.get("objectives_by_example"),
    ...
    if (
        ...
        raise ValueError("V20m exact V20l boundary output anchor differs")
    ...
    fold = _validate_hashed(
        _mapping(fragment.get("fold_receipt"), label="V20m fold receipt"),
        domain=_DECISION_DOMAIN,
        label="V20m fold receipt",
    )
    if (
        ...
        raise ValueError("V20m fold decision receipt differs")
...
def _publish_fold_fragment(
    ...
    _v20b._publish_scalar_fragment(
        ...
        label="V20m simplex_response fold fragment",
    )
...
def _fold_receipt_map(
    ...
    return {
        family: _mapping(
            fragments[family].get("fold_receipt"), label="V20m aggregate fold"
        )
...
def _aggregate_decision(
    ...
    if len(families) != _FAMILY_COUNT:
        raise ValueError("V20m decision requires all eight outer folds")
    ...
    for family in families:
        ...
        if tuple(fold.get("arm_order", ())) != _ARMS:
            raise ValueError("V20m aggregate arm order differs")
        raw_scores = _mapping(
            fold.get("held_objective_by_arm"), label="V20m aggregate arm scores"
        )
        if set(raw_scores) != set(_ARMS):
            raise ValueError("V20m aggregate arm geometry differs")
        scores[family] = {arm: float(raw_scores[arm]) for arm in _ARMS}
        if not all(math.isfinite(value) for value in scores[family].values()):
            raise ValueError("V20m aggregate score became nonfinite")
...
def _runner_work_accounting() -> dict[str, object]:
    """Return the fixed canonical V20m one-shot schedule.

    Authentication and resume attempts are deliberately excluded.  V20m
    ...
    if (
        ...
        or inner_forwards != 2128
        or outer_forwards != 128
        or teacher_accesses != 2368
    ):
        raise RuntimeError("V20m canonical work schedule drifted")
    return {
    ...
    *,
    ...
    aggregate = (
        ...
        if decision is None
            ...
            decision, domain=_DECISION_DOMAIN, label="V20m aggregate decision"
        )
    )
    families = tuple(
        _identifier(item, label="V20m report family")
        for item in _sequence(
            aggregate.get("family_ids"), label="V20m report families"
        )
    )
    if (
        ...
        raise RuntimeError("V20m report requires all eight authenticated folds")
    replayed = _aggregate_decision(folds)
    if _v14._canonical_json_bytes(replayed) != _v14._canonical_json_bytes(
        aggregate
    ):
        raise ValueError("V20m supplied decision differs from fold replay")
    ...
    report = {
        ...
        "classification": (
            "soft_polarity_simplex_response_nested_oof_passed_fresh_shadow_eligible"
            if passed
            else "soft_polarity_simplex_response_nested_oof_failed_rollback_to_base"
        ),
        "passed": passed,
        "development_oof_passed": passed,
        "primary_development_gate_passed": (
        ...
        "integrity": {
            "V20g_V20i_V20j_V20k_V20l_reports_and_all_fragments_authenticated_before_model_"
            "construction": True,
            ...
            "all_133_inner_providers_and_traces_frozen_before_each_outer_fold_"
            "inner_scoring": True,
            "all_eight_outer_providers_and_traces_frozen_before_each_outer_"
            "capability": True,
...
def _load_existing_report(
    ...
    *,
    ...
    value = _v20b._load_scalar_fragment(
        ...
        label="V20m simplex_response nested report",
    )
    ...
    if (
        ...
        raise ValueError("V20m report authority family geometry differs")
    folds = {
    ...
    if (
        ...
        raise ValueError("V20m report reconstruction differs")
    return dict(value)


def run_gemma3_l3_l4_complete_h4_soft_polarity_simplex_response_nested_development(
    *,
    output: Path | str = DEFAULT_OUTPUT,
    cache_dir: Path | str | None = None,
) -> dict[str, object]:
    """Run or resume the V20m nested reflection/response development screen."""
    ...
    panel_receipt = dict(
        _mapping(
            prerequisite.get("nested_panel_receipt"), label="V20m panel receipt"
        )
    )
    bridge_binding = _sha(
        prerequisite.get("authenticated_bridge_binding_sha256"),
        label="V20m bridge binding",
    )
    ...
    if (
        ...
        or set(
            _mapping(
                ...
                label="V20m panel families",
            )
        )
        != set(family_ids)
    ):
        raise RuntimeError("V20m authenticated family geometry differs")
    ...
    try:
        ...
        if context.bridge.bridge_binding_sha256 != bridge_binding:
            raise RuntimeError("V20m live bridge differs from authenticated authority")
        ...
        if tuple(live_families) != family_ids:
            raise RuntimeError("V20m live family order differs from authenticated A16")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the V20m nested soft-polarity simplex-response development "
            "screen"
        )
    )
