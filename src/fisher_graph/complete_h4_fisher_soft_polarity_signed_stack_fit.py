"""Pure V20l conditional-inner-LOFO signed-stack selection protocol.

The scalar response is

``q(z) = (1-abs(s)*z^2)*tanh(r*z) + s*z^2``.

This module freezes one zero control and fourteen ``(r, s)`` candidates before
...
SOFT_POLARITY_SIGNED_STACK_PAIRS = SOFT_POLARITY_SIGNED_STACK_LADDER
SOFT_POLARITY_SIGNED_STACK_CANDIDATE_IDS = tuple(
    f"signed_stack_{index:02d}"
    for index in range(len(SOFT_POLARITY_SIGNED_STACK_LADDER))
)

_SHA = re.compile(r"^[0-9a-f]{64}$")
_DEVELOPMENT_FAMILY_COUNT = 8
_INNER_FAMILY_COUNT = 7
_FORMULA = "q(z)=(1-abs(s)*z^2)*tanh(r*z)+s*z^2"
_OBJECTIVE_KIND = (
...
_SELECTION_KEY = (
    "family_equal_token_mean_exact_float64_full_vocabulary_"
    "kl_teacher_to_candidate_then_smaller_abs_s_then_smaller_r_then_"
    "fixed_ladder_index_then_candidate_artifact_sha256"
)

_PROTOCOL_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:protocol:v20l\0"
)
_CANDIDATE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:candidate:v20l\0"
)
_LADDER_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:ladder:v20l\0"
)
_OBJECTIVE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:objective:v20l\0"
)
_FAMILY_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:family:v20l\0"
)
_AGGREGATE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:aggregate:v20l\0"
)
_SELECTION_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-signed-stack-fit:selection:v20l\0"
)

_DATA_BOUNDARY = {
    "role": "v20l_development_conditional_inner_lofo_signed_stack_selection_only",
...
_PROTOCOL = {
    "protocol": "v20l_conditional_inner_lofo_soft_polarity_signed_stack_selection",
    "scientific_status": "development_only_after_v20k",
    "formula": _FORMULA,
    "candidate_order": SOFT_POLARITY_SIGNED_STACK_CANDIDATE_IDS,
    "candidate_pairs_r_s": SOFT_POLARITY_SIGNED_STACK_LADDER,
    "candidate_count": len(SOFT_POLARITY_SIGNED_STACK_LADDER),
...
SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256 = _hash(
...
def soft_polarity_signed_stack_q(
    z: float, *, r: float, s: float
) -> float:
    """Evaluate the finite scalar V20l signed-stack map."""

    selected_z = _number(z, "signed-stack input z")
    selected_r = _number(r, "signed-stack coefficient r")
    selected_s = _number(s, "signed-stack coefficient s")
    if selected_r < 0.0:
        raise ValueError("signed-stack coefficient r must be nonnegative")
    if not -0.5 <= selected_s <= 0.5:
        raise ValueError("signed-stack coefficient s must be inside [-1/2,1/2]")
    ...
    if not math.isfinite(z_squared) or not math.isfinite(argument):
        raise ValueError("signed-stack calibrator intermediate must be finite")
    try:
        result = math.fsum(
            (
                (1.0 - abs(selected_s) * z_squared) * math.tanh(argument),
                selected_s * z_squared,
            )
        )
    except OverflowError as error:
        raise ValueError(
            "signed-stack calibrator intermediate must be finite"
        ) from error
    if not math.isfinite(result):
        raise ValueError("signed-stack calibrator output must be finite")
    return 0.0 if result == 0.0 else result


def _candidate_receipt(index: int) -> dict[str, object]:
    candidate_id = SOFT_POLARITY_SIGNED_STACK_CANDIDATE_IDS[index]
    r, s = SOFT_POLARITY_SIGNED_STACK_LADDER[index]
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_signed_stack_candidate.v20l",
        _CANDIDATE_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256,
            ...
            "formula": _FORMULA,
            "zero_maps_to_zero": (
                soft_polarity_signed_stack_q(0.0, r=r, s=s) == 0.0
            ),
...
def _expected_ladder() -> dict[str, object]:
    ...
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_signed_stack_ladder.v20l",
        _LADDER_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256,
            "formula": _FORMULA,
...
def build_soft_polarity_signed_stack_ladder_receipt() -> dict[str, object]:
    """Return the authenticated, immutable fifteen-candidate V20l ladder."""

    result = _expected_ladder()
    validate_soft_polarity_signed_stack_ladder_receipt(result)
    return result


def validate_soft_polarity_signed_stack_ladder_receipt(
    value: Mapping[str, object],
) -> None:
    """Authenticate every candidate, its order, fields, and ladder hash."""

    receipt = _mapping(value, "signed-stack ladder receipt")
    expected = _expected_ladder()
    if set(receipt) != set(expected):
        raise ValueError("signed-stack ladder receipt key set drifted")
    raw_candidates = _sequence(
        receipt.get("candidate_receipts"), "signed-stack ladder candidates"
    )
    if len(raw_candidates) != len(SOFT_POLARITY_SIGNED_STACK_LADDER):
        raise ValueError("signed-stack ladder candidate geometry drifted")
    for index, raw in enumerate(raw_candidates):
        candidate = _mapping(raw, "signed-stack candidate receipt")
        expected_candidate = _candidate_receipt(index)
        if set(candidate) != set(expected_candidate):
            raise ValueError("signed-stack candidate receipt key set drifted")
        _sha(candidate.get("artifact_sha256"), "signed-stack candidate artifact")
        if _canonical(candidate) != _canonical(expected_candidate):
            raise ValueError("signed-stack candidate receipt content drifted")
    _sha(receipt.get("artifact_sha256"), "signed-stack ladder artifact")
    if _canonical(receipt) != _canonical(expected):
        raise ValueError("signed-stack ladder receipt content drifted")
...
def _objective_rows(
    ...
    *,
    ...
    candidate_artifacts = _mapping(
        ...
        "signed-stack candidate artifacts",
    )
    ...
    for family in inner_families:
        raw_candidates = _mapping(
            raw_families[family], f"{family} signed-stack exact KL objectives"
        )
        if set(raw_candidates) != set(SOFT_POLARITY_SIGNED_STACK_CANDIDATE_IDS):
            raise ValueError(
            ...
            objective_hashes[family][candidate_id] = _hash(
                _OBJECTIVE_DOMAIN,
                {
                    ...
                    "candidate_artifact_sha256": _sha(
                        ...
                        "signed-stack candidate artifact",
                    ),
                    "objective": objective,
        ...
        family_receipts[family] = _finish(
            "fisher_graph.complete_h4_soft_polarity_signed_stack_family_oof.v20l",
            _FAMILY_DOMAIN,
            {
                "protocol_sha256": (
                    SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256
...
def _build_selection_receipt(
    *,
    ...
    validate_soft_polarity_signed_stack_ladder_receipt(ladder_receipt)
    ladder = _mapping(ladder_receipt, "signed-stack ladder receipt")
    ...
    candidate_by_id = {
        _identifier(candidate["candidate_id"], "signed-stack candidate id"): candidate
        for candidate in _sequence(
            ladder["candidate_receipts"], "signed-stack candidate receipts"
        )
    }
    ranking = tuple(
        sorted(
            ...
            key=lambda candidate_id: (
                ...
                abs(float(candidate_by_id[candidate_id]["s"])),
    ...
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_signed_stack_selection.v20l",
        _SELECTION_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256,
...
def build_soft_polarity_signed_stack_inner_oof_selection_receipt(
    *,
    ladder_receipt: Mapping[str, object],
    all_development_family_ids: Sequence[str],
    outer_held_family_id: str,
    ...
    """Select one V20l response pair from conditional-inner-LOFO KL scores."""
...
def validate_soft_polarity_signed_stack_inner_oof_selection_receipt(
    value: Mapping[str, object],
    *,
    ladder_receipt: Mapping[str, object],
    all_development_family_ids: Sequence[str],
    ...
    """Replay the full V20l selection from authoritative exact KL values."""

    receipt = _mapping(value, "signed-stack selection receipt")
    ...
    if set(receipt) != set(expected):
        raise ValueError("signed-stack selection receipt key set drifted")
    _sha(receipt.get("artifact_sha256"), "signed-stack selection artifact")
    if _canonical(receipt) != _canonical(expected):
        raise ValueError("signed-stack selection receipt content drifted")
