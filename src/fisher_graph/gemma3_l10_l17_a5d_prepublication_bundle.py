"""Private crash-recovery bundle for final A5d report publication."""
...
__all__ = [
    ...
    "load_a5d_prepublication_bundle",
    "publish_a5d_report_with_prepublication_bundle",
    "save_a5d_prepublication_bundle",
]


_SCHEMA = "fisher_graph.gemma3_l10_l17_a5d_prepublication_bundle.v1"
_DOMAIN = b"fisher-graph:a5d-prepublication-bundle:v1\0"
_INPUT_DOMAIN = b"fisher-graph:a5d-prepublication-report-inputs:v1\0"
...
_BUILD_ROLE = "exact_tensor_free_inputs_for_build_gemma3_l10_l17_a5d_report"
...
def _normalize_inputs(value: object) -> dict[str, object]:
    if not isinstance(value, Mapping) or set(value) != _INPUT_FIELDS:
        raise ValueError("A5d prepublication report-input fields are invalid")
    _reject_sensitive(value)
    try:
        normalized = json.loads(_canonical_json_bytes(value))
    except (TypeError, ValueError, OverflowError) as error:
        raise ValueError("A5d prepublication inputs are not strict JSON") from error
    if not isinstance(normalized, dict) or set(normalized) != _INPUT_FIELDS:
        raise ValueError("A5d prepublication inputs changed during normalization")
    return normalized


def default_a5d_prepublication_bundle_path(final_output: Path | str) -> Path:
    destination = Path(final_output)
    if destination.suffix != ".json" or not destination.name:
        raise ValueError("A5d final output must have a JSON basename")
    return destination.with_name(
        f"{destination.stem}.prepublication-bundle.json"
    )
...
def _write_exclusive_atomic(path: Path, value: Mapping[str, object]) -> None:
    ...
    if len(encoded) > _MAX_BYTES:
        raise ValueError("A5d prepublication bundle exceeds its size bound")
...
def _validate_bundle(
    ...
    if not isinstance(value, Mapping) or set(value) != _FIELDS:
        raise ValueError("A5d prepublication bundle fields are invalid")
    if (
        value.get("schema") != _SCHEMA
        or value.get("format_version") != 1
        or value.get("status") != "ready_for_report_build"
        or value.get("target_report_schema") != GEMMA3_L10_L17_A5D_REPORT_SCHEMA
        ...
        or source != default_a5d_prepublication_bundle_path(final_output)
    ):
        raise ValueError("A5d prepublication bundle header/path is invalid")
    inputs = _normalize_inputs(value.get("report_inputs"))
    if value.get("report_inputs_sha256") != _sha256(_INPUT_DOMAIN, inputs):
        raise ValueError("A5d prepublication report-input hash mismatch")
    payload = dict(value)
    supplied = payload.pop("bundle_sha256")
    if supplied != _sha256(_DOMAIN, payload):
        raise ValueError("A5d prepublication bundle hash mismatch")
    return {**dict(value), "report_inputs": inputs}


def save_a5d_prepublication_bundle(
    path: Path | str,
    *,
    ...
    if destination != default_a5d_prepublication_bundle_path(final):
        raise ValueError("A5d prepublication path differs from final output")
    inputs = _normalize_inputs(report_inputs)
    payload: dict[str, object] = {
        "schema": _SCHEMA,
        "format_version": 1,
        "status": "ready_for_report_build",
        "target_report_schema": GEMMA3_L10_L17_A5D_REPORT_SCHEMA,
        "target_report_format_version": GEMMA3_L10_L17_A5D_REPORT_FORMAT_VERSION,
...
def load_a5d_prepublication_bundle(
    ...
    try:
        ...
        raise ValueError("A5d prepublication bundle is unavailable") from error
    try:
        if (
            ...
            raise ValueError("A5d prepublication bundle file boundary is invalid")
        ...
        raise ValueError("A5d prepublication bundle is not strict JSON") from error
    finally:
...
def finalize_a5d_prepublication_bundle(
    ...
    if output is None:
        ...
        if not source.name.endswith(suffix):
            raise ValueError("cannot derive A5d final output from bundle path")
        final = source.with_name(f"{source.name[:-len(suffix)]}.json")
    else:
        final = Path(output)
    bundle = load_a5d_prepublication_bundle(source, final_output=final)
    inputs = bundle["report_inputs"]
    assert isinstance(inputs, Mapping)
    report = build_gemma3_l10_l17_a5d_report(**inputs)
    saved = save_gemma3_l10_l17_a5d_report(final, report)
    _remove_bundle(source)
    return saved


def publish_a5d_report_with_prepublication_bundle(
    *, output: Path | str, report_inputs: Mapping[str, object]
) -> dict[str, object]:
    """Checkpoint exact builder inputs, publish, then clean up on success."""

    final = Path(output)
    bundle_path = default_a5d_prepublication_bundle_path(final)
    save_a5d_prepublication_bundle(
        bundle_path, final_output=final, report_inputs=report_inputs
    )
    # Every failure after the checkpoint deliberately preserves the bundle.
    return finalize_a5d_prepublication_bundle(bundle_path, output=final)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    finalize_a5d_prepublication_bundle(args.bundle, output=args.output)
    return 0
