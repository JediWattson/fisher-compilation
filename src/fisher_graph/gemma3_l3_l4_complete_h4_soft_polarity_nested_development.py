"""V20f nested family-held-out development for a smooth polarity router.

The experiment is deliberately confined to the historically reused A16
development panel.  Each outer fold reconstructs the exact V20a seven-family
endpoint, fits a four-scalar continuous router without the held family, freezes
the learned provider and three controls, and only then opens a capability for
the two held prompts.  A successful eight-fold screen triggers a separate
all-family endpoint and router refit; Calibration-B is merely marked eligible
after that final provider receipt is frozen and is never loaded or tokenized
here.
...
from . import complete_h4_fisher_soft_polarity_fit as _core
...
__all__ = [
    ...
    "build_parser",
    "main",
    "run_gemma3_l3_l4_complete_h4_soft_polarity_nested_development",
]


_LOCAL_ROOT = Path(".local-runs/google--gemma-3-270m")
DEFAULT_OUTPUT = _LOCAL_ROOT / (
    "modal-generator-l3-l4-complete-h4-soft-polarity-nested-"
    "r16-k256-a-fit16-dev-v20f.json"
)
...
_SCHEMA = "fisher_graph.gemma3_l3_l4.complete_h4_soft_polarity_nested.v20f"
_FORMAT_VERSION = 22
_FOLD_SCHEMA = "fisher_graph.complete_h4_soft_polarity_outer_fold.v20f"
_FINAL_SCHEMA = "fisher_graph.complete_h4_soft_polarity_final_refit.v20f"
_REPORT_DOMAIN = b"fisher-graph:soft-polarity-nested-report:v20f\0"
_SOURCE_DOMAIN = b"fisher-graph:soft-polarity-nested-source:v20f\0"
_FOLD_DOMAIN = b"fisher-graph:soft-polarity-nested-fold:v20f\0"
_FINAL_DOMAIN = b"fisher-graph:soft-polarity-nested-final:v20f\0"
_FIT_EXECUTION_DOMAIN = b"fisher-graph:soft-polarity-fit-execution:v20f\0"
_HELD_EXECUTION_DOMAIN = b"fisher-graph:soft-polarity-held-execution:v20f\0"
_PROVIDER_MANIFEST_DOMAIN = b"fisher-graph:soft-polarity-provider-manifest:v20f\0"
_ENDPOINT_DOMAIN = b"fisher-graph:soft-polarity-all-family-endpoint:v20f\0"
...
_FIXED_PROTOCOL: dict[str, object] = {
    "protocol": "v20f_eight_fold_soft_polarity_nested_development",
    "scientific_status": "historically_reused_A16_development_only",
    ...
    "router_selection": "fixed_alpha_ladder_exact_family_equal_teacher_KL",
    "held_arms": _ARMS,
...
def _validate_output(path: Path | str) -> Path:
    ...
    if destination.suffix != ".json" or not _v20b._is_under_local_runs(destination):
        raise ValueError("V20f output must be JSON under .local-runs")
    protected = (_V20B_OUTPUT, _V20E_OUTPUT, _v20a.DEFAULT_OUTPUT)
    if any(_v20b._same_destination(destination, item) for item in protected):
        raise ValueError("V20f must preserve immutable prerequisite reports")
    return destination
...
def _load_prerequisites() -> tuple[
    ...
    """Authenticate V20a, the complete V20b graph, and V20e before Gemma."""

    prerequisite, _v20a_payload, folds = _v20b._load_authenticated_v20a_artifact()
    if _v14._file_sha256(_V20B_OUTPUT) != _V20B_FILE_SHA256:
        raise RuntimeError("pinned V20b report file hash drifted")
    report_inputs = _v20b._load_report_ready_checkpoint(output=_V20B_OUTPUT)
    rebuilt_v20b = _v20b._build_report_from_inputs(
        output=_V20B_OUTPUT, report_inputs=report_inputs
    )
    rebuilt_v20b["report_sha256"] = _v14._sha256(
        rebuilt_v20b, domain=_v20b._REPORT_DOMAIN
    )
    raw_v20b = json.loads(_V20B_OUTPUT.read_bytes())
    if (
        not isinstance(raw_v20b, Mapping)
        or raw_v20b.get("report_sha256") != _V20B_LOGICAL_SHA256
        or raw_v20b.get("classification") != "nested_inner_selection_failed"
        or raw_v20b.get("passed") is not False
        or _v14._canonical_json_bytes(rebuilt_v20b)
        != _v14._canonical_json_bytes(raw_v20b)
    ):
        raise RuntimeError("pinned V20b authority differs")

    if _v14._file_sha256(_V20E_OUTPUT) != _V20E_FILE_SHA256:
        raise RuntimeError("pinned V20e report file hash drifted")
    v20e = _v20e._load_existing_report(_V20E_OUTPUT)
    if (
        v20e.get("report_sha256") != _V20E_LOGICAL_SHA256
        or v20e.get("classification") != "tangent_response_pair_smoke_failed"
        or v20e.get("passed") is not False
    ):
        raise RuntimeError("pinned V20e authority differs")
    source = _hashed(
        {
            "v20a_report_sha256": prerequisite["report_sha256"],
            "v20a_file_sha256": prerequisite["file_sha256"],
            "v20b_report_sha256": _V20B_LOGICAL_SHA256,
            "v20b_file_sha256": _V20B_FILE_SHA256,
            "v20e_report_sha256": _V20E_LOGICAL_SHA256,
            "v20e_file_sha256": _V20E_FILE_SHA256,
            "v20b_pair_fragment_sha256s": dict(
                sorted(_mapping(raw_v20b["pair_fragment_sha256s"], label="V20b pairs").items())
            ),
...
def _eta_tensor(value: Sequence[float] | Tensor) -> Tensor:
    ...
    if (
        ...
        raise ValueError("V20f eta must contain four finite float64 scalars")
...
def _local_eta_gradient(
    ...
    if (
        ...
        raise ValueError("V20f suffix H4 gradient geometry differs")
    ...
    if (
        ...
        raise RuntimeError("V20f local eta gradient became invalid")
...
def _execution_sha256(
    *,
    ...
    domain = _HELD_EXECUTION_DOMAIN if phase.startswith("held_") else _FIT_EXECUTION_DOMAIN
    return _v14._sha256(
        {
            "phase": phase,
    ...
    *,
    ...
    for sequence in sequences:
        ...
        if selected.numel() == 0 or not bool(torch.isfinite(selected).all()):
            raise RuntimeError("V20f provider gain trace is empty or nonfinite")
...
def _freeze_held_providers(
    ...
    *,
    ...
    if set(providers) != set(_ARMS):
        raise RuntimeError("V20f held provider arm geometry differs")
    ...
    if len(set(provider_hashes.values())) != len(_ARMS):
        raise RuntimeError("V20f held provider artifacts are not distinct")
    manifest = _hashed(
        {
            "outer_held_family_id": outer_family_id,
            "endpoint_receipt_sha256": endpoint.receipt["artifact_sha256"],
...
def _outer_endpoint(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("V20f outer endpoint differs from authenticated V20a")
    evidence = _hashed(
        {
...
def _all_family_endpoint(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("V20f all-family endpoint training geometry differs")
    ...
    for record in training:
        ...
        if len(captured) != 1 or captured[0] != score:
            raise RuntimeError("V20f all-family endpoint objective capture drifted")
    ...
    if any(
        ...
        raise RuntimeError("V20f all-family endpoint health failed")
    ...
    _v20b._validate_capability_receipt(
        ...
        label="V20f all-family endpoint capability",
    )
    evidence = _hashed(
        {
...
def _values_by_family(
    ...
    for record in _v20b._ordered_records(records):
        ...
        if example not in values:
            raise ValueError("V20f family map omitted an example")
        result.setdefault(record.sequence.family_id, {})[example] = values[example]
    if set(values) != expected:
        raise ValueError("V20f family map contains an unknown example")
    ...
    for key in ("natural_direction", "direction", "direction_values", "eta_direction"):
        ...
        if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
            ...
            if len(selected) == FISHER_SOFT_POLARITY_ETA_COUNT and all(
                ...
                return selected
    raise ValueError("V20f direction receipt omitted the finite four-vector")
...
def _fit_soft_router(
    ...
    *,
    ...
    """Fit the four-scalar natural-gradient ray and exact alpha ladder."""
    ...
    if (
        ...
        raise RuntimeError("V20f router training complement differs")
    ...
    for record in training:
        ...
        if len(captured) != 1 or captured[0] != score:
            raise RuntimeError("V20f eta-zero objective capture drifted")
    ...
    direction_values = _direction_vector(direction)
    ...
    if not alphas or alphas[0] != 0.0 or any(
        ...
        raise RuntimeError("V20f fixed alpha ladder differs")

    # Every training candidate is built and hash-bound before the first
    # positive-alpha exact score.  The held family is still absent here.
    ...
    for alpha in alphas:
        ...
        providers[alpha] = (
            eta_zero
            if alpha == 0.0
            else build_autonomous_complete_h4_fisher_soft_polarity(
                endpoint.base_provider,
                endpoint.proposal_provider,
                eta=_eta_tensor(tuple(alpha * item for item in direction_values)),
                transfer_protocol_sha256=_core.SOFT_POLARITY_FIT_PROTOCOL_SHA256,
    ...
    _v20b._validate_capability_receipt(
        ...
        label="V20f router-fit capability",
    )
    receipt = _hashed(
        {
            "outer_held_family_id": outer,
...
def _score_held_fold(
    ...
    *,
    ...
    if len(held) != _PROMPTS_PER_FAMILY:
        raise RuntimeError("V20f held fold prompt geometry differs")
    ...
    if (
        ...
        raise PermissionError("V20f held provider freeze barrier is not satisfied")
    ...
    for arm in _ARMS:
        ...
        if set(family_scores) != {outer}:
            raise RuntimeError("V20f held objective family geometry differs")
        execution_receipt = _v14._sha256(
            {
                "seed_sha256": seed,
    ...
    if (
        ...
        raise RuntimeError("V20f held fold selection drifted after freeze")
...
def _validate_fit_bundle(
    ...
    *,
    ...
    if len(candidates) != len(_core.SOFT_POLARITY_FIT_ALPHAS):
        raise ValueError("V20f fit alpha ladder is incomplete")
    ...
    for candidate in candidates:
        ...
        if alpha in candidates_by_alpha:
            raise ValueError("V20f fit alpha ladder contains a duplicate")
    ...
    if set(candidates_by_alpha) != set(expected_alphas):
        raise ValueError("V20f fit alpha ladder differs")
    selected = min(
        candidates,
        key=lambda item: (
            float(item["family_equal_train_objective"]),
    ...
    gradient_evidence = _validate_hashed(
        ...
        domain=_FIT_EXECUTION_DOMAIN,
        label="V20f fit gradient evidence",
    )
    provider_manifest = _validate_hashed(
        ...
        domain=_PROVIDER_MANIFEST_DOMAIN,
        label="V20f fit candidate provider manifest",
    )
    ...
    if set(provider_hashes) != set(alpha_keys):
        raise ValueError("V20f fit candidate provider ladder differs")
    ...
    if set(candidate_evidence) != set(alpha_keys):
        raise ValueError("V20f fit candidate evidence ladder differs")
    ...
    for field, label in (
        ...
        if set(hashes) != set(training_example_ids):
            raise ValueError("V20f fit gradient execution geometry differs")
    ...
    _v20b._validate_capability_receipt(
        ...
        label="V20f resumed router-fit capability",
    )
    if (
        ...
        raise ValueError("V20f fit capability or gradient boundary differs")
    all_families = tuple(
    ...
    for alpha in expected_alphas:
        ...
        row = _validate_hashed(
            ...
            domain=_FIT_EXECUTION_DOMAIN,
            label=f"V20f fit candidate evidence {key}",
        )
        ...
        if set(objectives_by_family) != set(training_families) or any(
            ...
            raise ValueError("V20f fit candidate prompt geometry differs")
        ...
        if not all(math.isfinite(item) for item in flattened_objectives.values()):
            raise ValueError("V20f fit candidate objective is nonfinite")
        ...
        if (
            ...
            raise ValueError("V20f fit candidate execution geometry differs")
        trace = _validate_hashed(
            _mapping(row.get("response_trace"), label=f"fit candidate {key} trace"),
            domain=_PROVIDER_MANIFEST_DOMAIN,
            label=f"V20f fit candidate {key} trace",
        )
        ...
        if set(trace_gain_hashes) != set(training_example_ids):
            raise ValueError("V20f fit candidate trace geometry differs")
        ...
        if (
            ...
            raise ValueError("V20f fit candidate execution binding differs")
    if _v14._canonical_json_bytes(
        ...
        raise ValueError("V20f eta-zero objective evidence differs")
    ...
    if (
        ...
        raise ValueError("V20f eta-zero execution evidence differs")
    ...
    if endpoint_receipt is not None or endpoint_evidence is not None:
        ...
        if (
            ...
            raise ValueError("V20f fit endpoint lineage differs")
    return fit, evidence
...
def _validate_outer_endpoint_bundle(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("V20f outer endpoint lineage differs")
...
def _validate_held_bundle(
    ...
    *,
    ...
    if set(provider_hashes) != set(_ARMS) or set(provider_receipts_raw) != set(_ARMS):
        raise ValueError("V20f held provider arm geometry differs")
    provider_receipts = {
        arm: _validate_hashed(
            _mapping(provider_receipts_raw[arm], label=f"held {arm} provider receipt"),
            domain=_PROVIDER_MANIFEST_DOMAIN,
            label=f"V20f held {arm} provider receipt",
        )
        for arm in _ARMS
    }
    for arm in _ARMS:
        if (
            ...
            raise ValueError("V20f held provider receipt binding differs")
    ...
    if (
        ...
        raise ValueError("V20f held provider manifest lineage differs")

    arm_evidence_raw = _mapping(
        held_evidence.get("arm_evidence"), label="held arm evidence"
    )
    if set(arm_evidence_raw) != set(_ARMS):
        raise ValueError("V20f held evidence arm geometry differs")
    ...
    for arm in _ARMS:
        ...
        if not objectives or not all(math.isfinite(item) for item in objectives.values()):
            raise ValueError("V20f held objectives are empty or nonfinite")
        h4_hashes = _mapping(
        ...
        if set(h4_hashes) != set(objectives) or set(logits_hashes) != set(objectives) or set(executions) != set(objectives):
            raise ValueError("V20f held execution example geometry differs")
        ...
        if (
            ...
            raise ValueError("V20f held arm lineage differs")
    ...
    for arm in _ARMS:
        ...
        if (
            ...
            raise ValueError("V20f held exact execution commitment differs")
    ...
    if len(held_ids) != _PROMPTS_PER_FAMILY or any(
        ...
        raise ValueError("V20f held arm prompt geometry differs")
    _v20b._validate_capability_receipt(
        ...
        expected_held_family_id=None,
        expected_accesses_per_example=len(_ARMS),
        label="V20f resumed held capability",
    )
    selected_trace = traces["soft_router"]
    all_healthy = all(
    ...
    if (
        ...
        raise ValueError("V20f held evidence or fold binding differs")
...
def _validate_fold_fragment(
    ...
    *,
    ...
    if set(selected) != _FOLD_FRAGMENT_KEYS:
        raise ValueError("V20f fold fragment fields differ")
    ...
    if (
        ...
        raise ValueError("V20f fold fragment lineage differs")
    _v14._scalar_report(selected)
    return selected
...
def _publish_fold_fragment(
    ...
    return _v20b._publish_scalar_fragment(
        ...
        label="V20f outer-fold fragment",
    )
...
def _execute_final_refit(
    ...
    *,
    ...
    if oof_qualification.get("full_refit_authorized") is not True:
        raise PermissionError("V20f all-family refit requires passing OOF authority")
...
def _validate_all_family_endpoint_bundle(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("V20f all-family endpoint execution geometry differs")
    ...
    capability = _v20b._validate_capability_receipt(
        ...
        expected_held_family_id=None,
        expected_accesses_per_example=1,
        label="V20f final endpoint capability",
    )
    del capability
    ...
    if (
        ...
        raise ValueError("V20f all-family endpoint lineage differs")
...
def _validate_final_fragment(
    ...
    *,
    ...
    if set(selected) != _FINAL_FRAGMENT_KEYS:
        raise ValueError("V20f final-refit fragment fields differ")
    if (
    ...
    if set(trace_gain_hashes) != set(endpoint_examples):
        raise ValueError("V20f final response-gain trace geometry differs")
    for item in trace_gain_hashes.values():
    ...
    if trace.get("response_gain_nonconstant") is not trace_nonconstant:
        raise ValueError("V20f final response-gain health differs")
    qualifies = bool(
    ...
    if (
        ...
        raise ValueError("V20f final-refit lineage differs")
    _v14._scalar_report(selected)
    return selected
...
def _publish_final_fragment(
    ...
    return _v20b._publish_scalar_fragment(
        ...
        hash_key="fragment_sha256",
        label="V20f all-family final-refit fragment",
    )
...
def _build_report(
    *,
    ...
    if gate is not (final_fragment is not None):
        raise ValueError("V20f report final-refit gate differs")
    ...
    report = {
        ...
        "artifact": {
            ...
            "role": "historically_reused_A16_soft_polarity_nested_development",
        },
        "runner_protocol_sha256": _RUNNER_PROTOCOL_SHA256,
        "core_protocol_sha256": _core.SOFT_POLARITY_FIT_PROTOCOL_SHA256,
        ...
        "classification": (
            "soft_polarity_oof_passed_final_provider_frozen"
            if eligible
            else (
                "soft_polarity_oof_passed_final_refit_rolled_back_to_base"
                if gate
                else "soft_polarity_oof_failed_rollback_to_base"
            )
        ),
        "passed": eligible,
        "oof_passed": gate,
...
def _load_existing_report(
    ...
    *,
    ...
    if qualification.get("full_refit_authorized") is True:
        ...
        raise ValueError("V20f failed OOF report has a stale final-refit fragment")
    ...
    if (
        ...
        raise ValueError("V20f report reconstruction differs")
    return dict(value)


def run_gemma3_l3_l4_complete_h4_soft_polarity_nested_development(
    *,
    ...
    panel_receipt = dict(
        _mapping(
            ...
            label="V20f nested panel receipt",
        )
    )
    bridge_binding = _sha(
        prerequisite.get("authenticated_bridge_binding_sha256"),
        label="V20f authenticated bridge",
    )
    ...
    family_ids = tuple(
        sorted(
            _mapping(
                panel_receipt.get("family_prompt_sha256s"),
                label="V20f development families",
            )
        )
    )
    if (
        ...
        raise RuntimeError("V20f authenticated family geometry differs")
    ...
    if all(_fold_path(destination, family).exists() for family in family_ids):
        ...
        if completed_qualification.get("full_refit_authorized") is True:
            ...
            raise ValueError("V20f failed OOF campaign has a final-refit fragment")
    ...
    try:
        ...
        if context.bridge.bridge_binding_sha256 != bridge_binding:
            raise RuntimeError("V20f live bridge differs from authenticated V20a")
        ...
        if tuple(live_families) != family_ids:
            raise RuntimeError("V20f live family order differs from authenticated A16")
        ...
        fold_receipts = tuple(
            _mapping(fragments[family]["fold_receipt"], label="V20f fold")
            for family in family_ids
        )
        qualification = _core.build_soft_polarity_oof_qualification(
            fold_receipts=fold_receipts
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the historically reused A16 V20f soft-polarity nested development screen"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_gemma3_l3_l4_complete_h4_soft_polarity_nested_development(
