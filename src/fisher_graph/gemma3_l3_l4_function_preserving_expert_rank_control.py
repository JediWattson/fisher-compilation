"""Run the fixed-outer-rank function-preserving expert-rank control.

The paired fit-side capacity oracle holds the outer latent width, two experts,
router, objective, data, optimizer, and fit budget fixed.  It compares the
authenticated expert-rank-16 replay with an expert-rank-64 treatment that
starts at the same observable function and provider-chart JVP.
...
from . import (
    gemma3_l3_l4_function_preserving_expert_rank_control_protocol
...
__all__ = [
    ...
    "LoadedFunctionPreservingExpertRankControlArtifact",
    "build_parser",
    "describe_function_preserving_expert_rank_control",
    "load_function_preserving_expert_rank_control_artifact",
    "main",
    "run_function_preserving_expert_rank_control",
]


DEFAULT_SOURCE_WIDTH = width.DEFAULT_OUTPUT
...
DEFAULT_OUTPUT = Path(
    ...
    "modal-generator-l3-l4-function-preserving-expert-rank-control.pt"
)
...
_FORMAT_VERSION = 1
_SOURCE_CANDIDATE_ID = "fp_width.primary.rank64"
...
_ARTIFACT_DOMAIN = b"fisher-graph:function-preserving-expert-rank:artifact:v1\0"
_REPORT_DOMAIN = b"fisher-graph:function-preserving-expert-rank:report:v1\0"
_CODE_DOMAIN = b"fisher-graph:function-preserving-expert-rank:code:v1\0"
_RESULT_DOMAIN = b"fisher-graph:function-preserving-expert-rank:result:v1\0"
_AUDIT_DOMAIN = b"fisher-graph:function-preserving-expert-rank:audit:v1\0"
_MEASUREMENT_DOMAIN = (
    b"fisher-graph:function-preserving-expert-rank:measurement:v1\0"
)
...
class _AuthenticatedSources:
    width_result: width.LoadedFunctionPreservingWidthControlArtifact
    width_plan: ContrastAwareReferenceProviderPlan
...
@dataclass(frozen=True, slots=True)
class LoadedFunctionPreservingExpertRankControlArtifact:
...
def _objective(
    protocol: FunctionPreservingExpertRankControlProtocol,
) -> ContrastAwareObjective:
...
class _ResidualExpertRankLift(nn.Module):
    """Fit-only split bank equivalent to one concatenated rank-64 executor."""

    def __init__(
        self,
        base: ResidualGatedCausalModalExecutor,
        *,
        target_config: GatedCausalModalExecutorConfig,
    ) -> None:
        super().__init__()
        if (
            base.config.input_modes != 66
            or base.config.output_modes != 64
            or base.config.expert_count != 2
            or base.config.expert_rank != 16
            or target_config.input_modes != 66
            or target_config.output_modes != 64
            or target_config.expert_count != 2
            or target_config.expert_rank != 64
            or asdict(base.config)
            | {"expert_rank": target_config.expert_rank}
            != asdict(target_config)
        ):
            raise ValueError("expert-rank split-bank geometry drifted")
        self.base = base
        ...
        with torch.no_grad():
            indices = torch.arange(48, device=base.device)
            self.extra_input_weight[0, 17 + indices, indices] = 1.0
            self.extra_input_weight[1, 17 + indices, indices] = -1.0
...
def _new_control_model(
    *,
    ...
    rank16 = width._new_training_model(
        ...
        config=width._executor_config(width_protocol.rank16_executor),
        seed=seed,
    )
    return width._lift_rank16_model(
        rank16,
        modal_center=modal_center,
        gain_log_center=gain_log_center,
        gain_log_scale=gain_log_scale,
        ...
        target_center=target_center,
        target_scale=target_scale,
        config=width._executor_config(width_protocol.rank64_executor),
        seed=seed,
    )
...
def _new_treatment_model(
    *,
    ...
    if any(
        ...
        raise RuntimeError("expert-rank treatment base initialization drifted")
    treatment.executor = _ResidualExpertRankLift(
        treatment.executor,
        target_config=_executor_config(protocol.expert64_executor),
    )
    return treatment
...
def _parameter_bindings(model: nn.Module) -> dict[str, str]:
    ...
    if isinstance(executor, _ResidualExpertRankLift):
...
def _concatenated_packed_model(
    ...
    if not isinstance(split, _ResidualExpertRankLift):
        raise TypeError("concatenated view requires split expert-rank lift")
...
def _initial_equivalence(
    ...
    *,
    ...
    if not isinstance(treatment.executor, _ResidualExpertRankLift):
        raise TypeError("expert-rank treatment lacks the split lift")
    ...
    state = {
        "artifact_kind": "fisher_graph.initial_expert_rank_equivalence",
        "format_version": _FORMAT_VERSION,
        "pair_role": pair_role,
        "seed": seed,
...
def _gradient_audit(
    *,
    applicable: bool,
    pair_role: str,
    step1_extra_output_gradient_norm: float,
    step1_extra_input_gradient_norm: float,
    step1_extra_output_parameter_delta_norm: float,
    step1_extra_input_parameter_delta_norm: float,
    step2_extra_output_gradient_norm: float,
    step2_extra_input_gradient_norm: float,
    step2_extra_output_parameter_delta_norm: float,
    step2_extra_input_parameter_delta_norm: float,
    protocol: FunctionPreservingExpertRankControlProtocol,
) -> dict[str, object]:
    floor = protocol.lift.gradient_norm_floor
    zero_tolerance = 0.0
    flags = {
        "step1_extra_output_gradient_open": (
            not applicable or step1_extra_output_gradient_norm > floor
        ),
        "step1_extra_input_gradient_zero": (
            not applicable
            or step1_extra_input_gradient_norm <= zero_tolerance
        ),
        "step1_extra_output_parameter_open": (
            not applicable
            or step1_extra_output_parameter_delta_norm > floor
        ),
        "step1_extra_input_parameter_unchanged": (
            not applicable
            or step1_extra_input_parameter_delta_norm <= zero_tolerance
        ),
        "step2_extra_output_gradient_open": (
            not applicable or step2_extra_output_gradient_norm > floor
        ),
        "step2_extra_input_gradient_open": (
            not applicable or step2_extra_input_gradient_norm > floor
        ),
        "step2_extra_output_parameter_open": (
            not applicable
            or step2_extra_output_parameter_delta_norm > floor
        ),
        "step2_extra_input_parameter_open": (
            not applicable
            or step2_extra_input_parameter_delta_norm > floor
        ),
    }
    state = {
        "artifact_kind": "fisher_graph.expert_rank_gradient_openness",
        "format_version": _FORMAT_VERSION,
        "pair_role": pair_role,
        "applicable": applicable,
        "step1_extra_output_gradient_norm": (
            step1_extra_output_gradient_norm
        ),
        "step1_extra_input_gradient_norm": (
            step1_extra_input_gradient_norm
        ),
        "step1_extra_output_parameter_delta_norm": (
            step1_extra_output_parameter_delta_norm
        ),
        "step1_extra_input_parameter_delta_norm": (
            step1_extra_input_parameter_delta_norm
        ),
        "step2_extra_output_gradient_norm": (
            step2_extra_output_gradient_norm
        ),
        "step2_extra_input_gradient_norm": step2_extra_input_gradient_norm,
        "step2_extra_output_parameter_delta_norm": (
            step2_extra_output_parameter_delta_norm
        ),
        "step2_extra_input_parameter_delta_norm": (
            step2_extra_input_parameter_delta_norm
        ),
...
def _fit_from_initialized_model(
    ...
    *,
    ...
    split = model.executor
    if audit_added_rank and not isinstance(split, _ResidualExpertRankLift):
        raise TypeError("expert-rank treatment lacks split-bank executor")
    if not audit_added_rank and isinstance(split, _ResidualExpertRankLift):
        raise TypeError("expert-rank control unexpectedly has split bank")
    ...
    if audit_added_rank:
        ...
        if initial_parity["passed"] is not True:
            raise RuntimeError(
                "initial split/concatenated expert-rank parity failed"
            )
    ...
    if audit_added_rank:
        assert isinstance(split, _ResidualExpertRankLift)
        initial_extra_input = split.extra_input_weight.detach().clone()
        initial_extra_output = split.extra_output_weight.detach().clone()
    for step in range(steps):
        ...
        if not bool(torch.isfinite(components.weighted_total)):
            raise ValueError("expert-rank fit produced a nonfinite loss")
        ...
        if any(
            ...
            raise ValueError("expert-rank fit produced a nonfinite gradient")
        if audit_added_rank and step in (0, 1):
            assert isinstance(split, _ResidualExpertRankLift)
            input_gradient = split.extra_input_weight.grad
            output_gradient = split.extra_output_weight.grad
            assert input_gradient is not None
            assert output_gradient is not None
            prefix = f"step{step + 1}"
            values[f"{prefix}_extra_input_gradient_norm"] = float(
                torch.linalg.vector_norm(input_gradient.detach())
            )
            values[f"{prefix}_extra_output_gradient_norm"] = float(
                torch.linalg.vector_norm(output_gradient.detach())
            )
        optimizer.step()
        if audit_added_rank and step in (0, 1):
            assert isinstance(split, _ResidualExpertRankLift)
            prefix = f"step{step + 1}"
            values[f"{prefix}_extra_input_parameter_delta_norm"] = float(
                torch.linalg.vector_norm(
                    split.extra_input_weight.detach() - initial_extra_input
                )
            )
            values[f"{prefix}_extra_output_parameter_delta_norm"] = float(
                torch.linalg.vector_norm(
                    split.extra_output_weight.detach() - initial_extra_output
                )
            )
    model.eval()
    ...
    if audit_added_rank:
        ...
        if postfit_parity["passed"] is not True:
            raise RuntimeError(
                "post-fit split/concatenated expert-rank parity failed"
            )
    else:
        plan_final_metrics = final
        postfit_parity = {
            "artifact_kind": (
                "fisher_graph.expert_rank_wrapper_concat_parity"
            ),
            "format_version": _FORMAT_VERSION,
            "pair_role": pair_role,
    ...
    if bindings != {synthetic_binding_sha256}:
        raise ValueError("expert-rank fit batches lost their binding")
...
def _authenticated_declarations(
    *,
    protocol_override: (
        FunctionPreservingExpertRankControlProtocol | None
    ) = None,
) -> tuple[
    FunctionPreservingExpertRankControlProtocol,
    object,
...
def _authenticate_sources(
    *,
    ...
    if not isinstance(plans, Mapping) or not isinstance(rows, Mapping):
        raise ValueError("authenticated width source tables are absent")
    ...
    if not isinstance(raw_plan, Mapping) or not isinstance(row, Mapping):
        raise ValueError("authenticated width rank64 source is absent")
    plan = ContrastAwareReferenceProviderPlan.from_state_dict(raw_plan)
    plan.validate_integrity()
    if (
        loaded.manifest.get("protocol_sha256")
        != source.width_protocol_sha256
        or loaded.manifest.get("code_bundle_sha256")
        != source.width_code_bundle_sha256
        or loaded.manifest.get("outcome") != source.width_outcome
        or loaded.manifest.get("primary_comparison_status")
        != source.width_primary_comparison_status
        or loaded.manifest.get("primary_treatment_valid")
        is not source.width_primary_treatment_valid
        or loaded.manifest.get("expert_core_control_authorized")
        is not source.width_expert_core_control_authorized
        or loaded.manifest.get("replication_executed") is not False
        or loaded.manifest.get("two_seed_outer_width_supported") is not False
        or loaded.manifest.get("compressed_width_ladder_authorized")
        is not False
        ...
        != source.width_primary_rank64_initial_metrics_sha256
        or plan.final_metrics.artifact_sha256
        != source.width_primary_rank64_final_metrics_sha256
        or row.get("fit_capability_pass") is not False
        or row.get("pair_comparison_status") != "both_fail"
        or plan.latent_rank != 64
        or plan.executor_config.expert_rank != 16
    ):
        raise ValueError("authenticated width source identity drifted")
    width_protocol = default_function_preserving_width_control_protocol()
...
def describe_function_preserving_expert_rank_control() -> dict[str, object]:
    ...
    report = {
        ...
        "control_expert_rank": 16,
        "treatment_expert_rank": 64,
...
def _prepare_live_fit_problem(
    *,
    ...
    if (
        ...
        raise ValueError("expert-rank execution and basis bindings are frozen")
    sources = _authenticate_sources(
        source_width_path=source_width_path,
        source_diagnostic_path=source_diagnostic_path,
        source_rank64_path=source_rank64_path,
        protocol=protocol,
    )
    ...
    if (
        ...
        raise ValueError("expert-rank gates drifted")
    ...
    if (
        ...
        raise ValueError("live execution differs from expert-rank protocol")
    ...
    if (
        ...
        raise ValueError("expert-rank calibration replay drifted")
    ...
    if any(
        ...
        raise RuntimeError("selection identity entered expert-rank fit")
    (
        modal_center,
        gain_log_center,
        gain_log_scale,
    ...
    if (
        ...
        raise ValueError("expert-rank Fisher gauge drifted")
    ...
    if (
        ...
        or fit_data_binding
        != sources.width_plan.synthetic_binding_sha256
        or tuple(value.artifact_sha256 for value in fit_pairs)
        != sources.source_d3.balanced_pair_sha256s
    ):
        raise ValueError("expert-rank fit problem differs from source")
...
def _run_fit_only_preflight(
    ...
    for pair_role, seed in (
        ...
        if (
            ...
            raise RuntimeError("expert-rank fit-only preflight failed")
        state = {
            "artifact_kind": (
                "fisher_graph.expert_rank_fit_only_preflight_seed"
            ),
            "format_version": _FORMAT_VERSION,
            "pair_role": pair_role,
            "seed": seed,
    ...
    preflight = {
        "artifact_kind": (
            "fisher_graph.expert_rank_fit_only_preflight"
        ),
        "format_version": _FORMAT_VERSION,
        "protocol_unsealed_sha256": protocol.protocol_sha256,
        "roles": by_role,
        "scoring_executed": False,
        "publication_executed": False,
    }
...
def _validate_fit_only_preflight(
    ...
    *,
    ...
    if not isinstance(preflight, Mapping) or set(preflight) != top_fields:
        raise ValueError("expert-rank live preflight schema drifted")
    ...
    if (
        ...
        or preflight.get("protocol_unsealed_sha256")
        ...
        raise ValueError("expert-rank live preflight binding drifted")
    ...
    for pair_role in ("primary", "replication"):
        ...
        if not isinstance(role, Mapping) or set(role) != role_fields:
            raise ValueError("expert-rank live preflight role schema drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank live preflight role drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank frozen live preflight drifted")
        if (
            control_gradient.get("passed") is not True
            or treatment_gradient.get("passed") is not True
        ):
            raise ValueError("expert-rank live preflight gradient failed")
    return preflight
...
def _source_replay_exact(
    *,
    ...
    if pair_role != "primary" or arm != "expert16":
        return False
    return (
        plan.artifact_sha256
        == protocol.source.width_primary_rank64_plan_sha256
        and plan.initial_metrics.artifact_sha256
        == protocol.source.width_primary_rank64_initial_metrics_sha256
        and plan.final_metrics.artifact_sha256
        == protocol.source.width_primary_rank64_final_metrics_sha256
    )
...
def _pair_status(control_pass: bool, treatment_pass: bool) -> str:
    ...
    if not control_pass and treatment_pass:
        return "expert16_fail_expert64_pass"
    if control_pass and treatment_pass:
        return "both_pass"
    return "expert16_pass_expert64_fail"
...
def _preflight_binding_for_role(
    ...
    if preflight is None or not hasattr(preflight, "for_role"):
        raise RuntimeError("expert-rank preflight binding is absent")
    binding = preflight.for_role(pair_role)
    if not isinstance(binding, Mapping):
        raise TypeError("expert-rank preflight role binding is invalid")
    return binding
...
def _evaluate_pair(
    *,
    ...
    if pair_role not in {"primary", "replication"}:
        raise ValueError("expert-rank pair role is invalid")
    expected_seed = (
    ...
    if seed != expected_seed:
        raise ValueError("expert-rank seed drifted")
    objective = _objective(protocol)
    data = contrast_fit._prepare_fit_data(
        fit_batches=problem.fit_batches,
        contrast_pairs=problem.fit_pairs,
    ...
    if (
        ...
        raise RuntimeError("expert-rank initial preflight binding drifted")
    ...
    if (
        ...
        raise RuntimeError("expert-rank live gradient/parity audit drifted")
    prefix = f"fp_expert_rank.{pair_role}"
    common = {
    ...
    control = _score_plan(
        candidate_id=f"{prefix}.expert16",
        pair_role=pair_role,
        arm="expert16",
        plan=control_plan,
    ...
    treatment = _score_plan(
        candidate_id=f"{prefix}.expert64",
        pair_role=pair_role,
        arm="expert64",
        plan=treatment_plan,
    ...
    flags = {
        ...
        "gradient_open_expert64": treatment_gradient["passed"] is True,
    ...
    validity = {
        ...
        "flags": flags,
        "failure_semantics": (
            "invalid_paired_expert_rank_comparison_no_capacity_conclusion"
        ),
    }
...
def _decision(
    ...
    if not primary.treatment_valid:
        ...
        raise RuntimeError("authorized expert-rank replication was not run")
    elif not replication.treatment_valid:
        outcome = "invalid_replication_pair"
    elif replication.comparison_status == (
        "expert16_fail_expert64_pass"
    ):
        outcome = "two_seed_inner_expert_rank_support"
    else:
        outcome = (
    ...
    return {
        ...
        "expert_count_control_authorized": outcome == "primary_both_fail",
        "fresh_c3_authorized": False,
...
def _interpretation_from_evidence(
    ...
    primary = rows.get("fp_expert_rank.primary.expert64")
    if not isinstance(primary, Mapping):
        raise ValueError("expert-rank primary treatment row is absent")
    ...
    if not isinstance(validity, Mapping):
        raise TypeError("expert-rank validity must be a mapping")
    if not isinstance(gradient, Mapping) or not isinstance(parity, Mapping):
        raise TypeError("expert-rank audits must be mappings")
    return {
        "outcome": decision.get("outcome"),
        "fit_only_capacity_oracle": True,
        "fixed_outer_rank": 64,
        "controlled_expert_rank_change": "16_to_64",
        ...
        "two_seed_inner_expert_rank_supported": (
            decision.get("two_seed_inner_expert_rank_supported") is True
        ),
        "expert_rank_alone_proves_only_bottleneck": False,
        "both_fail_opens_expert_count_control": (
            decision.get("expert_count_control_authorized") is True
        ),
        "descending_ladder_authorized": (
            decision.get("descending_expert_rank_ladder_authorized") is True
        ),
        "fresh_c3_authorized": False,
        "compression_claim_authorized": False,
        "speed_claim_authorized": False,
...
def _safety_contract() -> dict[str, bool]:
    return {
        ...
        "contains_loaded_width_source_final_provider_parameters": False,
        "contains_regenerated_width_final_equivalent_provider_parameters": (
            True
        ),
        "contains_regenerated_width_initial_parameter_hashes": True,
        "contains_expert16_provider_parameters": True,
        "contains_expert64_provider_parameters": True,
...
def _assert_safe_expert_rank_artifact_tree(
    value: object,
    *,
    path: str = "state",
) -> None:
    ...
    if isinstance(value, Mapping):
        for key, nested in value.items():
            ...
            if key_name in always_forbidden:
                raise ValueError(
                    f"{nested_path} is forbidden in expert-rank artifacts"
                )
            ...
            if key_name in _FIT_PAIR_TENSOR_FIELDS:
                ...
                continue
            _assert_safe_expert_rank_artifact_tree(
                nested,
                path=nested_path,
            )
...
def _validate_output_path(path: Path | str) -> Path:
    ...
    if destination.suffix != ".pt":
        raise ValueError("expert-rank output must use a .pt suffix")
    if destination.exists() or destination.with_suffix(".json").exists():
        raise FileExistsError("refusing to overwrite expert-rank output")
...
def _publish_artifact(
    ...
    *,
    ...
    _assert_safe_expert_rank_artifact_tree(state)
    ...
    if receipt is None:
        raise RuntimeError("expert-rank publication produced no receipt")
    return receipt
...
def _canonical_torch_zip_archive_end(payload: bytes) -> int:
    ...
    if len(payload) < 98 or payload[:4] != b"PK\x03\x04":
        raise ValueError("expert-rank tensor file is not a Torch ZIP archive")
    candidates: list[int] = []
    search_from = 0
    while True:
    ...
    if not candidates:
        raise ValueError("expert-rank Torch ZIP framing is invalid")
...
def _require_canonical_torch_zip_framing(payload: bytes) -> None:
    ...
    if _canonical_torch_zip_archive_end(payload) != len(payload):
        raise ValueError(
            "expert-rank tensor file contains trailing bytes"
        )
...
def _restore_ordinary_candidate(
    ...
    if not isinstance(raw, Mapping) or set(raw) != candidate_fields:
        raise ValueError("expert-rank ordinary candidate schema drifted")
    predictions_raw = raw.get("predictions")
    if not isinstance(predictions_raw, (tuple, list)):
        raise TypeError("expert-rank ordinary predictions are invalid")
    ...
    for value in predictions_raw:
        if not isinstance(value, Mapping) or set(value) != prediction_fields:
            raise ValueError(
                "expert-rank ordinary prediction schema drifted"
            )
        ...
        if (
            ...
            raise ValueError(
                "expert-rank ordinary prediction binding drifted"
            )
    ...
    if (
        ...
        raise ValueError("expert-rank ordinary candidate binding drifted")
    return candidate
...
def _recompute_ordinary_candidate_and_score(
    *,
    ...
    if (
        ...
        raise ValueError(
            "expert-rank ordinary structural replay drifted"
        )
    predictions: list[FullWidthCandidatePrediction] = []
    ...
    if (
        ...
        raise ValueError(
            "expert-rank ordinary candidate numerical replay drifted"
        )
    ...
    if (
        ...
        raise ValueError(
            "expert-rank ordinary score replay drifted: "
            f"{differing_fields}"
        )
    (
        recomputed_contrast,
    ...
    if (
        ...
        raise ValueError("expert-rank contrast replay drifted")
...
_DECISION_FIELDS = {
    ...
    "two_seed_inner_expert_rank_supported",
    "descending_expert_rank_ladder_authorized",
    "expert_count_control_authorized",
    "fresh_c3_authorized",
    "fresh_c3_after_descending_ladder_only",
...
_MANIFEST_FIELDS = {
    ...
    *_DECISION_FIELDS,
    ...
    "authenticated_width_source_loaded",
...
def _recompute_decision_from_rows(
    ...
    def pair(role: str) -> tuple[bool, str] | None:
        control = rows.get(f"fp_expert_rank.{role}.expert16")
        treatment = rows.get(f"fp_expert_rank.{role}.expert64")
        ...
        if not isinstance(control, Mapping) or not isinstance(
            ...
            raise ValueError("expert-rank paired rows are incomplete")
        ...
        if (
            ...
            raise ValueError("expert-rank treatment validity drifted")
        ...
        for name, value in (
            ...
            if not isinstance(value, Mapping):
                raise TypeError(f"expert-rank {name} must be a mapping")
        ...
        expected_flags = {
            ...
            "gradient_open_expert64": gradient.get("passed") is True,
        ...
        if (
            ...
            raise ValueError("expert-rank treatment flags drifted")
        valid = all(expected_flags.values())
        if left.get("passed") is not valid:
            raise ValueError("expert-rank validity decision drifted")
        status = _pair_status(
        ...
        if (
            ...
            raise ValueError("expert-rank pair status drifted")
        return valid, status

    primary = pair("primary")
    replication = pair("replication")
    if primary is None:
        raise ValueError("expert-rank primary pair is absent")
    primary_valid, primary_status = primary
    if replication is not None and not (
        primary_valid
        and primary_status == "expert16_fail_expert64_pass"
    ):
        raise ValueError("expert-rank replication was unauthorized")
    if not primary_valid:
        outcome = "invalid_primary_pair"
    elif primary_status != "expert16_fail_expert64_pass":
        outcome = f"primary_{primary_status}"
    elif replication is None:
        raise ValueError("authorized expert-rank replication is absent")
    elif not replication[0]:
        outcome = "invalid_replication_pair"
    elif replication[1] == "expert16_fail_expert64_pass":
        outcome = "two_seed_inner_expert_rank_support"
    else:
...
def _validate_hashed_audit(
    ...
    *,
    ...
    if not isinstance(audit, Mapping) or set(audit) != fields:
        raise ValueError(f"expert-rank {label} schema drifted")
    payload = dict(audit)
    supplied = payload.pop("artifact_sha256", None)
    if supplied != _json_sha256(payload, domain=_AUDIT_DOMAIN):
        raise ValueError(f"expert-rank {label} hash drifted")
    return audit
...
def _validate_initial_audit(
    ...
    *,
    ...
    if (
        ...
        != "fisher_graph.initial_expert_rank_equivalence"
        ...
        raise ValueError("expert-rank initial semantics drifted")
    tolerance = protocol.lift.equivalence_absolute_tolerance
    ...
    if (
        ...
        raise ValueError("expert-rank initial binding drifted")
    return state
...
def _validate_gradient_audit(
    ...
    *,
    ...
    applicable = arm == "expert64"
    expected = _gradient_audit(
        applicable=applicable,
        pair_role=pair_role,
        protocol=protocol,
        **(
            _preflight_binding_for_role(protocol, pair_role)[
                "treatment_gradient"
            ]
            if applicable
            else {
    ...
    if _canonical_json_bytes(state) != _canonical_json_bytes(expected):
        raise ValueError("expert-rank gradient binding drifted")
    return state
    ...
    *,
    ...
    if arm == "expert16":
        ...
        if _canonical_json_bytes(audit) != _canonical_json_bytes(expected):
            raise ValueError("expert-rank control parity drifted")
        return expected
    state = _validate_hashed_audit(
        audit,
        fields=_PARITY_FIELDS,
    ...
    if (
        ...
        raise ValueError("expert-rank postfit parity semantics drifted")
    tolerance = protocol.lift.equivalence_absolute_tolerance
    relative = protocol.lift.equivalence_relative_tolerance
    expected_flags = {
        "output_absolute": (
    ...
    if (
        ...
        raise ValueError("expert-rank postfit parity binding drifted")
    return state


def load_function_preserving_expert_rank_control_artifact(
    path: Path | str,
    *,
    ...
    """Strictly load, authenticate, and recompute an expert-rank result."""

    source = Path(path).expanduser().resolve()
    if source.suffix != ".pt":
        raise ValueError("expert-rank artifact must use a .pt suffix")
    tensor_payload = _read_regular(source, label="expert-rank artifact")
    report_path = source.with_suffix(".json")
    report_payload = _read_regular(report_path, label="expert-rank report")
    ...
    if tensor_sha256 != trusted_tensor:
        raise ValueError(
            "expert-rank external trust anchor mismatch: tensor file"
        )
    _require_canonical_torch_zip_framing(tensor_payload)
    raw = torch.load(
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("expert-rank tensor fields drifted")
    if _canonical_torch_payload(raw) != tensor_payload:
        raise ValueError(
            "expert-rank tensor file is not the canonical serialization "
            "of its consumed state"
        )
    if set(raw) != _STATE_FIELDS:
        raise ValueError("expert-rank tensor fields drifted")
    _assert_safe_expert_rank_artifact_tree(raw)
    manifest = raw["manifest"]
    if not isinstance(manifest, Mapping) or set(manifest) != _MANIFEST_FIELDS:
        raise ValueError("expert-rank manifest fields drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank logical binding drifted")
    try:
        report = json.loads(report_payload.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("expert-rank report is invalid JSON") from exc
    if not isinstance(report, Mapping):
        raise TypeError("expert-rank report must be a mapping")
    ...
    if (
        logical != trusted_artifact
        or computed_report_sha != trusted_report
    ):
        raise ValueError("expert-rank external trust anchor mismatch")
    ...
    if (
        ...
        raise ValueError("expert-rank report binding drifted")
    if set(report) != _MANIFEST_FIELDS | _REPORT_EXTRA_FIELDS:
        raise ValueError("expert-rank report fields drifted")
    for name, value in manifest.items():
        if _canonical_json_bytes(report.get(name)) != _canonical_json_bytes(
            value
        ):
            raise ValueError(f"expert-rank report field {name!r} drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank code binding drifted")
    protocol = FunctionPreservingExpertRankControlProtocol.from_state_dict(
        raw["protocol_state"]
    )
    if (
        protocol.protocol_sha256
        ...
        raise ValueError("expert-rank protocol binding drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank measurement evidence drifted")
    source_firewall = {
        "source_width_protocol_sha256": (
            protocol.source.width_protocol_sha256
        ),
        "source_width_code_bundle_sha256": (
            protocol.source.width_code_bundle_sha256
        ),
        "source_width_logical_artifact_sha256": (
            protocol.source.width_logical_artifact_sha256
        ),
        "source_width_tensor_file_sha256": (
            protocol.source.width_tensor_file_sha256
        ),
        "source_width_report_sha256": (
            protocol.source.width_report_sha256
        ),
        "source_width_primary_rank64_plan_sha256": (
            protocol.source.width_primary_rank64_plan_sha256
        ),
        "source_width_primary_rank64_result_sha256": (
            protocol.source.width_primary_rank64_result_sha256
        ),
    }
    if any(
        ...
        raise ValueError("expert-rank predecessor binding drifted")
    ...
    if not isinstance(metric, Tensor):
        raise TypeError("expert-rank canonical metric is not a tensor")
    ...
    if (
        ...
        raise ValueError("expert-rank manifest firewall drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank ordinary scoring batches drifted")
    ...
    if (
        ...
        raise ValueError(
            "expert-rank ordinary scoring batch order drifted"
        )
    ...
    if (
        ...
        raise ValueError("expert-rank fit contrast pairs drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank fit contrast pair order drifted")
    replay_sequences = {
        "fit_batch_sha256s": tuple(
    ...
    if any(
        ...
        raise ValueError(
            "expert-rank fit replay protocol commitment drifted"
        )
    ...
    if (
        ...
        raise ValueError("expert-rank ordinary controls replay drifted")
    ...
    if (
        ...
        raise ValueError("expert-rank candidate tables drifted")
    allowed_orders = (
        (
        ...
        (
            "fp_expert_rank.primary.expert16",
            "fp_expert_rank.primary.expert64",
            "fp_expert_rank.replication.expert16",
            "fp_expert_rank.replication.expert64",
        ),
    )
    if tuple(executed) not in allowed_orders:
        raise ValueError("expert-rank candidate order drifted")
    canonical_state = plans.get("fp_expert_rank.primary.expert16")
    if not isinstance(canonical_state, Mapping):
        raise ValueError("expert-rank canonical control is absent")
    ...
    if (
        ...
        raise ValueError("expert-rank canonical source replay drifted")
    objective_protocol = default_objective_balance_diagnostic_protocol()
    d3_recipe = objective_protocol.recipe(protocol.training.recipe_id)
    ...
    if not isinstance(report_gauge, Mapping) or not isinstance(
        ...
        raise TypeError("expert-rank balance evidence is absent")
    if (
        ...
        raise ValueError("expert-rank target geometry drifted")

    for candidate_id in executed:
        state = plans[candidate_id]
        row = rows[candidate_id]
        if not isinstance(state, Mapping) or not isinstance(row, Mapping):
            raise TypeError("expert-rank candidate entry is invalid")
        if set(row) != _ROW_FIELDS:
            raise ValueError("expert-rank candidate fields drifted")
        ...
        expected_config = (
            _executor_config(protocol.expert16_executor)
            if arm == "expert16"
            else _executor_config(protocol.expert64_executor)
        )
        seed = (
            protocol.training.primary_seed
            if role == "primary"
        ...
        if (
            ...
            or arm not in {"expert16", "expert64"}
            or candidate_id != f"fp_expert_rank.{role}.{arm}"
            ...
            raise ValueError("expert-rank candidate plan/result drifted")
        if (
            ...
            raise ValueError("expert-rank shared fit geometry drifted")
        if row.get("source_replay_exact") is not _source_replay_exact(
            ...
            raise ValueError("expert-rank source replay drifted")
        initial = _validate_initial_audit(
        ...
        paired_initial = rows.get(
            f"fp_expert_rank.{role}."
            f"{'expert64' if arm == 'expert16' else 'expert16'}"
        )
        if (
            ...
            raise ValueError("expert-rank paired initialization drifted")
        control_state = plans.get(f"fp_expert_rank.{role}.expert16")
        treatment_state = plans.get(f"fp_expert_rank.{role}.expert64")
        if not isinstance(control_state, Mapping) or not isinstance(
            treatment_state,
            Mapping,
        ):
            raise ValueError("expert-rank paired plans are absent")
        ...
        if (
            ...
            raise ValueError("expert-rank initial metrics binding drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank structural metadata drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank source sequence schema drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank source sequences drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank source sequence decision drifted")
        balance = d0d3._contribution_balance_gate(
            plan,
            recipe=d3_recipe,
            gates=objective_protocol.gates,
        ...
        if (
            ...
            raise ValueError("expert-rank final metrics replay drifted")
        if (
            ...
            raise ValueError("expert-rank objective audit drifted")
        ...
        if (
            ...
            raise ValueError(
                "expert-rank execution accounting replay drifted"
            )
        ...
        expected_canonical_execution = {
            ...
            "canonical_core_mac_count": (
                2_133_760 if arm == "expert16" else 4_499_008
            ),
        ...
        if (
            ...
            raise ValueError("expert-rank accounting/score binding drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank execution accounting drifted")
        ...
        if any(
            ...
            raise ValueError("expert-rank ordinary gates drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank contrast evidence drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank fit decision drifted")
        ...
        if (
            ...
            raise ValueError("expert-rank validity schema drifted")

    recomputed = _recompute_decision_from_rows(rows)
    for name, expected in recomputed.items():
        if manifest.get(name) != expected:
            raise ValueError("expert-rank final decision drifted")
    if (
        ...
        raise ValueError("expert-rank report safety semantics drifted")
    return LoadedFunctionPreservingExpertRankControlArtifact(
        state=raw,
        report=report,
        manifest=manifest,
...
def run_function_preserving_expert_rank_control(
    *,
    source_width_path: Path | str = DEFAULT_SOURCE_WIDTH,
    ...
    """Execute the authenticated paired fit-only expert-rank control."""
    ...
    try:
        loaded = load_function_preserving_expert_rank_control_artifact(
            destination,
            **receipt,
        )
    except BaseException:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = commands.add_parser(
        "run",
        help="execute the authenticated expert-rank control",
    )
    run.add_argument(
        "--source-width",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "describe":
        report = describe_function_preserving_expert_rank_control()
    else:
        report = run_function_preserving_expert_rank_control(
            source_width_path=args.source_width,
