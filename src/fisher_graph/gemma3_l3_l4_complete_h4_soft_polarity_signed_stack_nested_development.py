"""V20l nested signed-stack development campaign for Gemma 3.

V20l authenticates the completed V20k rollback and its V20g/V20i/V20j
lineage before model construction.  It keeps V20i's six-family masked Fisher
directions and training-only CVaR-2 reflection fit, but replaces the single
linear radius with a bounded signed stack

``q(z) = (1-abs(s)*z^2)*tanh(r*z) + s*z^2``.

The frozen fifteen-pair ladder contains a zero anchor, two matched-linear
controls, and twelve signed-stack alternatives.  All seven-by-fifteen
providers and traces are frozen before any inner-held capability is opened,
...
from . import complete_h4_fisher_soft_polarity_signed_stack_fit as _signed_stack_fit
...
__all__ = [
    ...
    "build_parser",
    "main",
    "run_gemma3_l3_l4_complete_h4_soft_polarity_signed_stack_nested_development",
]


_LOCAL_ROOT = Path(".local-runs/google--gemma-3-270m")
DEFAULT_OUTPUT = _LOCAL_ROOT / (
    "modal-generator-l3-l4-complete-h4-soft-polarity-signed-stack-nested-"
    "r16-k256-a-fit16-dev-v20l.json"
)
...
_SCHEMA = (
    "fisher_graph.gemma3_l3_l4.complete_h4_soft_polarity_signed_stack_nested.v20l"
)
_FOLD_SCHEMA = (
    "fisher_graph.complete_h4_soft_polarity_signed_stack_nested_outer_fold.v20l"
)
_FORMAT_VERSION = 28
_REPORT_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-nested-report:v20l\0"
_SOURCE_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-nested-source:v20l\0"
_FOLD_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-nested-fold:v20l\0"
_INNER_FIT_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-inner-fit:v20l\0"
_INNER_MANIFEST_DOMAIN = (
    b"fisher-graph:soft-polarity-signed_stack-inner-manifest:v20l\0"
)
_INNER_EXECUTION_DOMAIN = (
    b"fisher-graph:soft-polarity-signed_stack-inner-execution:v20l\0"
)
_RESPONSE_SELECTION_DOMAIN = (
    b"fisher-graph:soft-polarity-signed_stack-response-selection:v20l\0"
)
_OUTER_MANIFEST_DOMAIN = (
    b"fisher-graph:soft-polarity-signed_stack-outer-manifest:v20l\0"
)
_OUTER_EXECUTION_DOMAIN = (
    b"fisher-graph:soft-polarity-signed_stack-outer-execution:v20l\0"
)
_PROVIDER_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-provider:v20l\0"
_TRACE_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-trace:v20l\0"
_DECISION_DOMAIN = b"fisher-graph:soft-polarity-signed_stack-decision:v20l\0"
...
_RESPONSE_KEYS = tuple(
    f"radius={radius.hex()};signed_mix={signed_mix.hex()}" for radius, signed_mix in _RESPONSES
)
if _RESPONSES != tuple(_signed_stack_fit.SOFT_POLARITY_SIGNED_STACK_LADDER):
    raise RuntimeError("V20l runner response ladder differs from core protocol")
_SIGNED_STACK_LADDER_RECEIPT = (
    _signed_stack_fit.build_soft_polarity_signed_stack_ladder_receipt()
)
_SIGNED_STACK_LADDER_RECEIPT_SHA256 = str(
    _SIGNED_STACK_LADDER_RECEIPT["artifact_sha256"]
)
_ARMS = (
    "base",
    "fixed_plus",
    ...
    "same_stack_unreflected",
    "signed_stack_reflected",
    "signed_stack_reflected_exact_mirror",
)
_PRIMARY_ARM = "signed_stack_reflected"
...
_FIXED_PROTOCOL: dict[str, object] = {
    "protocol": "v20l_nested_reflection_and_signed_convex_stack_fit",
    "scientific_status": (
        "posthoc_after_v20k_reused_A16_development_hypothesis_only"
    ),
    "source": (
        "pinned_V20k_report_and_folds_plus_V20j_and_V20i_lineage_and_"
    ...
    "inner_validation": (
        ...
        "signed_stack_folds_on_one_fixed_seven_family_endpoint"
    ),
    ...
    "signed_stack_ladder_receipt_sha256": _SIGNED_STACK_LADDER_RECEIPT_SHA256,
    "signed_stack_ladder_receipt_constructed_before_provider_freeze": True,
    "response_selection": (
        "family_equal_mean_of_seven_inner_OOF_token_mean_exact_float64_"
        "full_vocabulary_KL_teacher_to_candidate_then_smaller_abs_signed_mix_then_"
        "smaller_radius_then_fixed_index_then_artifact_sha256"
    ),
    "signed_stack_formula": (
        "one_minus_abs_signed_mix_times_z_squared_times_tanh_radius_z_"
        "plus_signed_mix_times_z_squared"
    ),
    "signed_stack_constraints": (
        "radius_in_zero_one_fourth_signed_mix_in_closed_negative_half_"
        "positive_half_finite_bounded_on_normalized_box"
    ),
    "inner_freeze_barrier": (
        "all_seven_times_fifteen_providers_and_traces_before_any_inner_capability"
    ),
    "outer_arms": _ARMS,
    ...
    "mechanism_gate": (
        "candidate_macro_below_same_stack_unreflected_with_five_wins_"
    ...
    "signed_stack_evidence_gate": (
        "at_least_five_outer_folds_select_nonzero_signed_mix"
    ),
...
_TRANSFER_PROTOCOL_SHA256 = _v14._sha256(
    {
        ...
        "signed_stack_fit_protocol_sha256": (
            _signed_stack_fit.SOFT_POLARITY_SIGNED_STACK_FIT_PROTOCOL_SHA256
        ),
        "operation": "V20l_domain_separated_signed_stack_provider_materialization",
...
def _response_pair(value: object) -> tuple[float, float]:
    raw = _sequence(value, label="V20l response pair")
    if len(raw) != 2:
        raise ValueError("V20l response must contain exactly radius and signed_mix")
    if any(type(item) not in (int, float) for item in raw):
        raise ValueError("V20l response values must be JSON numbers")
    ...
    if (
        ...
        raise ValueError("V20l response is outside the fixed ladder")
    return selected[0], selected[1]


def _response_key(value: object) -> str:
    radius, signed_mix = _response_pair(value)
    return f"radius={radius.hex()};signed_mix={signed_mix.hex()}"


def _response_order(value: object) -> tuple[tuple[float, float], ...]:
    return tuple(
        _response_pair(item)
        for item in _sequence(value, label="V20l response order")
    )
...
def _validate_v20i_reflection_lineage(
    *,
    ...
    inherited_outer = _mapping(
        ...
        label="V20l inherited V20i outer reflection fit",
    )
    if _v14._canonical_json_bytes(outer_reflection_fit) != (
        _v14._canonical_json_bytes(inherited_outer)
    ):
        raise ValueError("V20l outer reflection lineage differs from pinned V20i")

    current_inner = _mapping(
        inner_receipt.get("inner_evidence_by_family"),
        label="V20l current inner reflection evidence",
    )
    inherited_inner_receipt = _mapping(
        authenticated_v20i_fold.get("inner_receipt"),
        label="V20l inherited V20i inner receipt",
    )
    inherited_inner = _mapping(
        inherited_inner_receipt.get("inner_evidence_by_family"),
        label="V20l inherited V20i inner reflection evidence",
    )
    if set(current_inner) != set(inherited_inner):
        raise ValueError("V20l inner reflection family lineage differs from V20i")
    for family in sorted(current_inner):
        current = _mapping(
            current_inner[family], label="V20l current inner reflection family"
        )
        inherited = _mapping(
            inherited_inner[family], label="V20l inherited inner reflection family"
        )
        for field in ("masked_direction_receipt", "reflection_fit_receipt"):
            if _v14._canonical_json_bytes(current.get(field)) != (
                ...
                raise ValueError(
                    f"V20l {field} lineage differs from pinned V20i"
                )
...
def _validate_output(path: Path | str) -> Path:
    ...
    if destination in protected:
        raise ValueError("V20l output must preserve immutable prerequisite artifacts")
    if destination.parent != local_root:
        raise ValueError("V20l output must remain directly under .local-runs")
    return destination
...
def _load_prerequisites() -> tuple[
    ...
    """Authenticate V20k and its complete lineage before construction."""
    ...
    (
        ...
        v20k_source,
    ) = _v20k._load_prerequisites()
    panel_receipt = dict(
        _mapping(
            prerequisite.get("nested_panel_receipt"),
            label="V20l inherited panel receipt",
        )
    )
    bridge_binding = _sha(
        prerequisite.get("authenticated_bridge_binding_sha256"),
        label="V20l inherited bridge binding",
    )
    if _v14._file_sha256(_V20K_OUTPUT) != _V20K_FILE_SHA256:
        raise RuntimeError("pinned V20k report file hash drifted")
    v20k_report = _v20k._load_existing_report(
        _V20K_OUTPUT,
        source=v20k_source,
    ...
    observed_fold_hashes = {
        _identifier(family, label="V20l V20k fold family"): _sha(
            value, label="V20l V20k fold hash"
        )
        for family, value in _mapping(
            v20k_report.get("fold_fragment_sha256s_by_family"),
            label="V20l V20k fold hashes",
        ).items()
    }
    if (
        v20k_report.get("report_sha256") != _V20K_LOGICAL_SHA256
        or v20k_report.get("classification")
        != "soft_polarity_log_response_nested_oof_failed_rollback_to_base"
        or v20k_report.get("development_oof_passed") is not False
        or v20k_report.get("primary_development_gate_passed") is not False
        or v20k_report.get("mechanism_gate_passed") is not False
        or _mapping(
            v20k_report.get("decision"), label="V20l V20k decision"
        ).get("integrity_passed")
        is not True
        or v20k_report.get("passed") is not False
        or v20k_report.get("rollback_to_base") is not True
        or v20k_report.get("final_refit") is not None
        or v20k_report.get("calibration_b_opened") is not False
        or _mapping(
            v20k_report.get("source_receipt"), label="V20l V20k source"
        ).get("artifact_sha256")
        != _V20K_SOURCE_SHA256
        or observed_fold_hashes != _V20K_FOLD_SHA256S
    ):
        raise RuntimeError("pinned V20k development authority differs")

    families = tuple(sorted(_V20K_FOLD_SHA256S))
    authenticated_v20k_folds = {
        family: _v20k._load_fold_fragment(
            output=_V20K_OUTPUT,
            source=v20k_source,
            panel_receipt=panel_receipt,
    ...
    if {
        ...
        for family, fragment in authenticated_v20k_folds.items()
    } != _V20K_FOLD_SHA256S:
        raise RuntimeError("pinned V20k fold authority differs")
    source = _hashed(
        {
            ...
            "v20k_report_sha256": _V20K_LOGICAL_SHA256,
            "v20k_file_sha256": _V20K_FILE_SHA256,
            "v20k_source_receipt_sha256": _V20K_SOURCE_SHA256,
            "v20k_fold_fragment_sha256s_by_family": dict(
                sorted(_V20K_FOLD_SHA256S.items())
            ),
            "reflection_fit_protocol_sha256": (
                _reflection.SOFT_POLARITY_REFLECTION_FIT_PROTOCOL_SHA256
            ),
...
def _selected_direction(
    ...
    if reflection_fit.get("selected_variant_available") is not True:
        raise RuntimeError("V20l reflection fit has no admissible direction")
    raw = tuple(
        float(item)
        for item in _sequence(
            reflection_fit.get("selected_normalized_direction"),
            label="V20l selected reflection direction",
        )
    )
    if len(raw) != 4 or not all(math.isfinite(item) for item in raw):
        raise RuntimeError("V20l reflection direction is not a finite four-vector")
...
def _unreflected_direction(
    ...
    raw = tuple(
        ...
        for item in _sequence(
            ...
            label="V20l unreflected direction",
        )
    )
    if len(raw) != 4 or not all(math.isfinite(item) for item in raw):
        raise RuntimeError("V20l unreflected direction is not finite")
    return raw  # type: ignore[return-value]
...
def _provider_seed(
    *,
    ...
    radius, signed_mix = _response_pair(response)
    ...
    return _v14._sha256(
        {
            ...
            "endpoint_receipt_sha256": _sha(
                endpoint_receipt_sha256, label="V20l provider endpoint"
            ),
            "direction_artifact_sha256": _sha(
                direction_artifact_sha256, label="V20l provider direction"
            ),
            "reflection_fit_sha256": _sha(
                reflection_fit_sha256, label="V20l provider reflection fit"
            ),
            "response": (radius, signed_mix),
            "response_key": _response_key((radius, signed_mix)),
            "direction": selected_direction,
            "direction_box_corner_scores": _box_corner_scores(selected_direction),
            "outer_held_family_id": _identifier(
                outer_family_id, label="V20l provider outer family"
            ),
...
def _materialize_provider(
    ...
    *,
    ...
    inner_family_id: str | None,
    role: str,
) -> tuple[AutonomousCompleteH4FisherSoftPolaritySignedStackProvider, str]:
    radius, signed_mix = _response_pair(response)
    seed = _provider_seed(
    ...
    *,
    ...
    if isinstance(
        ...
        if response is None or direction is None:
            raise ValueError(
                "V20l signed_stack provider receipt needs response and direction"
            )
        radius, signed_mix = _response_pair(response)
        expected = tuple(float(item) for item in direction)
        if selected_direction != expected:
            raise RuntimeError("V20l provider differs from its frozen direction")
        if (
            float(provider.radius) != radius
            or float(provider.signed_mix) != signed_mix
        ):
            raise RuntimeError("V20l provider coefficients differ from response")
        bound = max(abs(item) for item in corners)
        if abs(bound - 1.0) > 1.0e-12:
            raise RuntimeError("V20l provider direction is not box normalized")
    payload = {
        "role": role,
        "response": response,
        "response_key": (
        ...
        "box_certificate": (
            ...
            else fisher_soft_polarity_signed_stack_box_certificate(
                provider.direction,
                radius=float(provider.radius),
                signed_mix=float(provider.signed_mix),
            )
        ),
        "provider_artifact_sha256": _sha(
            provider.artifact_sha256, label=f"V20l {role} provider artifact"
        ),
...
def _expected_provider_accounting_from_v20i(
    ...
    manifest = _mapping(
        ...
        label="V20l inherited V20i provider manifest",
    )
    receipts = _mapping(
        manifest.get("provider_receipts"),
        label="V20l inherited V20i provider receipts",
    )
    reference_role = {
        "base": "base",
    ...
    authority = _mapping(
        ...
        label=f"V20l inherited V20i {reference_role} provider receipt",
    )
    ...
    if role not in ("base", "fixed_plus", "fixed_minus"):
        ...
        # SignedStack stores four direction values plus radius and signed_mix,
        # and adds one dense projection MAC.  The signed stack itself remains
...
def _validate_provider_receipt_evidence(
    ...
    *,
    ...
    if set(receipt) != _PROVIDER_RECEIPT_KEYS:
        raise ValueError("V20l provider receipt key set differs")
    provider_artifact = _sha(
        expected_provider_artifact_sha256,
        label="V20l expected provider artifact",
    )
    if (
        ...
        raise ValueError("V20l provider receipt identity differs")

    metadata = _mapping(
        receipt.get("provider_metadata"), label="V20l provider metadata"
    )
    metadata_sha = _sha(
        receipt.get("provider_metadata_sha256"),
        label="V20l provider metadata hash",
    )
    if (
        _v14._sha256(metadata, domain=_PROVIDER_DOMAIN) != metadata_sha
        or metadata.get("artifact_sha256") != provider_artifact
    ):
        raise ValueError("V20l provider metadata authentication differs")

    receipt_accounting = tuple(
        _strict_receipt_integer(receipt, key, label="V20l provider receipt")
        for key in (
            "rank",
            "conditional_rank",
            "prepared_float_scalar_count",
    ...
    metadata_accounting = tuple(
        _strict_receipt_integer(metadata, key, label="V20l provider metadata")
        for key in (
            "rank",
            "conditional_rank",
            "prepared_float_scalar_count",
    ...
    if (
        ...
        raise ValueError("V20l provider accounting differs from pinned V20i")

    if metadata.get("bridge_binding_sha256") not in (
        None,
        expected_bridge_binding_sha256,
    ):
        raise ValueError("V20l provider bridge metadata differs")

    signed_stack_role = expected_response is not None or expected_direction is not None
    if signed_stack_role:
        if expected_response is None or expected_direction is None:
            raise ValueError("V20l signed_stack provider expectation is incomplete")
        ...
        if len(direction) != 4 or not all(math.isfinite(item) for item in direction):
            raise ValueError("V20l expected signed_stack direction differs")
        payload = _mapping(
            receipt.get("provider_payload"), label="V20l signed_stack provider payload"
        )
        ...
        if _v14._canonical_json_bytes(
            ...
            raise ValueError("V20l signed_stack provider box certificate differs")
        expected_direction_sha = fisher_soft_polarity_signed_stack_direction_sha256(
            _v20g._eta_tensor(direction)
        )
        endpoint_base = _sha(
            expected_endpoint_receipt.get("base_provider_artifact_sha256"),
            label="V20l endpoint base provider",
        )
        endpoint_proposal = _sha(
            expected_endpoint_receipt.get("proposal_provider_artifact_sha256"),
            label="V20l endpoint proposal provider",
        )
        expected_transfer = _sha(
            expected_transfer_evidence_sha256,
            label="V20l signed_stack transfer evidence",
        )
        expected_bindings = {
            ...
            "radius": response[0],
            "signed_mix": response[1],
        }
        for key, expected in expected_bindings.items():
            if validated.payload.get(key) != expected:
                raise ValueError(f"V20l signed_stack provider {key} differs")
        for key in (
        ...
        if validated.artifact_sha256 != provider_artifact:
            raise ValueError("V20l signed_stack provider artifact replay differs")
    else:
        if receipt.get("provider_payload") is not None:
            raise ValueError("V20l non-signed_stack provider serialized a payload")
        if expected_role in ("fixed_plus", "fixed_minus"):
            expected_transfer = _sha(
                expected_transfer_evidence_sha256,
                label="V20l fixed-control transfer evidence",
            )
            expected_bindings = {
                "base_provider_artifact_sha256": _sha(
                    ...
                    label="V20l fixed-control endpoint base",
                ),
                "proposal_provider_artifact_sha256": _sha(
                    expected_endpoint_receipt.get(
                        "proposal_provider_artifact_sha256"
                    ),
                    label="V20l fixed-control endpoint proposal",
                ),
                "transfer_protocol_sha256": _TRANSFER_PROTOCOL_SHA256,
                "transfer_evidence_sha256": expected_transfer,
            }
            for key, expected in expected_bindings.items():
                if metadata.get(key) != expected:
                    raise ValueError(f"V20l fixed-control {key} differs")
...
def _freeze_inner_providers(
    ...
    *,
    ...
    """Freeze all 7x15 inner providers and traces before any capability."""

    outer = _identifier(outer_family_id, label="V20l inner outer family")
    ...
    if (
        ...
        raise RuntimeError("V20l inner family geometry differs")
    ...
    for inner in training_families:
        ...
        selected_artifact = _sha(
            ...
            label="V20l inner selected reflection variant",
        )
        held_records = tuple(
            record for record in ordered if record.sequence.family_id == inner
        )
        if len(held_records) != _PROMPTS_PER_FAMILY:
            raise RuntimeError("V20l inner-held prompt geometry differs")
        ...
        for response in _RESPONSES:
            ...
            radius, signed_mix = response
            traces[inner][response] = _provider_trace(
                provider,
                held_records,
                role=(
                    f"inner_{inner}_radius_{radius.hex()}_signed_mix_{signed_mix.hex()}"
                ),
            )
    ...
    if len(flat_hashes) != _INNER_FAMILY_COUNT * len(_RESPONSES) or len(
        ...
        raise RuntimeError("V20l inner provider artifacts are not all distinct")
    manifest = _hashed(
        {
            "outer_held_family_id": outer,
            "inner_family_order": training_families,
            "response_order": _RESPONSES,
            "signed_stack_ladder_receipt_sha256": (
                _SIGNED_STACK_LADDER_RECEIPT_SHA256
            ),
            ...
            "all_seven_times_fifteen_providers_frozen_before_any_inner_capability": True,
            "all_seven_times_fifteen_traces_frozen_before_any_inner_capability": True,
...
def _aggregate_response_selection(
    ...
    if len(families) != _INNER_FAMILY_COUNT:
        raise ValueError("V20l response selection requires seven inner OOF families")
    outer_ids = {
        _identifier(
            inner_evidence_by_family[family].get("outer_held_family_id"),
            label="V20l response selection outer family",
        )
        for family in families
    }
    if len(outer_ids) != 1 or next(iter(outer_ids)) in families:
        raise ValueError("V20l response selection outer family geometry differs")
    ...
    for family in families:
        raw = _mapping(
            ...
            label="V20l inner objective ladder",
        )
        if set(raw) != set(_RESPONSE_KEYS):
            raise ValueError("V20l inner objective response geometry differs")
    ...
    ladder_receipt = dict(_SIGNED_STACK_LADDER_RECEIPT)
    exact_by_family_and_candidate = {
        family: {
            ...
            for candidate_id, response in zip(
                _signed_stack_fit.SOFT_POLARITY_SIGNED_STACK_CANDIDATE_IDS,
                _RESPONSES,
                strict=True,
            )
        }
    ...
    core_selection = (
        _signed_stack_fit.build_soft_polarity_signed_stack_inner_oof_selection_receipt(
    ...
    selected = (
        ...
        float(core_selection["selected_s"]),
    )
    return _hashed(
        {
            "inner_family_order": families,
            ...
            "signed_stack_ladder_receipt": ladder_receipt,
            "signed_stack_selection_receipt": core_selection,
            "selection_rule": (
                "minimum_inner_OOF_family_equal_token_mean_exact_float64_full_"
                "vocabulary_KL_teacher_to_candidate_then_smaller_abs_signed_mix_then_"
...
def _fit_inner_response(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20l inner-response outer family")
    ...
    if (
        manifest.get(
            "all_seven_times_fifteen_providers_frozen_before_any_inner_capability"
        )
        is not True
        or manifest.get(
            "all_seven_times_fifteen_traces_frozen_before_any_inner_capability"
        )
        is not True
    ...
    for inner in tuple(manifest["inner_family_order"]):
        ...
        for response in _RESPONSES:
            ...
            if set(family_scores) != {inner}:
                raise RuntimeError("V20l inner score family geometry differs")
        ...
        if not zero_anchor:
            raise RuntimeError("V20l inner eta-zero output anchor differs from V20g")
...
def _freeze_outer_providers(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20l outer provider family")
    ...
    reflected_provider, reflected_seed = _materialize_provider(
        ...
        role="outer_signed_stack_reflected",
    )
    unreflected_provider, unreflected_seed = _materialize_provider(
        ...
        outer_family_id=outer,
        inner_family_id=None,
        role="outer_same_stack_unreflected",
    )
    mirror_provider, mirror_seed = _materialize_provider(
        ...
        outer_family_id=outer,
        inner_family_id=None,
        role="outer_signed_stack_reflected_exact_mirror",
    )
    ...
    providers: dict[str, object] = {
        ...
        "same_stack_unreflected": unreflected_provider,
        "signed_stack_reflected": reflected_provider,
        "signed_stack_reflected_exact_mirror": mirror_provider,
    }
    if tuple(providers) != _ARMS or len(
        {provider.artifact_sha256 for provider in providers.values()}
    ) != len(_ARMS):
        raise RuntimeError("V20l outer provider arm artifacts are not distinct")
    ...
    receipts = {
        ...
        "same_stack_unreflected": _provider_receipt(
            providers["same_stack_unreflected"],
            role="same_stack_unreflected",
            response=response,
            direction=unreflected,
        ),
        "signed_stack_reflected": _provider_receipt(
            providers["signed_stack_reflected"],
            role="signed_stack_reflected",
            response=response,
            direction=reflected,
        ),
        "signed_stack_reflected_exact_mirror": _provider_receipt(
            providers["signed_stack_reflected_exact_mirror"],
            role="signed_stack_reflected_exact_mirror",
            response=response,
            direction=mirror,
        ),
    }
    ...
    manifest = _hashed(
        {
            ...
            "soft_provider_transfer_evidence_sha256s": {
                ...
                "same_stack_unreflected": unreflected_seed,
                "signed_stack_reflected": reflected_seed,
                "signed_stack_reflected_exact_mirror": mirror_seed,
            },
...
def _score_outer_arms(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20l held outer family")
    held = _v20b._ordered_records(
        tuple(record for record in records if record.sequence.family_id == outer)
    )
    if len(held) != _PROMPTS_PER_FAMILY:
        raise RuntimeError("V20l outer-held prompt geometry differs")
    ...
    if (
        ...
        raise PermissionError("V20l outer freeze barrier is not satisfied")
    ...
    for arm in _ARMS:
        ...
        if set(family_scores) != {outer}:
            raise RuntimeError("V20l outer score family geometry differs")
    ...
    _v20b._validate_capability_receipt(
        ...
        label="V20l outer-held capability",
    )
    inherited_arms = _mapping(
        _mapping(
            authenticated_v20g_fold.get("held_evidence"),
            label="V20l inherited V20g held evidence",
        ).get("arm_evidence"),
        label="V20l inherited V20g held arms",
    )
    control_anchors: dict[str, bool] = {}
    for arm in ("base", "fixed_plus", "fixed_minus"):
        inherited = _mapping(
            inherited_arms.get(arm), label=f"V20l inherited V20g {arm} arm"
        )
    ...
    fold_receipt = _hashed(
        {
            ...
            "selected_signed_mix_nonzero": (
                _response_pair(selected_response)[1] != 0.0
            ),
...
def _execute_outer_fold(
    ...
    *,
    ...
    outer = _identifier(outer_family_id, label="V20l outer family")
    ...
    inherited_endpoint = _mapping(
        ...
        label="V20l inherited endpoint receipt",
    )
    inherited_evidence = _mapping(
        authenticated_v20g_fold.get("endpoint_evidence"),
        label="V20l inherited endpoint evidence",
    )
    if (
        ...
        raise RuntimeError("V20l reconstructed endpoint differs from pinned V20g")
    fit = _mapping(
        authenticated_v20g_fold.get("fit_receipt"),
        label="V20l inherited V20g fit receipt",
    )
    source_direction = _mapping(
        fit.get("direction_receipt"), label="V20l inherited V20g direction"
    )
    _v20g._core.validate_soft_polarity_direction_receipt(source_direction)
    if source_direction.get("held_family_id") != outer:
        raise RuntimeError("V20l inherited direction held family differs")
...
def _validate_inner_receipt(
    ...
    *,
    ...
    receipt = _validate_hashed(
        value, domain=_INNER_FIT_DOMAIN, label="V20l inner receipt"
    )
    ...
    if (
        ...
        raise ValueError("V20l inner receipt boundary differs")
    manifest = _validate_hashed(
        _mapping(
            receipt.get("inner_provider_manifest"),
            label="V20l inner provider manifest",
        ),
        domain=_INNER_MANIFEST_DOMAIN,
        label="V20l inner provider manifest",
    )
    families = tuple(
        _identifier(item, label="V20l inner family")
        for item in _sequence(
            manifest.get("inner_family_order"), label="V20l inner family order"
        )
    )
    source_families = tuple(
        _identifier(item, label="V20l source training family")
        for item in _sequence(
            source_direction.get("training_family_ids"),
            label="V20l source training families",
        )
    )
    training_ids_by_family = _mapping(
        source_direction.get("training_example_ids_by_family"),
        label="V20l source training example ids",
    )
    if (
        ...
        or manifest.get("signed_stack_ladder_receipt_sha256")
        != _SIGNED_STACK_LADDER_RECEIPT_SHA256
    ...
    if any(
        ...
        raise ValueError("V20l inner manifest family bindings differ")
    gradient_evidence = _mapping(
        _mapping(
            authenticated_v20g_fold.get("fit_training_evidence"),
            label="V20l inherited fit evidence",
    ...
    if set(inherited_zero_objectives) != family_set:
        raise ValueError("V20l inherited eta-zero family geometry differs")
    raw_inner = _mapping(
        receipt.get("inner_evidence_by_family"),
        label="V20l inner evidence map",
    )
    if set(raw_inner) != set(families):
        raise ValueError("V20l inner evidence family geometry differs")
    validated_inner: dict[str, Mapping[str, object]] = {}
    for family in families:
        evidence = _validate_hashed(
            _mapping(raw_inner[family], label="V20l inner family evidence"),
            domain=_INNER_EXECUTION_DOMAIN,
            label="V20l inner family evidence",
        )
        masked = _mapping(
            evidence.get("masked_direction_receipt"),
            label="V20l masked direction receipt",
        )
        ...
        reflection_fit = _mapping(
            ...
            label="V20l inner reflection fit",
        )
        ...
        expected_examples = tuple(
            _identifier(item, label="V20l inner expected example")
            for item in _sequence(
                training_ids_by_family.get(family),
                label="V20l inner expected examples",
            )
        )
        objectives = _mapping(
            evidence.get("objective_by_response"),
            label="V20l inner objectives",
        )
        response_evidence = _mapping(
            evidence.get("response_evidence"),
            label="V20l inner response evidence",
        )
        if (
            ...
            raise ValueError("V20l inner evidence schedule differs")
        family_provider_hashes = _mapping(
            provider_hashes[family], label="V20l inner family provider hashes"
        )
        family_provider_receipts = _mapping(
            provider_receipts[family], label="V20l inner family provider receipts"
        )
        family_transfer_hashes = _mapping(
            transfer_hashes[family],
            label="V20l inner family provider transfer hashes",
        )
        family_trace_hashes = _mapping(
            trace_hashes[family], label="V20l inner family trace hashes"
        )
        if any(
            ...
            raise ValueError("V20l inner manifest response bindings differ")
        trace_bundle_sha = _v14._sha256(
            {
        ...
        for response in _RESPONSES:
            ...
            arm = _validate_hashed(
                _mapping(
                    response_evidence[key], label="V20l inner response arm evidence"
                ),
                domain=_INNER_EXECUTION_DOMAIN,
                label="V20l inner response arm evidence",
            )
            trace = _validate_hashed(
                _mapping(arm.get("response_trace"), label="V20l inner trace"),
                domain=_TRACE_DOMAIN,
                label="V20l inner trace",
            )
            provider_receipt = _validate_hashed(
                _mapping(
                    family_provider_receipts[key],
                    label="V20l inner provider receipt",
                ),
                domain=_PROVIDER_DOMAIN,
                label="V20l inner provider receipt",
            )
            score_bundle = _validate_exact_score_bundle(
                ...
                label=f"V20l inner {family} response {key}",
            )
            arm_objectives, arm_h4, arm_logits, arm_executions = score_bundle
            provider_artifact = _sha(
                family_provider_hashes[key], label="V20l inner provider hash"
            )
            transfer_artifact = _sha(
                family_transfer_hashes[key],
                label="V20l inner provider transfer hash",
            )
            ...
            trace_artifact = _sha(
                family_trace_hashes[key], label="V20l inner trace hash"
            )
            seed = _v14._sha256(
                {
                    "inner_manifest_sha256": manifest["artifact_sha256"],
            ...
            response_gain_hashes = _mapping(
                ...
                label="V20l inner response gain hashes",
            )
            for value in response_gain_hashes.values():
                _sha(value, label="V20l inner response gain hash")
            expected_corners = _box_corner_scores(selected_direction)
            expected_certificate = fisher_soft_polarity_signed_stack_box_certificate(
                _v20g._eta_tensor(selected_direction),
                radius=response[0],
                signed_mix=response[1],
            )
            expected_trace_arm = (
                f"inner_{family}_radius_{response[0].hex()}_signed_mix_{response[1].hex()}"
            )
            if (
                ...
                != response[0]
                or float(
                    provider_receipt.get("signed_mix", math.nan)
                )
                != response[1]
                ...
                or _v14._canonical_json_bytes(
                    _mapping(
                        ...
                        label="V20l signed_stack box certificate",
                    )
                )
                ...
                raise ValueError("V20l inner response evidence differs")
        _v20b._validate_capability_receipt(
            ...
            expected_accesses_per_example=len(_RESPONSES),
            label="V20l resumed inner-held capability",
        )
        zero = _mapping(
            response_evidence[_response_key((0.0, 0.0))],
            label="V20l inner zero-response evidence",
        )
        expected_zero_objectives = {
            _identifier(example, label="V20l inherited zero objective example"): float(
                objective
            )
            for example, objective in _mapping(
                inherited_zero_objectives[family],
                label="V20l inherited family zero objectives",
            ).items()
        }
        zero_anchor = (
            dict(
        ...
        if (
            ...
            raise ValueError("V20l inner zero-response V20g output anchor differs")
    ...
    persisted_selection = _mapping(
        ...
        label="V20l persisted response selection",
    )
    if _v14._canonical_json_bytes(selection) != _v14._canonical_json_bytes(
        persisted_selection
    ):
        raise ValueError("V20l inner response selection replay differs")
    return receipt
...
def _validate_fold_fragment(
    ...
    *,
    ...
    fragment = _mapping(value, label="V20l fold fragment")
    if set(fragment) != _FOLD_FRAGMENT_KEYS:
        raise ValueError("V20l fold fragment key set differs")
    outer = _identifier(outer_family_id, label="V20l fold outer family")
    expected_header = {
        "schema": _FOLD_SCHEMA,
        "format_version": _FORMAT_VERSION,
        "target_output": _validate_output(output).as_posix(),
    ...
    if any(fragment.get(key) != expected for key, expected in expected_header.items()):
        raise ValueError("V20l fold fragment header differs")
    if (
        ...
        raise ValueError("V20l fold scalar-only boundary differs")
    for key in ("endpoint_receipt", "endpoint_evidence"):
        if _v14._canonical_json_bytes(fragment.get(key)) != _v14._canonical_json_bytes(
            ...
            raise ValueError("V20l fold endpoint lineage differs")

    fit = _mapping(
        authenticated_v20g_fold.get("fit_receipt"),
        label="V20l validation V20g fit",
    )
    source_direction = _mapping(
        fit.get("direction_receipt"), label="V20l validation source direction"
    )
    inner = _validate_inner_receipt(
        _mapping(fragment.get("inner_receipt"), label="V20l inner receipt"),
    ...
    if _v14._canonical_json_bytes(response_selection) != _v14._canonical_json_bytes(
        ...
        raise ValueError("V20l duplicated response selection differs")
    ...
    manifest = _validate_hashed(
        _mapping(
            fragment.get("provider_manifest"), label="V20l outer manifest"
        ),
        domain=_OUTER_MANIFEST_DOMAIN,
        label="V20l outer manifest",
    )
    endpoint_receipt = _mapping(
        authenticated_v20g_fold.get("endpoint_receipt"),
        label="V20l outer inherited endpoint receipt",
    )
    if (
        ...
        raise ValueError("V20l outer provider manifest differs")
    provider_hashes = _mapping(
        manifest.get("provider_artifact_sha256s"),
        label="V20l outer provider hashes",
    )
    provider_receipts = _mapping(
        manifest.get("provider_receipts"),
        label="V20l outer provider receipts",
    )
    trace_hashes = _mapping(
        manifest.get("response_trace_sha256s"),
        label="V20l outer trace hashes",
    )
    soft_transfer_hashes = _mapping(
        manifest.get("soft_provider_transfer_evidence_sha256s"),
        label="V20l outer soft provider transfer hashes",
    )
    ...
    if not all(arm in inherited_arms for arm in ("base", "fixed_plus", "fixed_minus")):
        raise ValueError("V20l inherited V20g control arms differ")
    expected_examples = tuple(
        sorted(
            _identifier(example, label="V20l outer expected example")
            for example in _mapping(
                _mapping(
                    inherited_arms["base"],
                    label="V20l inherited V20g base arm",
                ).get("objectives_by_example"),
                label="V20l inherited V20g base objectives",
            )
        )
    )
    held = _validate_hashed(
        _mapping(fragment.get("held_evidence"), label="V20l held evidence"),
        domain=_OUTER_EXECUTION_DOMAIN,
        label="V20l held evidence",
    )
    arms = _mapping(held.get("arm_evidence"), label="V20l held arm evidence")
    if (
        set(arms) != set(_ARMS)
        ...
        raise ValueError("V20l outer held schedule differs")
    objectives: dict[str, float] = {}
    exact_outputs: dict[
    ...
    expected_soft_transfers = {
        ...
        "same_stack_unreflected": _provider_seed(
            endpoint_receipt_sha256=str(manifest["endpoint_receipt_sha256"]),
        ...
        "signed_stack_reflected": _provider_seed(
            endpoint_receipt_sha256=str(manifest["endpoint_receipt_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
        ...
        "signed_stack_reflected_exact_mirror": _provider_seed(
            endpoint_receipt_sha256=str(manifest["endpoint_receipt_sha256"]),
            direction_artifact_sha256=str(
                reflection_fit["selected_variant_artifact_sha256"]
            ),
    ...
    expected_soft_directions = {
        ...
        "same_stack_unreflected": unreflected_direction,
        "signed_stack_reflected": selected_direction,
        "signed_stack_reflected_exact_mirror": mirror_direction,
    }
    expected_soft_responses = {
        "matched_linear_reflected": matched_linear_response,
        "same_stack_unreflected": selected_response,
        "signed_stack_reflected": selected_response,
        "signed_stack_reflected_exact_mirror": selected_response,
    }
    ...
    if (
        {
            arm: _sha(
                ...
                label=f"V20l {arm} soft transfer hash",
            )
        ...
        raise ValueError("V20l outer provider transfer lineage differs")
    health_by_arm: dict[str, bool] = {}
    for arm in _ARMS:
        evidence = _validate_hashed(
            _mapping(arms[arm], label=f"V20l {arm} arm evidence"),
            domain=_OUTER_EXECUTION_DOMAIN,
            label=f"V20l {arm} arm evidence",
        )
        trace = _validate_hashed(
            _mapping(evidence.get("response_trace"), label=f"V20l {arm} trace"),
            domain=_TRACE_DOMAIN,
            label=f"V20l {arm} trace",
        )
        provider_receipt = _validate_hashed(
            _mapping(
                provider_receipts[arm],
                label=f"V20l {arm} provider receipt",
            ),
            domain=_PROVIDER_DOMAIN,
            label=f"V20l {arm} provider receipt",
        )
        score_bundle = _validate_exact_score_bundle(
            evidence,
            expected_example_ids=expected_examples,
            label=f"V20l outer {arm}",
        )
        arm_objectives, arm_h4, arm_logits, arm_executions = score_bundle
        provider_artifact = _sha(
            provider_hashes[arm], label=f"V20l {arm} provider hash"
        )
        _validate_provider_receipt_evidence(
            provider_receipt,
            expected_role=arm,
        ...
        trace_artifact = _sha(
            trace_hashes[arm], label=f"V20l {arm} trace hash"
        )
        seed = _v14._sha256(
            {
                "outer_manifest_sha256": manifest["artifact_sha256"],
        ...
        response_gain_hashes = _mapping(
            trace.get("response_gain_sha256s"),
            label=f"V20l {arm} response gain hashes",
        )
        for value in response_gain_hashes.values():
            _sha(value, label=f"V20l {arm} response gain hash")
        ...
        if (
            ...
            or (
                ...
                and (
                    ...
                    or _v14._canonical_json_bytes(
                        _mapping(
                            provider_receipt.get("box_certificate"),
                            label=f"V20l {arm} signed_stack certificate",
                        )
                    )
                    != _v14._canonical_json_bytes(
                        fisher_soft_polarity_signed_stack_box_certificate(
                            ...
                            signed_mix=expected_soft_responses[arm][1],
                        )
                    )
                )
            )
            ...
            raise ValueError("V20l outer arm health differs")
    ...
    _v20b._validate_capability_receipt(
        ...
        expected_held_family_id=None,
        expected_accesses_per_example=len(_ARMS),
        label="V20l resumed outer-held capability",
    )
    control_anchors: dict[str, bool] = {}
    for arm in ("base", "fixed_plus", "fixed_minus"):
        inherited = _mapping(
            inherited_arms[arm], label=f"V20l inherited V20g {arm} arm"
        )
        ...
        control_anchors[arm] = bool(
            ...
            == {
                _identifier(example, label=f"V20l inherited {arm} example"): float(
                    objective
                )
                for example, objective in _mapping(
                    inherited.get("objectives_by_example"),
    ...
    persisted_control_anchors = _mapping(
        ...
        label="V20l persisted control anchors",
    )
    if (
        ...
        raise ValueError("V20l outer V20g control output anchor differs")
    ...
    fold = _validate_hashed(
        _mapping(fragment.get("fold_receipt"), label="V20l fold receipt"),
        domain=_DECISION_DOMAIN,
        label="V20l fold receipt",
    )
    if (
        ...
        or fold.get("selected_signed_mix_nonzero")
        ...
        raise ValueError("V20l fold decision receipt differs")
...
def _publish_fold_fragment(
    ...
    _v20b._publish_scalar_fragment(
        ...
        label="V20l signed_stack fold fragment",
    )
...
def _fold_receipt_map(
    ...
    return {
        family: _mapping(
            fragments[family].get("fold_receipt"), label="V20l aggregate fold"
        )
...
def _aggregate_decision(
    ...
    if len(families) != _FAMILY_COUNT:
        raise ValueError("V20l decision requires all eight outer folds")
    ...
    for family in families:
        ...
        if tuple(fold.get("arm_order", ())) != _ARMS:
            raise ValueError("V20l aggregate arm order differs")
        raw_scores = _mapping(
            fold.get("held_objective_by_arm"), label="V20l aggregate arm scores"
        )
        if set(raw_scores) != set(_ARMS):
            raise ValueError("V20l aggregate arm geometry differs")
        scores[family] = {arm: float(raw_scores[arm]) for arm in _ARMS}
        if not all(math.isfinite(value) for value in scores[family].values()):
            raise ValueError("V20l aggregate score became nonfinite")
    ...
    nonzero_signed_mix_count = sum(
        responses[family][1] != 0.0 for family in families
    )
    signed_stack_evidence = nonzero_signed_mix_count >= 5
    ...
    mechanism_gate = (
        integrity
        and positive_changed
        and signed_stack_evidence
        and macro[_PRIMARY_ARM] < macro["same_stack_unreflected"]
        and wins_vs["same_stack_unreflected"] >= 5
        and macro[_PRIMARY_ARM] < macro["signed_stack_reflected_exact_mirror"]
        and wins_vs["signed_stack_reflected_exact_mirror"] >= 6
    ...
    return _hashed(
        {
            ...
            "selected_signed_mix_nonzero_by_family": {
                family: responses[family][1] != 0.0 for family in families
            },
            "selected_signed_mix_nonzero_count": nonzero_signed_mix_count,
            "signed_stack_evidence_gate_passed": signed_stack_evidence,
...
def _runner_work_accounting() -> dict[str, object]:
    """Return the fixed canonical V20l one-shot schedule.

    Authentication and resume attempts are deliberately excluded.  V20l
    ...
    if (
        ...
        or inner_forwards != 1680
        or outer_forwards != 112
        or teacher_accesses != 1904
    ):
        raise RuntimeError("V20l canonical work schedule drifted")
    return {
    ...
    *,
    ...
    aggregate = (
        ...
        if decision is None
            ...
            decision, domain=_DECISION_DOMAIN, label="V20l aggregate decision"
        )
    )
    families = tuple(
        _identifier(item, label="V20l report family")
        for item in _sequence(
            aggregate.get("family_ids"), label="V20l report families"
        )
    )
    if (
        ...
        raise RuntimeError("V20l report requires all eight authenticated folds")
    replayed = _aggregate_decision(folds)
    if _v14._canonical_json_bytes(replayed) != _v14._canonical_json_bytes(
        aggregate
    ):
        raise ValueError("V20l supplied decision differs from fold replay")
    ...
    report = {
        ...
        "classification": (
            "soft_polarity_signed_stack_nested_oof_passed_fresh_shadow_eligible"
            if passed
            else "soft_polarity_signed_stack_nested_oof_failed_rollback_to_base"
        ),
        "passed": passed,
        "development_oof_passed": passed,
        "primary_development_gate_passed": (
        ...
        "integrity": {
            "V20g_V20i_V20j_V20k_reports_and_all_fragments_authenticated_before_model_"
            "construction": True,
            ...
            "all_105_inner_providers_and_traces_frozen_before_each_outer_fold_"
            "inner_scoring": True,
...
def _load_existing_report(
    ...
    *,
    ...
    value = _v20b._load_scalar_fragment(
        ...
        label="V20l signed_stack nested report",
    )
    families = tuple(sorted(authenticated_v20g_folds))
    if set(authenticated_v20i_folds) != set(families):
        raise ValueError("V20l report authority family geometry differs")
    folds = {
    ...
    if (
        ...
        raise ValueError("V20l report reconstruction differs")
    return dict(value)


def run_gemma3_l3_l4_complete_h4_soft_polarity_signed_stack_nested_development(
    *,
    output: Path | str = DEFAULT_OUTPUT,
    cache_dir: Path | str | None = None,
) -> dict[str, object]:
    """Run or resume the V20l nested reflection/response development screen."""
    ...
    panel_receipt = dict(
        _mapping(
            prerequisite.get("nested_panel_receipt"), label="V20l panel receipt"
        )
    )
    bridge_binding = _sha(
        prerequisite.get("authenticated_bridge_binding_sha256"),
        label="V20l bridge binding",
    )
    ...
    if (
        ...
        or set(
            _mapping(
                ...
                label="V20l panel families",
            )
        )
        != set(family_ids)
    ):
        raise RuntimeError("V20l authenticated family geometry differs")
    ...
    try:
        ...
        if context.bridge.bridge_binding_sha256 != bridge_binding:
            raise RuntimeError("V20l live bridge differs from authenticated authority")
        ...
        if tuple(live_families) != family_ids:
            raise RuntimeError("V20l live family order differs from authenticated A16")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the V20l nested six-family Fisher reflection and exact-response "
            "development screen"
        )
    )
