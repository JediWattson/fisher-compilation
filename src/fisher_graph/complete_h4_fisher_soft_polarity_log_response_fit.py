"""Pure V20k conditional-inner-LOFO log-response selection protocol.

The scalar response is

``q(z) = tanh(r * ((1-lambda)*z + lambda*asinh(4*z)/4))``.

This module freezes one zero control and ten ``(r, lambda)`` candidates before
it accepts any objective.  It then selects on the family-equal mean of seven
token-mean exact-float64 full-vocabulary ``KL(teacher || candidate)`` values.
The seven conditional inner leave-one-family-out scores share one endpoint
fitted on those seven families; consequently this is calibrator selection on a
fixed endpoint, not fully nested endpoint fitting.  The outer-held family is
absent from endpoint, direction, reflection, and calibrator selection.
...
__all__ = [
    "SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS",
    "SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256",
    "SOFT_POLARITY_LOG_RESPONSE_LADDER",
    "SOFT_POLARITY_LOG_RESPONSE_PAIRS",
    "build_soft_polarity_log_response_fit_receipt",
    "build_soft_polarity_log_response_inner_oof_selection_receipt",
    "build_soft_polarity_log_response_ladder_receipt",
    "soft_polarity_log_response_q",
    "validate_soft_polarity_log_response_fit_receipt",
    "validate_soft_polarity_log_response_inner_oof_selection_receipt",
    "validate_soft_polarity_log_response_ladder_receipt",
]
...
SOFT_POLARITY_LOG_RESPONSE_PAIRS = SOFT_POLARITY_LOG_RESPONSE_LADDER
SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS = tuple(
    f"log_response_{index:02d}"
    for index in range(len(SOFT_POLARITY_LOG_RESPONSE_LADDER))
)

_SHA = re.compile(r"^[0-9a-f]{64}$")
_DEVELOPMENT_FAMILY_COUNT = 8
_INNER_FAMILY_COUNT = 7
_FORMULA = "q(z)=tanh(r*((1-lambda)*z+lambda*asinh(4*z)/4))"
_OBJECTIVE_KIND = (
...
_SELECTION_KEY = (
    "family_equal_token_mean_exact_float64_full_vocabulary_"
    "kl_teacher_to_candidate_then_smaller_lambda_then_smaller_r_then_"
    "fixed_ladder_index_then_candidate_artifact_sha256"
)

_PROTOCOL_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:protocol:v20k\0"
)
_CANDIDATE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:candidate:v20k\0"
)
_LADDER_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:ladder:v20k\0"
)
_OBJECTIVE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:objective:v20k\0"
)
_FAMILY_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:family:v20k\0"
)
_AGGREGATE_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:aggregate:v20k\0"
)
_SELECTION_DOMAIN = (
    b"fisher-graph:complete-h4-soft-polarity-log-response-fit:selection:v20k\0"
)

_DATA_BOUNDARY = {
    "role": "v20k_development_conditional_inner_lofo_log_response_selection_only",
...
_PROTOCOL = {
    "protocol": "v20k_conditional_inner_lofo_soft_polarity_log_response_selection",
    "scientific_status": "development_only_after_v20j",
    "formula": _FORMULA,
    "candidate_order": SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS,
    "candidate_pairs_r_lambda": SOFT_POLARITY_LOG_RESPONSE_LADDER,
    "candidate_count": len(SOFT_POLARITY_LOG_RESPONSE_LADDER),
...
SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256 = _hash(
...
def soft_polarity_log_response_q(
    z: float, *, r: float, lambda_: float
) -> float:
    """Evaluate the finite scalar V20k log-response map."""

    selected_z = _number(z, "log-response input z")
    selected_r = _number(r, "log-response coefficient r")
    selected_lambda = _number(lambda_, "log-response mixture lambda")
    if selected_r < 0.0:
        raise ValueError("log-response coefficient r must be nonnegative")
    if not 0.0 <= selected_lambda <= 1.0:
        raise ValueError("log-response mixture lambda must be inside [0,1]")
    scaled_z = 4.0 * selected_z
    if not math.isfinite(scaled_z):
        raise ValueError("log-response calibrator intermediate must be finite")
    curved = math.asinh(scaled_z) / 4.0
    try:
        mixture = math.fsum(
            (
                (1.0 - selected_lambda) * selected_z,
                selected_lambda * curved,
            )
        )
        argument = selected_r * mixture
    except OverflowError as error:
        raise ValueError(
            "log-response calibrator intermediate must be finite"
        ) from error
    if not math.isfinite(mixture) or not math.isfinite(argument):
        raise ValueError("log-response calibrator intermediate must be finite")
    result = math.tanh(argument)
    if not math.isfinite(result):
        raise ValueError("log-response calibrator output must be finite")
    return 0.0 if result == 0.0 else result


def _candidate_receipt(index: int) -> dict[str, object]:
    candidate_id = SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS[index]
    r, lambda_ = SOFT_POLARITY_LOG_RESPONSE_LADDER[index]
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_log_response_candidate.v20k",
        _CANDIDATE_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256,
            ...
            "formula": _FORMULA,
            "zero_maps_to_zero": (
                soft_polarity_log_response_q(0.0, r=r, lambda_=lambda_) == 0.0
            ),
...
def _expected_ladder() -> dict[str, object]:
    ...
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_log_response_ladder.v20k",
        _LADDER_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256,
            "formula": _FORMULA,
...
def build_soft_polarity_log_response_ladder_receipt() -> dict[str, object]:
    """Return the authenticated, immutable eleven-candidate V20k ladder."""

    result = _expected_ladder()
    validate_soft_polarity_log_response_ladder_receipt(result)
    return result


def validate_soft_polarity_log_response_ladder_receipt(
    value: Mapping[str, object],
) -> None:
    """Authenticate every candidate, its order, fields, and ladder hash."""

    receipt = _mapping(value, "log-response ladder receipt")
    expected = _expected_ladder()
    if set(receipt) != set(expected):
        raise ValueError("log-response ladder receipt key set drifted")
    raw_candidates = _sequence(
        receipt.get("candidate_receipts"), "log-response ladder candidates"
    )
    if len(raw_candidates) != len(SOFT_POLARITY_LOG_RESPONSE_LADDER):
        raise ValueError("log-response ladder candidate geometry drifted")
    for index, raw in enumerate(raw_candidates):
        candidate = _mapping(raw, "log-response candidate receipt")
        expected_candidate = _candidate_receipt(index)
        if set(candidate) != set(expected_candidate):
            raise ValueError("log-response candidate receipt key set drifted")
        _sha(candidate.get("artifact_sha256"), "log-response candidate artifact")
        if _canonical(candidate) != _canonical(expected_candidate):
            raise ValueError("log-response candidate receipt content drifted")
    _sha(receipt.get("artifact_sha256"), "log-response ladder artifact")
    if _canonical(receipt) != _canonical(expected):
        raise ValueError("log-response ladder receipt content drifted")
...
def _objective_rows(
    ...
    *,
    ...
    candidate_artifacts = _mapping(
        ...
        "log-response candidate artifacts",
    )
    ...
    for family in inner_families:
        raw_candidates = _mapping(
            raw_families[family], f"{family} log-response exact KL objectives"
        )
        if set(raw_candidates) != set(SOFT_POLARITY_LOG_RESPONSE_CANDIDATE_IDS):
            raise ValueError(
            ...
            objective_hashes[family][candidate_id] = _hash(
                _OBJECTIVE_DOMAIN,
                {
                    ...
                    "candidate_artifact_sha256": _sha(
                        ...
                        "log-response candidate artifact",
                    ),
                    "objective": objective,
        ...
        family_receipts[family] = _finish(
            "fisher_graph.complete_h4_soft_polarity_log_response_family_oof.v20k",
            _FAMILY_DOMAIN,
            {
                "protocol_sha256": (
                    SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256
...
def _build_selection_receipt(
    *,
    ...
    """Select one frozen response pair on seven family-equal exact KL scores."""

    validate_soft_polarity_log_response_ladder_receipt(ladder_receipt)
    ladder = _mapping(ladder_receipt, "log-response ladder receipt")
    ...
    candidate_by_id = {
        _identifier(candidate["candidate_id"], "log-response candidate id"): candidate
        for candidate in _sequence(
            ladder["candidate_receipts"], "log-response candidate receipts"
        )
    }
    ranking = tuple(
        sorted(
            ...
            key=lambda candidate_id: (
                ...
                float(candidate_by_id[candidate_id]["lambda"]),
    ...
    return _finish(
        "fisher_graph.complete_h4_soft_polarity_log_response_selection.v20k",
        _SELECTION_DOMAIN,
        {
            "protocol_sha256": SOFT_POLARITY_LOG_RESPONSE_FIT_PROTOCOL_SHA256,
...
def build_soft_polarity_log_response_inner_oof_selection_receipt(
    *,
    ladder_receipt: Mapping[str, object],
    all_development_family_ids: Sequence[str],
    outer_held_family_id: str,
    ...
    """Select one V20k response pair from conditional-inner-LOFO KL scores."""
...
def validate_soft_polarity_log_response_inner_oof_selection_receipt(
    value: Mapping[str, object],
    *,
    ladder_receipt: Mapping[str, object],
    all_development_family_ids: Sequence[str],
    ...
    """Replay the full V20k selection from authoritative exact KL values."""

    receipt = _mapping(value, "log-response selection receipt")
    ...
    if set(receipt) != set(expected):
        raise ValueError("log-response selection receipt key set drifted")
    _sha(receipt.get("artifact_sha256"), "log-response selection artifact")
    if _canonical(receipt) != _canonical(expected):
        raise ValueError("log-response selection receipt content drifted")
